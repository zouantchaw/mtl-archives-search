...
Reviewed Metrics v2 remains candidate-only for issue #96. Activation requires an authority-only commit whose `candidate_commit` equals its sole parent and whose only changed path is the byte-identical fixed authority file. The v2.1 authority requires distinct principals, sessions, and pairwise-distinct filesystem routes for the visual predictor, search predictor, private evaluator, gold/dossier reviewer, task reviewer, and publisher. The repo/history-denied search predictor receives only a pre-freeze neutral prompt, opaque task ID, generic HTTPS Quebec-government hostname-suffix policy, and bundle-local schema bytes—no expected/internal fields, source-specific hostname/path, or post-review artifact. Prediction freezes before dossier/envelope authoring and scoring. Gate E history remains tracked/readable, so the salted commitment provides ordered-workflow blinding and envelope integrity, not secrecy from a history reader. Place metrics derive only from a passing receipt for the opaque task. No real authority, prediction, envelope, score, or publication exists. See `docs/dataset-factory/reviewed-metrics-v2.md`.
