...
- Issue #69 public source acquisition separates ignored staging via `dataset-factory:real-pilot-sources-acquire-v1` from clean-checkout offline replay via `dataset-factory:real-pilot-sources-verify-v1`. The default verifier reads `docs/dataset-factory/fixtures/real-pilot-source-acquisition-v1/`, whose complete manifest binds every bounded payload/evidence member; use `-- --root /path/to/staging` only for a newly acquired tree. `dataset-factory:real-pilot-sources-seal-v1` deterministically refreshes the tracked manifest/descriptor after reviewed acquisition changes.
...
- Visual Family Graph v1 is documented in `docs/dataset-factory/visual-family-graph-v1.md`. It covers the 18,462-record Canonical Corpus identity universe with typed evidence, streaming-capped and identity-bound 256px DCT-pHash features, complete linear aerial-run grouping, explicit CLIP/DINO coverage contracts, grouping-authoritative versus review-only edges, component-safe benchmark splits, component-wide canonical/alternate recommendation support, review precision, and frozen search duplicate-rate evaluation. Its public API recapture has its own content-derived acquisition snapshot ID and references, but never impersonates, the issue-66 bytes.
...
Gate H2 broker runtime source lives in `crates/gate-h2-broker`. `npm run gate-h2:broker-self-test` enforces locked Rust checks, the frozen issue #98 oracle, strict OCI/admission/publication negatives, and the exact source descriptor. The production stage uses fixed inherited connected Unix streams; launch replay uses a privileged pre-opened append journal bound by trusted inode identity; signing-key launch material is exactly 43 canonical unpadded base64url bytes for one 32-byte Ed25519 seed; request and credential descriptor sets, lengths, and aggregate bounds are checked before reads; and raw output evidence hashes the exact retained output FD. The digest-pinned builder compiles both host helpers twice from the exact allowlisted source, compares them, and emits their independent digests; the host verifies those artifacts, unlinks their paths on Linux, and retained-FD executes them with a strict environment. Admission and publication stay in one process. Their inherited interface is one unlinked admission descriptor FD, one retained destination-parent directory FD, and six ordered unlinked admitted member FDs. Admission closes the candidate source-directory FD before returning that publication capability, and publication never consumes a source pathname or source-directory capability. Publication carries exact admitted sizes/hashes/modes, verifies retained identities, keeps a durable descriptor-hash-bound `published` receipt state, and distinguishes observed exact conflicts from unknown rename errors. The frozen issue #98 bytes/IDs and exact OCI/SBOM joins remain unchanged. Issue #100 owns implementation of the Podman supervisor, broker lifecycle, connected channel provisioning, D1 begin/completion chronology, and transcript/output joins. Issue #101 depends on that #100 implementation and owns independent real-Linux conformance and admission proof; it cannot replace or skip #100. Nothing here activates Cloudflare, D1, R2, providers, publication authority, or deploys.

Issue #104 adds an inactive source-pinned post-begin enrollment contract for issue #100. Its only production admission entry point derives the shipped root itself and always fails closed for the synthetic fixture; it has no caller-selected adapter, fetch, D1 account/database/endpoint, or credential. It fixes future verifier, D1 identity, native retained-descriptor, request-byte, endpoint/replay/report, authority/launch, and completion-reconciliation contracts, but #100 still owns concrete retained-FD validation and coordinator-owned capability use. See `docs/dataset-factory/gate-h2-post-begin-authority-enrollment-v1.md`.
