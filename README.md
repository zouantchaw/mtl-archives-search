...
Gate H2 broker runtime source lives in `crates/gate-h2-broker`. `npm run gate-h2:broker-self-test` runs locked Rust format/clippy/tests plus the production issue #98 TypeScript artifact oracle. Evidence append/fsync faults are fail-sticky after handle consumption: multi-exchange authority stops, unsupported responses are never released, and sealing rejects every nonterminal or invalid lifecycle. The frozen v1 semantic contract and IDs remain unchanged; runtime transcript artifacts are exactly JCS UTF-8 plus one LF, the v2 envelope hashes those retained bytes, and the oracle rejects byte reformatting. Envelope verification loads an exact schema-pinned signer trust-entry artifact and joins its signer ID and Ed25519 key, so a self-selected key with a copied digest fails. Raw `.bin` outputs remain exact, including empty bodies. OCI assembly fixes the umask and explicitly normalizes all archived modes; local synthetic assembly proves caller-umask independence, while issue #101 still owns the reviewed builder digest and independent Linux static-linkage, memfd, sandbox, external-TLS, and two-clean-build proof. These files activate no Cloudflare, D1, R2, provider, prediction, publication, or deploy action.
