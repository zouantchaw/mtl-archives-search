...
Gate H2 broker runtime source lives in `crates/gate-h2-broker`. `npm run gate-h2:broker-self-test` runs locked Rust format/clippy/tests plus the production issue #98 TypeScript artifact oracle. Local raw TCP/rustls fixtures exercise truthful DNS/TLS/request milestone prefixes as well as redirects, compression, transfer encoding, malformed/duplicate lengths, overflow, timeout, truncation/trailing bytes, SNI failure, and mixed DNS answers; UDS fixtures cover peer, setup, accept deadline, seal-before-return, nonterminal delivery failure, and terminal evidence without provider or internet traffic. Production admission requires a detached Ed25519 signature over every strict canonical config field, checked against independently embedded key identity, minimum sequence, validity bounds, and owner-only durable replay state; the signed final broker pin avoids self-attestation without creating an executable hash cycle. Ordinary builds embed neither code admission nor authority trust. Raw `.bin` outputs are reopened and hashed after exact no-newline commit, and real broker/UDS/stage tests cover zero bytes, trailing/no trailing newline, and empty bodies. V1 transcripts remain byte-compatible with #98; the v2 evidence envelope is unchanged. The hermetic build contract requires a separately reviewed digest-pinned builder image to supply all output-affecting tools and retains that input in provenance. Issue #101 must supply the real digest and independent Linux static-linkage, memfd, sandbox, external-TLS, and two-clean-build binary/rootfs/OCI evidence. These files activate no Cloudflare, D1, R2, provider, prediction, publication, or deploy action.
