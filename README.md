...
Gate H2 broker runtime source lives in `crates/gate-h2-broker`. `npm run gate-h2:broker-self-test` runs locked Rust format/clippy/tests plus the production issue #98 TypeScript artifact oracle. Local raw TCP/rustls fixtures exercise redirects, compression, transfer encoding, malformed/duplicate lengths, overflow, timeout, truncation/trailing bytes, SNI failure, and mixed DNS answers through the production parser; UDS fixtures cover peer, setup, accept-deadline, seal-before-return, and terminal evidence behavior without provider or internet traffic. The inactive launcher embeds a reviewed code identity independent of its final executable hash, while the exact config admission ID binds that identity and the measured broker binary, avoiding a SHA fixed point. It verifies the full native-root `FilePin`, requires fully sealed inherited Linux memfds, and carries no admitted identity in ordinary builds. The stage binds accepted responses to request IDs, ordered authority roles/statuses, and retained output bytes. V1 transcripts remain byte-compatible with #98; the closed v2 authority envelope binds D1 begin/attempt, session, binaries, runtime, roots, socket/token commitments, manifest, signer, and outcome. The checked toolchain lock pins every output-affecting build-tool identity and is retained in provenance. Issue #101 retains only independent real-Linux execution evidence for static linkage, sealed memfds, sandbox flags, external TLS behavior, and two-clean-build binary/rootfs/OCI equality. These files activate no Cloudflare, D1, R2, provider, prediction, publication, or deploy action.
