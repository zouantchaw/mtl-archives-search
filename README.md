...
Verified Multimodal Intelligence Batch 001 now has a synthetic foundation, a 26-record canonical-real candidate pool, and a schema-bound primary visual promotion. Independent visual review approved all 16 primary decisions with zero disagreements: 12 selected records (6 ground and 6 aerial/control) and 4 reserves. Historical verification, completed dossiers, and derived benchmarks remain zero, so issue #69 acceptance remains open. See [docs/dataset-factory/verified-multimodal-batch-001.md](docs/dataset-factory/verified-multimodal-batch-001.md).
...
- `docs/dataset-factory/artifact-registry.v0.jsonl` is the machine-readable registry for ignored artifacts and durable archives. Its 101 entries form an acyclic phase graph and include the Gold Label Batch 002 evidence, the Verified Multimodal foundation, and tracked descriptors for the issue #69 candidate, primary visual-promotion, and independent visual-review outputs. Broad trees remain exact, non-overlapping file sets with explicit human-decision and acquisition boundaries.
