...
Reviewed Metrics v2 remains candidate-only for issue #96. Activation requires an authority-only direct-child commit whose `candidate_commit` equals its sole parent and whose only changed path is the byte-identical fixed authority file. The v2.1 authority binds an evaluator Ed25519 public key and seven disjoint role identities (implementation, visual prediction, source search, private evaluation, gold/dossier review, task review, publication), each with an evidence-derived `surface_id` and signed host-route receipt binding requested and canonical physical roots; same-surface equality, ancestry, Kami `/tmp` aliases, remote symlink aliases, relabeling, and forged receipts are rejected. Remote isolation is proven only by a supplied signed/verified host-route receipt; sanitized sync inventory remains separate operational evidence. The public source bundle has exactly two files and exposes the public subject `The Gazette`, neutral prompt/opaque task ID, generic Quebec-government HTTPS policy, deterministic normalization contract, and bundle-local output schema. Exact bundle bytes/inventory/joins are validator evidence; repository/history denial is an operational attestation supported by sanitized sync inventory, a fresh isolated route, cleanup, and readback, not code-proven. Completion requires the exact issue #96 metric universe, fixed denominators, gold-support gates, stop conditions, and honest mask/geolocation N/A plus cost explicitly unavailable because v2 has no usage-receipt ingestion path. Protected private finalization opens retained inputs once without following symlinks, validates/hashes the same immutable buffers, fsyncs and exact-compares canonical detail bytes, rechecks inode metadata, and signs the exact public receipt; offline replay verifies the signature and full frozen chain without tracking plaintext, salt, or private detail. No real key, authority, prediction, gold/private evidence, score, or publication exists. See `docs/dataset-factory/reviewed-metrics-v2.md`.
