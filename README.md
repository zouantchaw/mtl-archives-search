...
Issue #90 Gate F aerial source evidence is documented in `docs/dataset-factory/aerial-source-evidence-v1.md`. The candidate-only tracked contract binds 20 Phase D aerial records plus two component-distinct reserves to 946,387,779 exact official media bytes, hashes, dimensions, transport, rights, and attribution evidence. The deterministic private archive and authenticated readback are complete, but authority remains `archived_candidate_external_review_required`: publication requires a fresh independent review accepting the complete record/media, collection, license, and credit proposition set for all 22 records. Location, scale, land use, and measurement remain held or abstained, and verified dossier/task/production-mutation counts remain zero.
