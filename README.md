...
Gate H2 broker runtime source lives in `crates/gate-h2-broker`. `npm run gate-h2:broker-self-test` runs its locked, offline Rust format/clippy/unit/adversarial checks; it performs no DNS, TCP, TLS, provider, or internet request. The broker core enforces the merged issue #98 manifest and UDS contracts, ordered handles, owner-only socket policy, inherited credential descriptors, exact request/response bounds, hash-only fsynced evidence, and detached transcript signing. The production TLS launcher and Linux `FROM scratch` image remain deliberately disabled pending the issue #101 clean-Linux static/TLS/OCI gates documented in the crate README. These files activate no authority and authorize no Cloudflare, D1, R2, provider, prediction, publication, or deploy action.
