...
Reviewed Metrics v2 remains candidate-only for issue #96. Activation still requires the authority-only direct-child commit and exact clean-parent controls. The v2.2 authority binds one coordinator Ed25519 trust public key and an exact seven-entry trusted surface inventory; each `surface_id` derives from canonical inventory bytes, and every one-shot route receipt is signed by that coordinator key over the candidate, authority binding hash, nonce, role, inventory digest, requested route, canonical physical root, device/inode/ancestor evidence, issue/expiry times. Local inventory uses a coordinator-verified stable host UUID; AWS/Kami inventory requires a signed EC2 identity-document digest and instance ID or a coordinator-verified SSH host fingerprint plus instance identity. Duplicate inventory mappings, self-signed receipts, wrong/stale bindings, nonce replay, same-surface equality/ancestry, `/tmp` aliases, and symlink aliases are rejected. This is coordinator-attested operational evidence, not hardware remote attestation unless an AWS identity signature is actually verified.

Private finalization opens inputs and the evaluator key descriptor-first with `O_NOFOLLOW`, requires regular one-link owner files at exact private modes, checks path/descriptor inode identity, and hashes/parses/scores retained exact buffers. Canonical envelope/detail objects are sealed `O_EXCL` at `0400` with file and directory fsync. The CLI performs no R2 write: a first scoring pass can prepare the sealed detail, and finalization requires `--retention-receipt` containing evaluator-signed, content-addressed private-store write/readback evidence for the exact envelope/detail bytes, opaque object-key hashes, provider/bucket capability ID, version/etag when available, timestamps, and no-public-ACL/readback attestations. Only then may the final public receipt be signed; synthetic tests use an internal memory store.
