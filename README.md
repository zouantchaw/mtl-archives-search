...
Reviewed Metrics v2 remains candidate-only for issue #96. Activation requires an authority-only commit whose `candidate_commit` equals its sole parent and whose only changed path is the byte-identical fixed authority file. The v2.1 unified authority preserves the visual-run fields and adds distinct search-predictor/private-evaluator identities, routes, and strictly later source-search timestamps. The exact `c0-rpcq` chain uses a sanitized public search bundle, distinct raw-byte prediction freeze, uncommitted salted private expected envelope, deterministic private detail, non-leaking public receipt, and independent task review. Place metrics derive only from a passing receipt for that exact task—never from visual rows or authored booleans—and final matrix/publication parse the entire chain. These remain synthetic-tested candidate guarantees: no real authority, prediction, envelope, score, or publication exists. See `docs/dataset-factory/reviewed-metrics-v2.md`.
