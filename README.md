...
Gate H2 broker runtime source lives in `crates/gate-h2-broker`. `npm run gate-h2:broker-self-test` runs locked Rust format/clippy/tests plus the production issue #98 TypeScript artifact oracle. Its network fixture is local loopback only: a real resolve-once abstraction, pinned TCP peer, rustls TLS 1.3 SNI/PKIX session, exact HTTP framing, UDS service, and stage runtime are exercised without provider or internet traffic. The production launcher is implemented but remains inactive because no admitted launch configuration or provider authority is tracked. It requires byte-pinned native roots and fully sealed inherited Linux memfds for all credentials, the one-run token, and the Ed25519 seed. V1 transcripts remain byte-compatible with #98; the closed v2 authority envelope binds D1 begin/attempt, session, binaries, runtime, roots, socket/token commitments, manifest, signer, and outcome. Issue #101 retains only independent Linux static, sealed-memfd, sandbox, and two-clean-build OCI evidence. These files activate no Cloudflare, D1, R2, provider, prediction, publication, or deploy action.
