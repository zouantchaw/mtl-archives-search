...
- [x] Issue #96 Reviewed Metrics v2 candidate contracts validate parent-relative authority-only activation; pairwise non-overlapping implementation/visual/search/evaluator/gold/task/publisher routes; a portable pre-freeze public task whose non-answer organization subject identifies the official-record search without exposing scored fields or internal evidence; both freezes before gold; envelope sealing before scoring; bounded salt-generation chronology; dynamic joins to tracked/readable Gate E history; deterministic normalized private scoring and receipt-only place metrics; canonical input-authority binding; and exact commit-last publication membership. Code/tool evidence proves the sanitized two-file bundle bytes, sync inventory shape, and validator joins. Repository/history denial remains an operational attestation evidenced by isolated Kami/Crabbox sync inventory, fresh route, cleanup, and readback, not a validator-proven property. The commitment protects integrity/workflow blinding rather than secrecy from Gate E history readers; real authority/prediction/envelope/score/publication remain absent.
