...
- [x] Issue #96 Reviewed Metrics v2 candidate contracts now cover the exact 63-row metric universe with fixed IDs/denominators, raw counts, support/provenance, reviewed-scene place precision/coverage, explicit aerial reviewability exclusions, gold-support/stop gates, prerequisite N/A rows, actual operation timing, and receipt-only cost availability. Authority activation remains a direct-child-only candidate gate and now binds all seven principals/sessions to stable surface IDs plus canonical physical roots, rejecting same-surface aliases and ancestry. The exact two-file public bundle includes the public subject and normalization contract; code proves its bytes/inventory/joins, while repository/history denial remains operational evidence from isolated sync inventory, fresh route, cleanup, and readback. Protected private finalization validates retained envelope/detail bytes, recomputes commitment and score, binds their hashes to the frozen chain, and emits an Ed25519-signed public receipt for offline verification. Results require private score time strictly before final scoring; exact-code adversarial tests cover raw metric recomputation, signatures, aliases, chronology, and coherently repinned publication universes. Real key/authority/prediction/gold/private evidence/score/publication remain absent.
