...
- [x] **Canonical Corpus v1 (#66)** — Added production-read-only local/D1/R2/text/CLIP inventory collectors, source-identity alias reconciliation with exactly one primary state per observed identity, individually enumerated blockers, generated rights/enrichment/vector/R2 coverage summaries and immutable hashes, a 12-state offline deterministic fixture, six registry bundles, and a no-write convergence/rollback plan. Current counts and decisions are generated in `docs/dataset-factory/canonical-corpus-v1-snapshot-summary.json`.
