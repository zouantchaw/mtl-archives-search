...
- [ ] **Issue #101 real Linux/D1 evidence and admission** — Implement the pinned hermetic builder recipe and cross-surface receipt comparator, then independently prove real-Linux conformance and admission, including `SO_PEERCRED`, mandatory sealed memfds, static linkage, sandbox flags, external TLS conformance, D1 chronology, and two-clean-source binary/rootfs/OCI byte identity. No paid host, Podman execution, D1 access or mutation, credentials, provider/model call, activation, publication, or deploy has occurred; real Linux/D1 execution requires explicit approval.
