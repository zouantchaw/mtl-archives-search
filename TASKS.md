...
- [ ] Issue #100 implementation remains inactive: the retained combined relay/supervisor pin has fixed request, authorizer, and relay-liveness inputs; source-pinned compiled enrollment supplies verifier trust. Its signed SCM_RIGHTS grant carries config, replay journal, empty report, canonical completion-expectation bytes, exact supervisor-config expectation bytes, and HTTPS broker descriptors. Rust and TypeScript verify the same exact retained descriptor bytes and every config/channel commitment before replay or process creation; none mode requires exact null/empty values. A config names a pinned no-follow base run root plus a single invocation child; only the exclusively created child is eligible for cleanup. Typed raw-response roles must exact-join their declared writable host paths, and retained broker evidence carries that path. The supervisor-owned liveness pipe reaches a fixed container stage-runtime watchdog FD and EOF terminates that workload. The handshake cap is separate from Rust's absolute lifecycle deadline and cleanup reserve. Exact 12-stage configs use typed output/work transitions and supervisor-owned immutable read-only snapshots. Strict reports retain signed grant expectations and all D1/session/program/runtime-source/image/Podman/trust/mount/output joins; none mode alone has null broker evidence. Synthetic tests remain production-ineligible; real Linux/Podman conformance and admission remain exclusively issue #101.
- [ ] Issue #101 dependent evidence only: after #100 is implemented, independently approve/supply the builder-image digest and prove real-Linux conformance and admission, including `SO_PEERCRED`, mandatory sealed memfds, static linkage, sandbox flags, external TLS conformance, and two-clean-source binary/rootfs/OCI byte identity. #101 cannot replace or skip #100 implementation.
