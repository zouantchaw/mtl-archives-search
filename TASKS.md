...
- [x] **Canonical Corpus v1 (#66)** — Added production-read-only local/D1/R2/text/CLIP inventory collectors, exact tracked 12-input snapshot lineage, source-identity alias reconciliation with exactly one primary state per observed identity, full derived-summary and row consistency checks, 54 adversarial offline cases, bounded R2 sample planning, individually enumerated blockers, six registry bundles, and an identity-level no-write convergence/rollback plan. Current counts and decisions are generated in `docs/dataset-factory/canonical-corpus-v1-snapshot-summary.json`.
