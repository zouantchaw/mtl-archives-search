...
- [x] **Dataset Factory v0 durability gate (#65)** — Restored tracked Dataset Factory v0 docs/scripts, added a 27-entry machine-readable artifact registry with SHA-256/count/lineage/rights metadata, added missing-artifact diagnostics, and added `npm run dataset-factory:smoke-v0` so packets, labels/adjudication, benchmark, active learning, search/reranker evaluation, quality repair, visual family graph, research enrichment, judgments, and reward data can be smoke-tested from tracked fixtures without copying generated report trees.
