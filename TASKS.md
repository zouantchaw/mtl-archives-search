...
- [x] **Dataset Factory v0 durability gate (#65)** — Restored tracked Dataset Factory v0 docs/scripts; added a 49-entry, schema-validated, acyclic artifact phase graph with explicit human-decision inputs and SHA-256/count/lineage/rights metadata; added missing-artifact diagnostics and semantic negative tests; and made `npm run dataset-factory:smoke-v0` a fixed-clock exact-content/hash contract for packets, labels/adjudication, benchmark, active learning, search/reranker evaluation, quality repair, visual family graph, research enrichment, judgments, and reward data without copying generated report trees.
