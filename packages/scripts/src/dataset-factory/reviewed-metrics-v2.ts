...
function validateGoldValue(value: J): void {
  ...
  if (value.status === "blank_external_review_required") assert(value.bundle_tree_sha256 === null && value.reviewer === null && value.reviews.length === 0 && value.source_task_dossier_decision === null && value.private_expected_commitment === null && value.reviewed_exclusions.length === 0, "blank gold must contain no review evidence");
    ...
    const exclusions = new Map(value.reviewed_exclusions.map((x: J) => [x.opaque_id, x.reason])); exactSet([...exclusions.keys()] as string[], value.reviews.filter((row: J) => row.exclusion !== null).map((row: J) => row.opaque_id), "gold exclusion IDs"); let reviewableAerial = 0;
    for (const row of value.reviews) {
      assert(IMAGE_OPAQUE.includes(row.opaque_id) ? IMAGE_MODE_CLASSES.includes(row.image_mode) : row.image_mode === null, `gold image mode support ${row.opaque_id}`);
      assert(OCR_OPAQUE.includes(row.opaque_id) ? typeof row.ocr_raw === "string" && typeof row.ocr_normalized === "string" : row.ocr_raw === null && row.ocr_normalized === null, `gold OCR support ${row.opaque_id}`);
      ...
      assert((row.exclusion === null) === !exclusions.has(row.opaque_id), `gold exclusion join ${row.opaque_id}`); if (row.exclusion !== null) assert(exclusions.get(row.opaque_id) === row.exclusion, `gold exclusion reason join ${row.opaque_id}`);
    }
    assert(reviewableAerial === 12, `reviewable aerial denominator drift: ${reviewableAerial}`);
  }
}
function executionAuthority(injected?: J, capability?: InternalSyntheticCapability): J {
  let value: J;
  ...
  else { assert(fs.existsSync(EXECUTION_AUTHORITY), "prediction freeze unavailable: no committed execution authorization exists"); value = load(EXECUTION_AUTHORITY); }
  schema("execution-authorization.schema.v2.json", value); return value;
}
function freezePrediction(input: string, output: string, injected?: J, capability?: InternalSyntheticCapability): J {
  ...
  assert(authority.candidate_id === CANDIDATE_ID && authority.implementation_base_commit === IMPLEMENTATION_BASE_COMMIT, "freeze candidate binding"); assert(authority.bundle_tree_sha256 === prediction.bundle_tree_sha256, "freeze bundle binding");
...
function validateResultsValue(value: J): void {
  ...
  const universes = metricUniverses(); for (const metric of value.metrics) { exactSet(metric.fixed_universe_ids, universes[metric.metric_id], `metric universe ${metric.metric_id}`); unique(metric.excluded.map((x: J) => x.opaque_id), `metric exclusions ${metric.metric_id}`); exactSet([...metric.included_ids, ...metric.excluded.map((x: J) => x.opaque_id)], metric.fixed_universe_ids, `metric retained denominator ${metric.metric_id}`); assert(metric.denominator === metric.included_ids.length, `metric denominator ${metric.metric_id}`); if (metric.status === "observed") { assert(metric.denominator > 0 && metric.numerator !== null && metric.numerator >= 0 && metric.numerator <= metric.denominator && metric.value !== null && Math.abs(metric.value - metric.numerator / metric.denominator) < 1e-12 && metric.zero_support_reason === null && metric.raw_counts.support !== null && metric.raw_counts.support > 0, `observed metric arithmetic ${metric.metric_id}`); } else if (metric.status.startsWith("observed_undefined")) { assert(metric.denominator > 0 && metric.numerator === null && metric.value === null && typeof metric.zero_support_reason === "string" && metric.zero_support_reason.length > 0, `undefined observed metric state ${metric.metric_id}`); if (metric.status === "observed_undefined_zero_predicted_support") assert(metric.raw_counts.support === 0, `zero predicted support count ${metric.metric_id}`); } else assert(metric.denominator === 0 && metric.numerator === null && metric.value === null && typeof metric.zero_support_reason === "string" && metric.zero_support_reason.length > 0, `not-applicable metric state ${metric.metric_id}`); }
...
function validateMatrixValue(value: J): void { schema("final-criterion-matrix.schema.v2.json", value); exactSet(value.rows.map((x: J) => x.criterion_id), FINAL_CRITERION_IDS, "criterion row IDs"); if (value.status === "candidate_open") assert(!value.issue_92_complete && !value.issue_69_complete, "open matrix cannot complete issues"); else { assert(value.rows.every((row: J) => row.required && row.verdict === "satisfied" && row.result_ids.length > 0 && row.evidence.length > 0), "final matrix requires satisfied supported rows"); assert(value.issue_92_complete && value.issue_69_complete, "final matrix completion booleans"); } }
function validatePublicationValue(value: J): void { schema("publication-descriptor.schema.v2.json", value); const pins = value.members_before_descriptor; unique(pins.map((x: J) => x.path), "descriptor member path"); same(pins.map((x: J) => x.path), pins.map((x: J) => x.path).sort(), "descriptor sorted members"); assert(value.tree_before_descriptor_sha256 === hash(`${pins.map((x: J) => `${x.path}\t${x.sha256}\t${x.bytes}`).join("\n")}\n`) && value.counts.files_before_descriptor === pins.length && value.counts.bytes_before_descriptor === pins.reduce((sum: number, x: J) => sum + x.bytes, 0), "descriptor tree/count arithmetic"); if (value.status === "candidate_only") { assert(value.candidate_commit === null && Object.values(value.completion).every((x) => x === false), "candidate descriptor completion state"); return; } assert(value.candidate_commit && value.artifact_id === PUBLICATION_ID && Object.values(value.completion).every((x) => x === true), "published descriptor completion state"); for (const key of ["candidate", "execution_authorization", "prediction_freeze", "gold_review", "results", "criterion_matrix"]) assert(value.predecessors[key], `published predecessor missing: ${key}`); exactSet(pins.map((x: J) => x.path), value.required_publication_members, "publication commit-last members"); assert(value.counts.published_members === pins.length + 1, "publication member arithmetic"); }
function validateIndependentChronology(prediction: J, gold: J, freeze: J): void { assert(prediction.status === "completed" && gold.status === "completed" && freeze.status === "frozen", "completed chronology evidence required"); assert(prediction.bundle_tree_sha256 === gold.bundle_tree_sha256 && prediction.bundle_tree_sha256 === freeze.bundle_tree_sha256, "chronology bundle binding"); assert(prediction.session.principal !== gold.reviewer.identity && prediction.session.session_id !== gold.reviewer.session_id, "reviewer/predictor independence"); before(freeze.frozen_at, gold.reviewer.reviewed_at, "freeze before gold review"); }
...
async function selfTest(): Promise<J> {
  ...
  const allAbstained = structuredClone(completedResults); const abstentionMetric = allAbstained.metrics.find((metric: J) => metric.metric_id === "abstention_selective_error"); abstentionMetric.status = "observed_undefined_zero_predicted_support"; abstentionMetric.numerator = null; abstentionMetric.value = null; abstentionMetric.zero_support_reason = "all predictions abstained; fixed denominator retained"; abstentionMetric.raw_counts.support = 0; await accept(() => validateResultsValue(allAbstained));
  ...
  const syntheticAuth = syntheticExecutionAuthority(descriptor.media_tree.sha256); const predictionFile = path.join(root, "completed-prediction.json"); writeJson(predictionFile, completedPrediction); const freezeFile = path.join(root, "prediction-freeze.json"); await accept(() => freezePrediction(predictionFile, freezeFile, syntheticAuth, capability)); const freeze = load(freezeFile); await accept(() => validateIndependentChronology(completedPrediction, completedGold, freeze));
  const overlapGold = structuredClone(completedGold); overlapGold.reviewer.identity = completedPrediction.session.principal; await reject(() => validateIndependentChronology(completedPrediction, overlapGold, freeze)); const earlyGold = structuredClone(completedGold); earlyGold.reviewer.reviewed_at = "2026-07-15T00:00:02.000Z"; await reject(() => validateIndependentChronology(completedPrediction, earlyGold, freeze));
...
function syntheticCompletedGold(bundle: string): J { let aerialSeen = 0; return { schema_version: "reviewed_metrics_gold_review_authority_v2.0.0", status: "completed", candidate_id: CANDIDATE_ID, bundle_tree_sha256: bundle, reviewer: { identity: "synthetic-reviewer", session_id: "synthetic-gold-session", model: "synthetic-review-model", reasoning_effort: "synthetic", reviewed_at: "2026-07-15T00:00:05.000Z", independence: { prediction_blind: true, no_implementation_overlap: true, no_predictor_overlap: true, distinct_session: true } }, reviews: FIXED_OPAQUE_IDS.map((opaque_id) => { const aerial = AERIAL_OPAQUE.includes(opaque_id); const reviewable = aerial && aerialSeen++ < 12; return { opaque_id, image_mode: IMAGE_OPAQUE.includes(opaque_id) ? "ground_street" : null, ocr_raw: OCR_OPAQUE.includes(opaque_id) ? "synthetic" : null, ocr_normalized: OCR_OPAQUE.includes(opaque_id) ? "synthetic" : null, entities: [], place_opportunities: [], aerial_reviewable: aerial ? reviewable : null, aerial_labels: aerial ? (reviewable ? ["mixed_urban"] : ["unreviewable"]) : [], answerable: ABSTENTION_OPAQUE.includes(opaque_id) ? true : null, exclusion: null }; }), required_opaque_ids: FIXED_OPAQUE_IDS, reviewed_exclusions: [], source_task_dossier_decision: { status: "accepted", source_claim_supported: true, prediction_blind: true, rights_boundary_passed: true, no_pixel_identity_claim: true }, private_expected_commitment: { algorithm: "sha256", domain: "gate-h-v2-search-expected", value: "b".repeat(64) } }; }
function syntheticExecutionAuthority(bundle: string): J { return { schema_version: "reviewed_metrics_execution_authorization_v2.0.0", status: "activated_exact_one_shot", candidate_id: CANDIDATE_ID, implementation_base_commit: IMPLEMENTATION_BASE_COMMIT, candidate_commit: "c".repeat(40), bundle_tree_sha256: bundle, predictor: { principal: "synthetic-predictor", session_id: "synthetic-prediction-session", model: "synthetic-model", reasoning_effort: "synthetic", route: "synthetic-isolated-route" }, authorized_at: "2026-07-15T00:00:01.000Z", started_at: "2026-07-15T00:00:02.000Z", ended_at: "2026-07-15T00:00:03.000Z", freeze_at: "2026-07-15T00:00:04.000Z", expires_at: "2026-07-15T00:10:00.000Z" }; }
function syntheticCompletedResults(): J { const universes = metricUniverses(); return { schema_version: "reviewed_metrics_results_v2.0.0", status: "completed", candidate_id: CANDIDATE_ID, metrics: METRIC_IDS.map((metric_id) => ({ metric_id, fixed_universe_ids: universes[metric_id], included_ids: universes[metric_id], excluded: [], raw_counts: { tp: null, fp: null, fn: null, tn: null, edits: null, support: universes[metric_id].length }, numerator: universes[metric_id].length, denominator: universes[metric_id].length, status: "observed", value: 1, zero_support_reason: null, limitations: [] })), criterion_matrix: { required_rows: FINAL_CRITERION_IDS.length, satisfied_rows: FINAL_CRITERION_IDS.length }, limitations: [] }; }
