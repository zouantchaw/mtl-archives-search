...
const PREACTIVATION_TRUST = path.join(ROOT, "docs/dataset-factory/authorities/reviewed-metrics-v2/pre-activation-attestor-trust-v2.json");
...
const INTERNAL_AUTHORITY_VALIDATION_CONTEXT = Symbol("reviewed-metrics-v2-internal-authority-validation-context");
...
function hasInternalSyntheticCapability(authority: J, capability?: InternalSyntheticCapability): boolean {
  ...
  return capability?.[INTERNAL_SYNTHETIC_CAPABILITY] === true || internalSyntheticProcessAuthorized || INTERNAL_SYNTHETIC_AUTHORITIES.has(authority) || (
...
type RawSnapshot = Omit<FileSnapshot, "value">;
...
function rejectPlaceholderEvidence(value: J): void {
  ...
  const strings: string[] = []; const visit = (item: J): void => { if (typeof item === "string") strings.push(item); else if (Array.isArray(item)) item.forEach(visit); else if (item && typeof item === "object") Object.values(item).forEach(visit); }; visit(value);
  codedAssert(!strings.some((item) => /(?:synthetic|placeholder|example|test[-_ ]?only|dummy|fake)/i.test(item)), "H2_PREACTIVATION_PLACEHOLDER", "production attestation contains a placeholder or synthetic marker");
  codedAssert(!serialized.includes("0".repeat(64)), "H2_PREACTIVATION_PLACEHOLDER", "production attestation contains a zero placeholder commitment");
}
...
const EXECUTOR_LINUX_TEST_UNIVERSE = [
  "canonical_program_success", "program_substitution", "schema_substitution", "executor_substitution",
...
function validateExecutorSemanticsBundle(bundle: J, boundary: J, allowSynthetic = false, trust?: J): void {
  ...
  if (allowSynthetic) codedAssert(attestation.synthetic === true && receipt.synthetic === true && attestation.production_eligible === false && receipt.production_eligible === false && attestation.status === "synthetic_nonproduction_fixture" && receipt.status === "synthetic_nonproduction_fixture", "H2_EXECUTOR_SEMANTICS_SYNTHETIC", "internal executor semantics fixture must be explicitly production-ineligible");
    ...
    codedAssert(attestation.synthetic === false && receipt.synthetic === false && attestation.production_eligible === true && receipt.production_eligible === true && attestation.status === "observed_real_linux_conformant" && receipt.status === "independently_reviewed_conformant", "H2_EXECUTOR_SEMANTICS_SYNTHETIC", "production requires real independently reviewed executor semantics evidence");
...
const PREACTIVATION_ADMISSION_VALIDITY_POLICY = Object.freeze({
  version: "gate_h2_pre_activation_admission_validity_v1",
  admission_max_age_seconds: 86_400,
  host_inventory_max_age_seconds: 86_400,
  clock_skew_seconds: 60,
});
function attestorIdentity(identity: J): J {
  return { principal: identity.principal, session_id: identity.session_id, logical_route: canonicalLogicalRoute(identity.route ?? identity.logical_route), physical_route_identity_sha256: identity.physical_route_identity_sha256, public_key_sha256: identity.public_key_sha256 };
}
...
function canonicalLogicalRoute(route: unknown): string {
  codedAssert(typeof route === "string" && route.length > 1 && /^[\x20-\x7e]+$/.test(route), "H2_LOGICAL_ROUTE_CANONICAL", "logical route must be a non-root absolute ASCII route");
  codedAssert(!route.includes("\\") && !route.includes("%") && !route.includes("\0"), "H2_LOGICAL_ROUTE_CANONICAL", "logical route forbids platform, percent-encoding, and NUL aliases");
  const segments = route.slice(1).split("/");
  codedAssert(route.startsWith("/") && segments.every((segment) => segment.length > 0 && segment !== "." && segment !== ".." && /^[A-Za-z0-9._-]+$/.test(segment)), "H2_LOGICAL_ROUTE_CANONICAL", "logical route must contain only canonical nonempty POSIX segments");
  const canonical = `/${segments.join("/")}`;
  codedAssert(path.posix.normalize(route) === canonical && route === canonical, "H2_LOGICAL_ROUTE_CANONICAL", "submitted logical route must equal its canonical POSIX form");
  return canonical;
}
function logicalRoutesOverlap(left: string, right: string): boolean {
  const canonicalLeft = canonicalLogicalRoute(left); const canonicalRight = canonicalLogicalRoute(right);
...
function admissionEvidenceTime(inventory: J): number {
  const value = inventory?.physical_host_identity?.measured_at ?? inventory?.physical_host_identity?.pending_time;
  const parsed = Date.parse(value);
  codedAssert(typeof value === "string" && Number.isFinite(parsed) && new Date(parsed).toISOString() === value, "H2_PREACTIVATION_ADMISSION_FRESHNESS", "trusted host inventory time must be canonical UTC");
  return parsed;
}
function assertAdmissionValidityPolicy(value: J, code = "H2_PREACTIVATION_ADMISSION_FRESHNESS"): void {
  codedAssert(canon(value) === canon(PREACTIVATION_ADMISSION_VALIDITY_POLICY), code, "admission validity policy/version differs from the reviewed contract");
}
...
function validateAdmissionReceipt(receipt: J, entry: J, raw: Buffer, coordinatorTrust: J, nowMs: number): void {
  canonicalLogicalRoute(receipt?.subject?.logical_route); canonicalLogicalRoute(receipt?.reviewer?.logical_route);
  schema("pre-activation-attestor-admission.schema.v2.json", receipt);
  rejectPlaceholderEvidence(receipt);
  assertAdmissionValidityPolicy(receipt.validity_policy);
  ...
  const reviewed = Date.parse(receipt.reviewed_at); const subjectMeasured = admissionEvidenceTime(receipt.subject.trusted_surface_inventory); const reviewerMeasured = admissionEvidenceTime(receipt.reviewer.trusted_surface_inventory);
  const { admission_max_age_seconds: admissionAge, host_inventory_max_age_seconds: hostAge, clock_skew_seconds: skew } = PREACTIVATION_ADMISSION_VALIDITY_POLICY;
  codedAssert(Number.isFinite(reviewed) && new Date(reviewed).toISOString() === receipt.reviewed_at && subjectMeasured <= reviewed && reviewerMeasured <= reviewed, "H2_PREACTIVATION_ADMISSION_FRESHNESS", "admission review must strictly follow canonical trusted host inventory observations");
  codedAssert(reviewed <= nowMs + skew * 1000 && nowMs - reviewed <= admissionAge * 1000, "H2_PREACTIVATION_ADMISSION_FRESHNESS", "admission review is stale or future-dated beyond allowed clock skew");
  codedAssert(nowMs - subjectMeasured <= hostAge * 1000 && nowMs - reviewerMeasured <= hostAge * 1000 && subjectMeasured <= nowMs + skew * 1000 && reviewerMeasured <= nowMs + skew * 1000, "H2_PREACTIVATION_ADMISSION_FRESHNESS", "trusted host inventory evidence is stale or future-dated beyond allowed clock skew");
}
function committedFileAt(commit: string, repositoryPath: string, repositoryRoot = ROOT): Buffer {
  const git = trustedExecutable("git");
  try {
    execFileSync(git, ["cat-file", "-e", `${commit}^{commit}`], { cwd: repositoryRoot, env: {}, stdio: "ignore" });
    execFileSync(git, ["merge-base", "--is-ancestor", commit, "HEAD"], { cwd: repositoryRoot, env: {}, stdio: "ignore" });
    codedAssert(commit !== execFileSync(git, ["rev-parse", "HEAD"], { cwd: repositoryRoot, env: {}, encoding: "utf8" }).trim(), "H2_PREACTIVATION_ADMISSION_COMMIT", "admission commit must be a reviewed strict ancestor, not the current commit");
    return execFileSync(git, ["show", `${commit}:${repositoryPath}`], { cwd: repositoryRoot, env: {}, maxBuffer: 16 * 1024 * 1024 });
  } catch (error) {
    if (error instanceof GateH2Error) throw error;
    throw new GateH2Error("H2_PREACTIVATION_ADMISSION_COMMIT", "admission commit is nonexistent, not an ancestor, or does not contain the pinned receipt");
  }
}
function validatePreActivationTrustIdentitySet(trust: J): void {
  codedAssert(trust && Array.isArray(trust.attestors), "H2_PREACTIVATION_TRUST", "pre-activation trust registry structure is missing");
  assertAdmissionValidityPolicy(trust.admission_validity_policy, "H2_PREACTIVATION_TRUST");
  if (trust.status !== "reviewed_real_attestors_admitted") {
  ...
  for (const entry of trust.attestors) canonicalLogicalRoute(entry.logical_route);
  schema("pre-activation-attestor-trust.schema.v2.json", trust);
...
function validatePreActivationTrustRegistry(trust: J, coordinatorTrust?: J, options: { now?: number; repositoryRoot?: string } = {}): Map<string, J> {
  ...
  const nowMs = options.now ?? Date.now(); codedAssert(Number.isFinite(nowMs), "H2_PREACTIVATION_ADMISSION_FRESHNESS", "registry consumption time must be finite");
  for (const entry of trust.attestors) {
    const raw = committedFileAt(entry.admission_commit, entry.admission_receipt.path, options.repositoryRoot ?? ROOT);
    ...
    validateAdmissionReceipt(receipt, entry, raw, coordinatorTrust, nowMs); admissions.set(entry.evidence_kind, receipt);
  }
  return admissions;
}
function validateAdmissionEvidenceUse(admissions: Map<string, J>, kind: string, usedAt: string, nowMs: number): void {
  const receipt = admissions.get(kind); codedAssert(receipt !== undefined, "H2_PREACTIVATION_ADMISSION_FRESHNESS", `${kind} admission receipt is unavailable at evidence use`);
  assertAdmissionValidityPolicy(receipt.validity_policy);
  const reviewed = Date.parse(receipt.reviewed_at); const used = Date.parse(usedAt); const { admission_max_age_seconds: maxAge, clock_skew_seconds: skew } = PREACTIVATION_ADMISSION_VALIDITY_POLICY;
  codedAssert(Number.isFinite(used) && new Date(used).toISOString() === usedAt && reviewed <= used && used <= nowMs + skew * 1000 && used - reviewed <= maxAge * 1000, "H2_PREACTIVATION_ADMISSION_FRESHNESS", `${kind} evidence use must chronologically follow a still-valid admission and not postdate consumption beyond clock skew`);
}
function trustedPreActivationIdentity(trust: J, kind: string, identity: J): boolean {
  ...
  return trust.attestors.some((entry: J) => Object.entries(expected).every(([field, value]) => field === "logical_route" ? canonicalLogicalRoute(entry[field]) === value : entry[field] === value));
}
function assertAdmittedAuthorityIndependence(trust: J, authority: J): void {
  ...
  const actorIdentities = authorityRoles.map((role) => {
    ...
    return { role, principal: actor.principal, session_id: actor.session_id, logical_route: canonicalLogicalRoute(actor.route), surface_id: actor.surface_id, surface_inventory_digest: actor.surface_inventory_digest, canonical_physical_host_id: inventory.canonical_physical_host_id, public_key_sha256: actor.signing_public_key_pem ? hash(actor.signing_public_key_pem) : undefined };
  });
...
function reviewedTrustSnapshot(coordinatorTrust?: J): FileSnapshot {
  ...
  try {
    ...
    if (error instanceof GateH2Error) throw error;
    throw new GateH2Error("H2_PREACTIVATION_TRUST", "reviewed trust registry could not be verified at HEAD");
  }
}
function validateD1LiveAttestation(value: J): void {
  canonicalLogicalRoute(value?.observation?.route); canonicalLogicalRoute(value?.signer?.route);
  validateSignedAttestation(value, "d1-live-attestation.schema.v2.json");
  const canonicalRows = [...value.deployed_schema.canonical_rows].sort((a: J, b: J) => `${a.type}\0${a.name}`.localeCompare(`${b.type}\0${b.name}`));
  const schemaBytes = Buffer.from(`${canon(canonicalRows)}\n`);
...
function validateLinuxSandboxAttestation(value: J, allowSyntheticExecutorSemantics = false, trust?: J): void {
  ...
  unique([...value.mounts.readable_inputs, ...value.mounts.output_mounts].map((mount: J) => mount.guest_path), "sandbox guest mount path");
  same([...value.writable_confinement.writable_paths].sort(), value.mounts.output_mounts.map((mount: J) => mount.host_path).sort(), "sandbox writable paths and output mounts");
  const runtimeMembers = [...value.runtime_tree.members].sort((a: J, b: J) => a.realpath.localeCompare(b.realpath));
  validateResolutionClosure(value.runtime_tree.resolution_contract, value.runtime_tree.physical_roots, runtimeMembers, "H2_PREACTIVATION_RUNTIME_TREE");
...
function sandboxArg(index: number, argument_kind: "derived" | "fixed", semantic_role: string, source_ref: string, value: string, semantic_fields: J): J {
  const entry = { index, argument_kind, value, value_sha256: hash(value), semantic_role, source_ref, semantic_fields, semantic_join_sha256: "" };
  entry.semantic_join_sha256 = wrapperJoin(entry); return entry;
}
function deriveSandboxArgvPrefix(value: J): J[] {
  const result: J[] = []; const add = (kind: "derived" | "fixed", role: string, source: string, argument: string, semantics: J) => result.push(sandboxArg(result.length, kind, role, source, argument, semantics));
  add("derived", "image_identity", "image:complete", `--image-identity=${Buffer.from(canon(value.image)).toString("base64url")}`, value.image);
  for (const mount of [...value.mounts.readable_inputs].sort((a: J, b: J) => a.artifact_role.localeCompare(b.artifact_role))) add("derived", "read_only_mount", `readable_mount:${mount.artifact_role}`, `--mount-ro=\${IMMUTABLE_ROOT}/rootfs${mount.host_path}:${mount.guest_path}:${mount.mode}:${mount.content_sha256}`, mount);
  for (const mount of [...value.mounts.output_mounts].sort((a: J, b: J) => a.artifact_role.localeCompare(b.artifact_role))) add("derived", "output_mount", `output_mount:${mount.artifact_role}`, `--mount-rw=${mount.host_path}:${mount.guest_path}:${mount.mode}:${mount.content_sha256}`, mount);
  const network = { policy: value.network.policy, namespace_identity_sha256: value.network.namespace_identity_sha256, firewall_rules_sha256: value.network.firewall_rules_sha256 };
  add("derived", "network_boundary", "network:complete", `--network-boundary=${Buffer.from(canon(network)).toString("base64url")}`, network);
  add("derived", "runtime_entrypoint", "runtime:complete", `--runtime=\${IMMUTABLE_ROOT}/rootfs${value.runtime_tree.runtime.realpath}`, value.runtime_tree.runtime);
  for (const capability of [...value.network.destination_capability_ids].sort()) add("derived", "destination_capability", `destination_capability:${capability}`, `--destination-capability=${capability}`, { capability_id: capability });
  for (const fixed of value.fixed_argument_allowlist) { validateScalarValue(fixed.value, fixed.scalar_type, fixed.allowed_values); codedAssert(fixed.value_sha256 === hash(fixed.value), "H2_PREACTIVATION_SANDBOX_ARGV", "fixed wrapper scalar hash differs"); add("fixed", "fixed_scalar", `fixed:${fixed.allowlist_id}`, `--fixed-scalar=${fixed.allowlist_id}:${fixed.scalar_type}:${fixed.value}`, fixed); }
  return result;
}
function deriveBoundaryArgvPrefix(boundary: J, runtime: J): J[] {
  const attestationShape = { image: { identity_kind: boundary.image_identity_kind, immutable_identity: boundary.image_immutable_identity, digest: boundary.image_digest }, mounts: { readable_inputs: boundary.readable_mounts, output_mounts: boundary.output_mounts }, network: { policy: boundary.network_policy, namespace_identity_sha256: boundary.network_namespace_sha256, firewall_rules_sha256: boundary.firewall_rules_sha256, destination_capability_ids: boundary.network_capability_ids }, runtime_tree: { runtime }, fixed_argument_allowlist: boundary.fixed_argument_allowlist ?? [] };
  return deriveSandboxArgvPrefix(attestationShape);
}
function verifyAttestationArtifact(pinValue: J, schemaPin: string, expectedSchemaName: string): J {
  let snapshot: OperationSnapshot;
  try { snapshot = snapshotOperationFile(pinValue.artifact, "pre-activation attestation artifact"); }
  catch (error) { if (error instanceof GateH2Error) throw error; throw new GateH2Error("H2_PREACTIVATION_MISSING", "pre-activation attestation artifact is absent or unreadable"); }
  try {
    const value = JSON.parse(snapshot.raw.toString("utf8"));
    codedAssert(snapshot.raw.equals(Buffer.from(pretty(value))), "H2_PREACTIVATION_CANONICAL", "attestation artifact must use canonical JSON bytes");
    codedAssert(schemaPin === schemaFilePin(expectedSchemaName).sha256, "H2_PREACTIVATION_SCHEMA_PIN", "authority attestation schema pin differs");
    return value;
...
function writeOwnedExclusive(file: string, raw: Buffer, code: string): void {
  ...
  try {
    ...
    codedAssert(opened.isFile() && opened.uid === process.getuid!() && opened.nlink === 1, code, "exclusive output is not the newly created owner-controlled file");
    let offset = 0; while (offset < raw.length) offset += fs.writeSync(fd, raw, offset, raw.length - offset, offset);
    fs.fsyncSync(fd); fs.closeSync(fd); fd = undefined;
  } catch (error) {
    if (fd !== undefined) { try { fs.closeSync(fd); } catch { /* cleanup continues by retained inode */ } }
...
function readJsonSnapshot(
  ...
  expectedRaw?: Buffer,
  allowedModes: readonly number[] = [0o400, 0o600],
): FileSnapshot {
  ...
  try {
    ...
    codedAssert(
      before.nlink === 1 && allowedModes.includes(before.mode & 0o777),
      code,
      `retained input must have one link and one of the exact modes ${allowedModes.map((mode) => mode.toString(8)).join(",")}`,
    );
    const retainedFd = fd;
    fd = undefined;
...
function readRawSnapshot(file: string, code: string, allowedModes: readonly number[] = [0o400, 0o600]): RawSnapshot {
  let fd: number | undefined;
  try {
    fd = fs.openSync(file, fs.constants.O_RDONLY | fs.constants.O_NOFOLLOW);
    const before = fs.fstatSync(fd); const raw = fs.readFileSync(fd); const after = fs.fstatSync(fd); const current = fs.lstatSync(file);
    codedAssert(before.isFile() && before.uid === process.getuid!() && before.nlink === 1 && allowedModes.includes(before.mode & 0o777) && (before.mode & 0o022) === 0, code, "retained input must be an owner-controlled one-link regular file with an exact allowed mode");
    codedAssert(before.dev === after.dev && before.ino === after.ino && before.size === after.size && before.mtimeMs === after.mtimeMs && raw.length === before.size && current.dev === before.dev && current.ino === before.ino, code, "retained input changed while safely opening");
    const retainedFd = fd; fd = undefined; return { file, fd: retainedFd, raw, stat: before };
  } catch (error) {
    if (error instanceof GateH2Error) throw error;
    throw new GateH2Error(code, "retained input could not be opened safely");
  } finally { if (fd !== undefined) fs.closeSync(fd); }
}
function closeSnapshot(snapshot: FileSnapshot | RawSnapshot): void {
...
function assertSnapshotPathUnchanged(snapshot: FileSnapshot | RawSnapshot): void {
  ...
  codedAssert(
    current.isFile() &&
      ...
      current.nlink === 1 &&
      current.uid === snapshot.stat.uid &&
      (current.mode & 0o777) === (snapshot.stat.mode & 0o777),
    "H2_FINALIZATION_PATH_SUBSTITUTION",
  ...
  codedAssert(current.mtimeMs === snapshot.stat.mtimeMs, "H2_FINALIZATION_PATH_SUBSTITUTION", "retained evaluator input metadata changed before finalization");
}
...
class InternalAppendOnlyStageLedger implements StageLedgerAdapter {
  ...
  async attestSchema(authorityLedger: Record<string, unknown>): Promise<void> {
    const expected = gateH2LedgerSchemaPin(ROOT);
    codedAssert(authorityLedger.table_schema_sha256 === expected.sha256 && authorityLedger.table_schema_bytes === expected.bytes, "H2_LEDGER_SCHEMA_ATTESTATION", "synthetic ledger schema pin differs");
  }
}
...
function stageLedger(authority: J, capability?: InternalSyntheticCapability): StageLedgerAdapter {
  if (capability?.[INTERNAL_SYNTHETIC_CAPABILITY] === true && process.env.GATE_H2_INTERNAL_D1_FIXTURE) return internalD1FixtureAdapter(authority, path.resolve(process.env.GATE_H2_INTERNAL_D1_FIXTURE), capability);
...
function internalD1FixtureAdapter(authority: J, fixtureFile: string, capability?: InternalSyntheticCapability): StageLedgerAdapter {
  codedAssert(capability?.[INTERNAL_SYNTHETIC_CAPABILITY] === true, "H2_INTERNAL_CAPABILITY", "production-shaped D1 fixture adapter is internal-test-only");
  ...
  const request: typeof fetch = async (_input, init) => {
    ...
    const body = JSON.parse(String(init?.body)); const sql = String(body.sql); const params = body.params as J[];
    let results: J[];
    ...
    else return new Response(JSON.stringify({ success: false, result: [], errors: [{ code: 1000, message: "fixture rejects unexpected SQL" }] }), { status: 400, headers: { "content-type": "application/json" } });
...
function snapshotOperationFile(pinValue: J, label: string): OperationSnapshot {
  ...
  try {
    ...
    const current = fs.lstatSync(pinValue.realpath);
    codedAssert(stat.isFile() && stat.nlink === 1 && stat.dev === after.dev && stat.ino === after.ino && stat.size === raw.length && current.dev === stat.dev && current.ino === stat.ino && hash(raw) === pinValue.sha256 && raw.length === pinValue.bytes && (stat.mode & 0o022) === 0, "H2_STAGE_OPERATION_ARTIFACT", `${label} bytes or identity differ from authority`);
    return { pin: pinValue, fd, raw, stat };
...
function assertOperationSnapshotsUnchanged(snapshots: OperationSnapshot[]): void {
  for (const snapshot of snapshots) {
    ...
    try { codedAssert(current.stat.dev === snapshot.stat.dev && current.stat.ino === snapshot.stat.ino && current.raw.equals(snapshot.raw), "H2_STAGE_OPERATION_CHANGED", "operation artifact changed during stage execution"); }
    finally { fs.closeSync(current.fd); }
  }
}
...
function validateResolutionClosure(contract: J, roots: J[], members: J[], code = "H2_STAGE_DEPENDENCY_TREE"): void {
  ...
  const synthetic = contract.root_derivation === "synthetic_host_node_fixture_nonproduction_v1";
  codedAssert((synthetic ? contract.package_manager?.kind === "synthetic_npm_lockfile_v3_fixture_nonproduction_v1" && contract.module_resolution?.kind === "synthetic_host_node_resolution_nonproduction_v1" : contract.root_derivation === "static_stage_program_zero_dependency_v1" && contract.package_manager?.kind === "npm_lockfile_v3_zero_dependency_v1" && contract.module_resolution?.kind === "gate_h2_stage_program_no_module_loader_v1") && contract.native_resolution?.kind === "linux_elf_ld_so_closure_v1", code, "runtime roots do not use one exact production or explicitly synthetic non-production derivation contract");
  codedAssert(contract.module_resolution.dynamic_specifiers === "forbidden" && contract.module_resolution.unresolved_specifiers.length === 0 && contract.native_resolution.unresolved_libraries.length === 0, code, "runtime import or native-library closure contains unresolved/dynamic edges");
  ...
  if (!synthetic) codedAssert(contract.package_manager.resolved_package_roots.length === 0 && contract.module_resolution.physical_roots.length === 0 && contract.module_resolution.edges.length === 0, code, "production stage programs categorically forbid npm packages and JavaScript module-loader edges");
  ...
  const contains = (rootValue: J, file: string) => file === rootValue.realpath || file.startsWith(`${rootValue.realpath}${path.sep}`);
  codedAssert(contract.module_resolution.physical_roots.every((rootValue: J) => contract.module_resolution.edges.some((edge: J) => contains(rootValue, edge.resolved_realpath))) && contract.native_resolution.physical_roots.every((rootValue: J) => contract.native_resolution.edges.some((edge: J) => contains(rootValue, edge.resolved_realpath))), code, "every package/native discovery root must be justified by a resolved import or loader edge");
...
function deriveNpmPackageProof(packageManager: J, members: J[], rawFor: (realpath: string) => Buffer, production: boolean): J {
  const parse = (pinValue: J, label: string): J => { const raw = rawFor(pinValue.realpath); let value: J; try { value = JSON.parse(raw.toString("utf8")); } catch { throw new GateH2Error("H2_STAGE_PACKAGE_LOCK", `${label} is not retained JSON`); } codedAssert(raw.equals(Buffer.from(pretty(value))), "H2_STAGE_PACKAGE_LOCK", `${label} is not canonical JSON`); return value; };
  const manifest = parse(packageManager.package_manifest, "root package manifest"); const lock = parse(packageManager.lockfile, "npm lockfile");
  const exactKeys = (value: J, allowed: string[], label: string): void => codedAssert(canon(Object.keys(value).sort()) === canon(Object.keys(value).filter((key) => allowed.includes(key)).sort()), "H2_STAGE_PACKAGE_LOCK", `${label} has a field outside the strict supported subset`);
  exactKeys(manifest, ["name", "version", "private", "dependencies"], "root package manifest"); exactKeys(lock, ["name", "version", "lockfileVersion", "requires", "packages"], "npm lockfile");
  ...
  for (const field of dependencyFields.slice(1)) codedAssert(manifest[field] === undefined, "H2_STAGE_PACKAGE_LOCK", `unsupported root dependency field: ${field}`);
  const rootDependencies = manifest.dependencies ?? {}; codedAssert(rootDependencies && typeof rootDependencies === "object" && !Array.isArray(rootDependencies), "H2_STAGE_PACKAGE_LOCK", "root dependencies must be an object");
  codedAssert(lock.lockfileVersion === 3 && lock.requires === true && lock.packages && typeof lock.packages === "object" && !Array.isArray(lock.packages), "H2_STAGE_PACKAGE_LOCK", "retained lock must be a complete npm lockfile v3 packages map");
  const root = lock.packages[""]; codedAssert(root, "H2_STAGE_PACKAGE_LOCK", "lock root package is missing"); exactKeys(root, ["name", "version", "dependencies"], "lock root package"); codedAssert(typeof lock.name === "string" && typeof lock.version === "string" && lock.name === manifest.name && lock.version === manifest.version && root.name === manifest.name && root.version === manifest.version && lock.name === root.name && lock.version === root.version && canon(root.dependencies ?? {}) === canon(rootDependencies), "H2_STAGE_PACKAGE_LOCK", "lock top-level/root identities do not exactly equal retained package.json name/version/dependencies");
  const packageName = /^[a-z0-9][a-z0-9-]{0,63}$/; const exactVersion = /^[0-9]+\.[0-9]+\.[0-9]+$/;
  codedAssert(packageName.test(manifest.name) && exactVersion.test(manifest.version) && manifest.private === true, "H2_STAGE_PACKAGE_LOCK", "root package identity must be private with an exact supported name/version");
  const expected = new Map<string, string>(Object.entries(rootDependencies)); const packages: J[] = [];
  for (const [name, version] of expected) codedAssert(packageName.test(name) && typeof version === "string" && exactVersion.test(version), "H2_STAGE_PACKAGE_LOCK", "supported dependency names and versions must be unscoped exact semver");
  const queue = [...expected.keys()].sort(); const visited = new Set<string>();
  while (queue.length > 0) {
    const name = queue.shift()!; if (visited.has(name)) continue; visited.add(name); const lockPath = `node_modules/${name}`; const entry = lock.packages[lockPath]; const version = expected.get(name);
    codedAssert(entry, "H2_STAGE_PACKAGE_LOCK", `lock entry is missing: ${lockPath}`); exactKeys(entry, ["name", "version", "resolved", "integrity", "dependencies"], `lock entry ${lockPath}`); codedAssert(entry.name === name && entry.version === version, "H2_STAGE_PACKAGE_LOCK", `lock entry is version-mismatched: ${lockPath}`);
    const dependencies = entry.dependencies ?? {}; codedAssert(dependencies && typeof dependencies === "object" && !Array.isArray(dependencies), "H2_STAGE_PACKAGE_LOCK", `lock dependency map is invalid: ${lockPath}`);
    for (const [child, childVersion] of Object.entries(dependencies)) { codedAssert(packageName.test(child) && typeof childVersion === "string" && exactVersion.test(childVersion), "H2_STAGE_PACKAGE_LOCK", "transitive dependencies must use the strict flat exact-semver subset"); const prior = expected.get(child); codedAssert(prior === undefined || prior === childVersion, "H2_STAGE_PACKAGE_LOCK", "dependency graph requires unsupported version nesting"); expected.set(child, childVersion); queue.push(child); }
    const rootValue = packageManager.resolved_package_roots.find((candidate: J) => candidate.realpath.endsWith(`${path.sep}node_modules${path.sep}${name}`)); codedAssert(rootValue, "H2_STAGE_PACKAGE_LOCK", `installed root is missing for ${name}`);
    const installedManifestPath = path.join(rootValue.realpath, "package.json"); const installedPin = members.find((member: J) => member.realpath === installedManifestPath); codedAssert(installedPin, "H2_STAGE_PACKAGE_LOCK", `installed package manifest is missing for ${name}`); const installed = parse(installedPin, `installed package ${name}`);
    exactKeys(installed, ["name", "version", "main", "dependencies"], `installed package manifest ${name}`); codedAssert(installed.name === name && installed.version === version && canon(installed.dependencies ?? {}) === canon(dependencies), "H2_STAGE_PACKAGE_LOCK", `installed package identity/dependencies differ from lock: ${name}`);
    const main = installed.main ?? "index.js"; codedAssert(typeof main === "string" && /^(?:[a-zA-Z0-9_-]+\/)*[a-zA-Z0-9_.-]+$/.test(main) && !main.includes(".."), "H2_STAGE_PACKAGE_LOCK", `installed package main is outside the strict relative subset: ${name}`);
    const entrypoint = fs.realpathSync(path.join(rootValue.realpath, main)); codedAssert(members.some((member: J) => member.realpath === entrypoint), "H2_STAGE_PACKAGE_LOCK", `installed package entrypoint is outside retained members: ${name}`);
    if (production) throw new GateH2Error("H2_STAGE_PACKAGE_LOCK", "production subset forbids every installed npm package");
    codedAssert(entry.resolved === "file:synthetic-fixture-only" && entry.integrity === undefined, "H2_STAGE_PACKAGE_LOCK", "synthetic fixture package must use the explicit non-production local resolution policy");
    const packageMembers = members.filter((member: J) => member.realpath === rootValue.realpath || member.realpath.startsWith(`${rootValue.realpath}${path.sep}`)).map(({ realpath, sha256, bytes }: J) => ({ relative_path: path.relative(rootValue.realpath, realpath), sha256, bytes })).sort((a: J, b: J) => a.relative_path.localeCompare(b.relative_path));
    packages.push({ name, version, lock_path: lockPath, resolved: entry.resolved, integrity: null, root_realpath: rootValue.realpath, package_manifest_sha256: installedPin.sha256, entrypoint_realpath: entrypoint, dependencies, members: packageMembers, members_sha256: hash(canon(packageMembers)) });
  }
  codedAssert(canon(Object.keys(lock.packages).sort()) === canon(["", ...[...expected.keys()].sort().map((name) => `node_modules/${name}`)].sort()), "H2_STAGE_PACKAGE_LOCK", "lock packages map has an omitted or extra package");
  codedAssert(canon(packageManager.resolved_package_roots.map((rootValue: J) => rootValue.realpath).sort()) === canon(packages.map((entry: J) => entry.root_realpath).sort()), "H2_STAGE_PACKAGE_LOCK", "installed package roots differ from the recursively derived lock graph");
  if (production) codedAssert(Object.keys(rootDependencies).length === 0 && packages.length === 0, "H2_STAGE_PACKAGE_LOCK", "production npm subset is exactly zero dependencies");
  return { schema_version: "gate_h2_package_resolution_proof_v2", proof_mode: production ? "production_zero_dependency" : "synthetic_local_fixture_nonproduction", runtime_sha256: packageManager.runtime_sha256, entrypoint_sha256: packageManager.entrypoint_sha256, lockfile_sha256: packageManager.lockfile.sha256, package_manifest_sha256: packageManager.package_manifest.sha256, lock_identity: { name: lock.name, version: lock.version }, root_package: { name: manifest.name, version: manifest.version, dependencies: rootDependencies }, packages };
}
function makeSyntheticResolutionClosure(wrapper: J, runtime: J, entrypoint: J, lockfile: J, packageManifest: J, roots: J[], members: J[], proofRoot: string): J {
  ...
  const packageManager: J = { kind: "synthetic_npm_lockfile_v3_fixture_nonproduction_v1", installed_layout: "node_modules_v1", lockfile, package_manifest: packageManifest, resolved_package_roots: moduleRoots, runtime_sha256: runtime.sha256, entrypoint_sha256: entrypoint.sha256 };
  const packageProof = writeProof("package-resolution-proof.json", deriveNpmPackageProof(packageManager, members, (realpath) => fs.readFileSync(realpath), false)); delete packageManager.runtime_sha256; delete packageManager.entrypoint_sha256;
  const moduleProof = writeProof("module-resolution-proof.json", { schema_version: "gate_h2_synthetic_host_module_resolution_proof_v1", production_eligible: false, host_runtime_sha256: hash(fs.readFileSync(process.execPath)), entrypoint_sha256: entrypoint.sha256, edges: moduleEdges, dynamic_imports: [], unresolved_specifiers: [] });
  ...
  const value: J = { version: "gate_h2_runtime_resolution_closure_v1", root_derivation: "synthetic_host_node_fixture_nonproduction_v1", wrapper, runtime, entrypoints: [entrypoint], package_manager: { ...packageManager, proof: packageProof, proof_sha256: packageProof.sha256 }, module_resolution: { kind: "synthetic_host_node_resolution_nonproduction_v1", dynamic_specifiers: "forbidden", unresolved_specifiers: [], physical_roots: moduleRoots, edges: moduleEdges, proof: moduleProof, proof_sha256: moduleProof.sha256 }, native_resolution: { kind: "linux_elf_ld_so_closure_v1", runtime_linkage: "static", unresolved_libraries: [], physical_roots: [], edges: [], proof: nativeProof, proof_sha256: nativeProof.sha256 }, physical_roots: [...roots].sort((a: J, b: J) => a.realpath.localeCompare(b.realpath)), resolved_member_realpaths: members.map((member: J) => member.realpath), roots_sha256: hash(canon([...roots].sort((a: J, b: J) => a.realpath.localeCompare(b.realpath)))), closure_complete: true, closure_sha256: "" };
  const payload = structuredClone(value); delete payload.closure_sha256; value.closure_sha256 = hash(canon(payload)); return value;
}
function makeZeroDependencyStageProgramResolutionClosure(wrapper: J, runtime: J, entrypoint: J, lockfile: J, packageManifest: J, roots: J[], members: J[], proofRoot: string): J {
  const writeProof = (name: string, payload: J): J => { const file = path.join(proofRoot, name); fs.writeFileSync(file, pretty(payload), { mode: 0o600 }); const raw = fs.readFileSync(file); return { path: file, realpath: fs.realpathSync(file), sha256: hash(raw), bytes: raw.length, version: "canonical-proof-v1" }; };
  const packageManager: J = { kind: "npm_lockfile_v3_zero_dependency_v1", installed_layout: "node_modules_v1", lockfile, package_manifest: packageManifest, resolved_package_roots: [], runtime_sha256: runtime.sha256, entrypoint_sha256: entrypoint.sha256 };
  const packageProof = writeProof("package-resolution-proof.json", deriveNpmPackageProof(packageManager, members, (realpath) => fs.readFileSync(realpath), true)); delete packageManager.runtime_sha256; delete packageManager.entrypoint_sha256;
  const moduleProof = writeProof("module-resolution-proof.json", { schema_version: "gate_h2_stage_program_module_proof_v1", entrypoint_sha256: entrypoint.sha256, loader_surface: "none", edges: [] });
  const nativeProof = writeProof("native-resolution-proof.json", { schema_version: "gate_h2_native_resolution_proof_v1", wrapper_sha256: wrapper.sha256, runtime_sha256: runtime.sha256, runtime_linkage: "static", unresolved_libraries: [] });
  const value: J = { version: "gate_h2_runtime_resolution_closure_v1", root_derivation: "static_stage_program_zero_dependency_v1", wrapper, runtime, entrypoints: [entrypoint], package_manager: { ...packageManager, proof: packageProof, proof_sha256: packageProof.sha256 }, module_resolution: { kind: "gate_h2_stage_program_no_module_loader_v1", dynamic_specifiers: "forbidden", unresolved_specifiers: [], physical_roots: [], edges: [], proof: moduleProof, proof_sha256: moduleProof.sha256 }, native_resolution: { kind: "linux_elf_ld_so_closure_v1", runtime_linkage: "static", unresolved_libraries: [], physical_roots: [], edges: [], proof: nativeProof, proof_sha256: nativeProof.sha256 }, physical_roots: [...roots].sort((a: J, b: J) => a.realpath.localeCompare(b.realpath)), resolved_member_realpaths: members.map((member: J) => member.realpath), roots_sha256: hash(canon([...roots].sort((a: J, b: J) => a.realpath.localeCompare(b.realpath)))), closure_complete: true, closure_sha256: "" };
  const payload = structuredClone(value); delete payload.closure_sha256; value.closure_sha256 = hash(canon(payload)); return value;
}
function enumerateDependencyRoot(rootValue: J): J[] {
  codedAssert(path.isAbsolute(rootValue.path) && path.normalize(rootValue.path) === rootValue.path && fs.realpathSync(rootValue.path) === rootValue.realpath && rootValue.discovery_rule === "recursive_regular_files_no_symlinks_v1", "H2_STAGE_DEPENDENCY_TREE", "dependency root path, realpath, or discovery rule differs");
  const members: J[] = [];
  const visit = (current: string): void => {
    const stat = fs.lstatSync(current); codedAssert(!stat.isSymbolicLink(), "H2_STAGE_DEPENDENCY_TREE", "dependency discovery refuses symlinks");
    if (stat.isDirectory()) { for (const name of fs.readdirSync(current).sort()) visit(path.join(current, name)); return; }
    codedAssert(stat.isFile() && stat.nlink === 1 && (stat.mode & 0o022) === 0, "H2_STAGE_DEPENDENCY_TREE", "dependency member must be a one-link regular file without group/world write permission");
    const raw = fs.readFileSync(current); const realpath = fs.realpathSync(current); members.push({ path: current, realpath, sha256: hash(raw), bytes: raw.length, version: "content-sha256-v1" });
  };
  visit(rootValue.realpath); return members;
}
function installedDependencyTreeSnapshots(manifestPin: J): { snapshots: OperationSnapshot[]; manifest: J } {
  const snapshots: OperationSnapshot[] = [];
  try {
    const manifestSnapshot = snapshotOperationFile(manifestPin, "installed dependency tree manifest"); snapshots.push(manifestSnapshot); const value = JSON.parse(manifestSnapshot.raw.toString("utf8"));
    schema("installed-dependency-tree.schema.v2.json", value);
    ...
    codedAssert(value.tree_sha256 === hash(bytes) && value.tree_bytes === bytes.length, "H2_STAGE_DEPENDENCY_TREE", "installed dependency tree arithmetic differs");
...
function validateDerivedResolutionClosure(operation: J, dependencyTree: { snapshots: OperationSnapshot[]; manifest: J }, syntheticBoundary: boolean, authorityContext?: ExternalOperationAuthorityContext): void {
  ...
  if (!syntheticBoundary) validateExternalOperationAuthorityPreflight(operation, entrypoint.raw, authorityContext, false);
  ...
  const derivedPackageProof = deriveNpmPackageProof({ ...contract.package_manager, runtime_sha256: operation.runtime.sha256, entrypoint_sha256: operation.script.sha256 }, dependencyTree.manifest.members, (realpath) => { const snapshot = byPath.get(realpath); codedAssert(snapshot, "H2_STAGE_PACKAGE_LOCK", `package proof input is not retained: ${realpath}`); return snapshot.raw; }, !syntheticBoundary);
  codedAssert(canon(packageProof) === canon(derivedPackageProof), "H2_STAGE_DEPENDENCY_PROOF", "package proof does not independently derive from retained package.json, npm lock v3, installed package manifests, and exact members");
...
function immutableCopyPath(root: string, realpath: string): string {
  ...
  return path.join(root, "rootfs", realpath.slice(1));
}
function materializeImmutableSnapshot(root: string, snapshot: OperationSnapshot, executable: boolean): string {
  const target = immutableCopyPath(root, snapshot.pin.realpath); fs.mkdirSync(path.dirname(target), { recursive: true, mode: 0o700 });
  fs.writeFileSync(target, snapshot.raw, { flag: "wx", mode: executable ? 0o500 : 0o400 });
  const stat = fs.lstatSync(target); codedAssert(stat.isFile() && stat.uid === process.getuid!() && stat.nlink === 1 && (stat.mode & 0o077) === 0 && hash(fs.readFileSync(target)) === snapshot.pin.sha256, "H2_STAGE_IMMUTABLE_TREE", "private immutable copy differs from retained authority bytes");
  return target;
}
...
function externalOperationPreflight(operation: J, outputs: J[], syntheticBoundary = false, authorityContext?: ExternalOperationAuthorityContext): ExternalPreflight {
  ...
  const artifactPins = operation.artifacts.map((artifact: J) => artifact.pin);
  ...
  const dirty = execFileSync(git, ["-C", operation.cwd, "status", "--porcelain=v1", "--untracked-files=all"], { encoding: "utf8", env: {} });
  codedAssert(hash(gitTree) === operation.repo_tree_sha256 && dirty === "", "H2_STAGE_OPERATION_REPO_TREE", "physical cwd must be the exact clean committed tree with no modified or untracked content");
  immutableRoot = fs.mkdtempSync(path.join(os.tmpdir(), "rmv2-immutable-operation-")); fs.chmodSync(immutableRoot, 0o700);
  const uniqueSnapshots = new Map<string, OperationSnapshot>(); for (const snapshot of snapshots) uniqueSnapshots.set(snapshot.pin.realpath, snapshot);
  for (const snapshot of uniqueSnapshots.values()) materializeImmutableSnapshot(immutableRoot, snapshot, snapshot.pin.realpath === operation.execution_boundary.wrapper.realpath || snapshot.pin.realpath === operation.runtime.realpath);
  const immutableWrapper = immutableCopyPath(immutableRoot, operation.execution_boundary.wrapper.realpath); const immutableRuntime = immutableCopyPath(immutableRoot, operation.runtime.realpath); const immutableScript = immutableCopyPath(immutableRoot, operation.script.realpath);
  const immutableCwd = immutableCopyPath(immutableRoot, operation.cwd); fs.mkdirSync(immutableCwd, { recursive: true, mode: 0o700 });
  const version = execFileSync(immutableRuntime, ["--version"], { encoding: "utf8", env: {}, timeout: 10_000 }).trim();
  ...
  codedAssert(operation.argv_bindings.length === operation.argv.length, "H2_STAGE_OPERATION_ARGV", "argv bindings are incomplete");
  ...
  codedAssert(canon([...operation.execution_boundary.writable_paths].sort()) === canon([...operation.declared_output_paths].sort()) && operation.execution_boundary.network_policy === operation.network_policy && canon(operation.execution_boundary.network_capability_ids) === canon(operation.network_capability_ids), "H2_STAGE_SANDBOX_ATTESTATION", "sandbox writable/network boundary differs from authority operation");
  const attestation = operation.execution_boundary;
  const attestedPayload = structuredClone(attestation); delete attestedPayload.attestation_sha256;
  codedAssert(attestation.kind === "linux_sandbox_attestation_v1" && attestation.platform === "linux" && attestation.repo_tree_sha256 === operation.repo_tree_sha256 && attestation.dependency_tree_sha256 === dependencyTree.manifest.tree_sha256 && attestation.attestation_sha256 === hash(canon(attestedPayload)) && (syntheticBoundary || process.platform === "linux"), "H2_STAGE_SANDBOX_ATTESTATION", "production execution requires the exact pinned Linux sandbox/container/VM attestation");
  if (!syntheticBoundary) {
    const joinedPrefix = attestation.argv_prefix.map((value: string, index: number) => ({ ...attestation.argv_prefix_bindings[index], value }));
    codedAssert(attestation.wrapper_grammar_version === "gate_h2_linux_wrapper_argv_v3" && attestation.argv_prefix_bindings.length === attestation.argv_prefix.length && canon(joinedPrefix) === canon(deriveBoundaryArgvPrefix(attestation, operation.runtime)), "H2_STAGE_SANDBOX_ARGV", "wrapper argv prefix must exactly derive from the typed signed boundary grammar");
    codedAssert(dependencyTree.manifest.tree_sha256 === attestation.dependency_tree_sha256 && dependencyTree.manifest.tree_bytes === attestation.dependency_tree_bytes && dependencyTree.manifest.member_count === attestation.dependency_tree_members && canon(dependencyTree.manifest.resolution_contract) === canon(attestation.resolution_contract), "H2_STAGE_DEPENDENCY_TREE", "installed runtime/module/native closure differs from sandbox attestation");
    const readableByRole = new Map<string, J>(attestation.readable_mounts.map((mount: J): [string, J] => [mount.artifact_role, mount]));
    const requiredReadable = [["runtime", operation.runtime], ["script", operation.script], ["dependency_lock", operation.dependency_lock], ["installed_dependency_tree", operation.installed_dependency_tree], ...operation.artifacts.map((artifact: J) => [artifact.artifact_role, artifact.pin])] as [string, J][];
    codedAssert(readableByRole.size === attestation.readable_mounts.length && canon([...readableByRole.keys()].sort()) === canon(requiredReadable.map(([role]) => role).sort()), "H2_STAGE_SANDBOX_MOUNTS", "signed readable mounts are not the exact role set materialized into the immutable tree");
    for (const [role, pinValue] of requiredReadable) {
      const mount = readableByRole.get(role); codedAssert(mount?.host_path === pinValue.realpath && mount.mode === "ro" && mount.content_sha256 === pinValue.sha256, "H2_STAGE_SANDBOX_MOUNTS", `readable input ${role} is not joined to the signed read-only mount`);
    }
  ...
  codedAssert(/^[a-f0-9]{64}$/.test(operation.executor_capability_id), "H2_STAGE_PROGRAM_GRAMMAR", "external operation must pin one derived stage executor capability");
...
function executeExternalStageOperationInternal(operation: J, outputs: J[], syntheticBoundary: boolean, beforeChild?: () => void | (() => void), authorityContext?: ExternalOperationAuthorityContext): ReturnType<typeof externalOperationPreflight> {
...
function executeSyntheticExternalStageOperation(operation: J, outputs: J[], beforeChild?: () => void | (() => void)): ReturnType<typeof externalOperationPreflight> {
  return executeExternalStageOperationInternal(operation, outputs, true, beforeChild);
}
function executeProductionExternalStageOperation(authority: J, stageId: StageId, reviewedTrust: J, authorityContext: ExternalOperationAuthorityContext, beforeChild?: () => void | (() => void)): ReturnType<typeof externalOperationPreflight> {
  codedAssert(authorityContext !== undefined && reviewedTrust !== undefined, "H2_STAGE_AUTHORITY_CONTEXT", "production external operation requires the exact validated authority context and reviewed trust snapshot");
  const expectedContext = externalOperationAuthorityContext(authority, stageId, reviewedTrust);
  codedAssert(canon(authorityContext) === canon(expectedContext), "H2_STAGE_AUTHORITY_CONTEXT", "production external operation authority version, stage binding, operation, or reviewed trust snapshot was omitted or substituted");
  const entry = stageManifestEntry(authority, stageId);
...
function executeReviewedProductionStageOperation(authority: J, stageId: StageId, beforeChild?: () => void | (() => void)): ReturnType<typeof externalOperationPreflight> {
  ...
  try {
    ...
    const result = executeProductionExternalStageOperation(authority, stageId, trustSnapshot.value, authorityContext, beforeChild);
...
function executePublicationAssemblyPlanExternalOperation(authority: J, reviewedTrust: J, authorityContext: ExternalOperationAuthorityContext): ReturnType<typeof externalOperationPreflight> {
  return executeProductionExternalStageOperation(authority, "publication_assembly_plan", reviewedTrust, authorityContext);
}
function executeReviewedPublicationAssemblyPlanExternalOperation(authority: J): ReturnType<typeof externalOperationPreflight> {
  const trustSnapshot = reviewedTrustSnapshot(authority.coordinator_trust);
  try {
    const authorityContext = externalOperationAuthorityContext(authority, "publication_assembly_plan", trustSnapshot.value);
    const result = executePublicationAssemblyPlanExternalOperation(authority, trustSnapshot.value, authorityContext);
    assertSnapshotPathUnchanged(trustSnapshot);
    return result;
  } finally { closeSnapshot(trustSnapshot); }
}
function internalStageRaceHook(operation: J, capability?: InternalSyntheticCapability): (() => void | (() => void)) | undefined {
  ...
  codedAssert(capability?.[INTERNAL_SYNTHETIC_CAPABILITY] === true, "H2_STAGE_INTERNAL_RACE", "stage race injection is internal-test-only");
  if (race === "replace_artifact") return () => { const file = operation.artifacts[0].pin.realpath; const raw = fs.readFileSync(file); fs.writeFileSync(file, Buffer.alloc(raw.length, 0x72)); return () => fs.writeFileSync(file, raw); };
  const manifest = load(operation.installed_dependency_tree.realpath); const member = manifest.members.find((item: J) => item.realpath.includes(`${path.sep}node_modules${path.sep}`)); codedAssert(member, "H2_STAGE_INTERNAL_RACE", "internal dependency race fixture lacks a module member");
  if (race === "replace_dependency") return () => { const raw = fs.readFileSync(member.realpath); fs.chmodSync(member.realpath, 0o600); fs.writeFileSync(member.realpath, Buffer.alloc(raw.length, 0x73)); return () => { fs.writeFileSync(member.realpath, raw); fs.chmodSync(member.realpath, 0o400); }; };
...
export function externalOperationContractSelfTest(): J {
  ...
  try {
    ...
    fs.mkdirSync(dependencyRoot, { recursive: true }); fs.writeFileSync(dependencyMember, "module.exports='exact';\n", { mode: 0o400 }); fs.writeFileSync(path.join(dependencyRoot, "package.json"), pretty({ name: "exact-test-runtime", version: "1.0.0", main: "index.js" }), { mode: 0o400 });
    fs.writeFileSync(prompt, "sanitized prompt\n", { mode: 0o600 }); fs.writeFileSync(packageManifest, pretty({ name: "gate-h2-test", version: "1.0.0", private: true, dependencies: { "exact-test-runtime": "1.0.0" } }), { mode: 0o600 }); fs.writeFileSync(lock, pretty({ name: "gate-h2-test", version: "1.0.0", lockfileVersion: 3, requires: true, packages: { "": { name: "gate-h2-test", version: "1.0.0", dependencies: { "exact-test-runtime": "1.0.0" } }, "node_modules/exact-test-runtime": { name: "exact-test-runtime", version: "1.0.0", resolved: "file:synthetic-fixture-only" } } }), { mode: 0o600 }); fs.writeFileSync(wrapper, syntheticInProcessWrapperSource(), { mode: 0o500 });
    ...
    const dependencyValue = load(dependencies); const runtimePin = filePin(process.execPath, process.version); const scriptPin = filePin(script); const wrapperPin = filePin(wrapper); const imageDigest = `sha256:${hash("synthetic-linux-image")}`; const outputMounts = [{ artifact_role: "stage_output", host_path: output, guest_path: "/output/result", mode: "rw", content_sha256: hash("empty-synthetic-output") }]; const semantics = syntheticExecutorSemanticsBundle({ wrapper: wrapperPin, runtime: runtimePin, stageProgram: scriptPin, imageDigest, readableMounts: [], outputMounts, writablePaths: [output] }); const boundaryPayload: J = { kind: "linux_sandbox_attestation_v1", platform: "linux", wrapper: wrapperPin, argv_prefix: [], image_identity_kind: "oci_manifest", image_immutable_identity: "synthetic.invalid/gate-h2@sha256", image_digest: imageDigest, resolution_contract: dependencyValue.resolution_contract, dependency_tree_sha256: dependencyValue.tree_sha256, executor_semantics: semantics, network_policy: "deny_all", network_capability_ids: [], writable_paths: [output], repo_tree_sha256: repoTree };
    const operation: J = { kind: "external_command", execution_boundary: { ...boundaryPayload, attestation_sha256: hash(canon(boundaryPayload)) }, runtime: runtimePin, script: scriptPin, artifacts: [{ artifact_role: "sanitized_prompt", pin: filePin(prompt) }], dependency_lock: filePin(lock), installed_dependency_tree: filePin(dependencies), repo_tree_sha256: repoTree, cwd: fs.realpathSync(root), argv: [fs.realpathSync(script), fs.realpathSync(prompt)], argv_bindings: [{ index: 0, artifact_role: "script", value_sha256: hash(fs.realpathSync(script)) }, { index: 1, artifact_role: "sanitized_prompt", value_sha256: hash(fs.realpathSync(prompt)) }], declared_output_paths: [output], executor_capability_id: semantics.capability_id, network_policy: "deny_all", network_capability_ids: [], environment: { public: publicEnvironment, secret_capability_ids: [] } };
    const packageManagerForTest = { ...resolutionContract.package_manager, runtime_sha256: operation.runtime.sha256, entrypoint_sha256: operation.script.sha256 }; const retainedPackageBytes = new Map(members.map((member: J): [string, Buffer] => [member.realpath, fs.readFileSync(member.realpath)])); const lockRealpath = operation.dependency_lock.realpath; const packageManifestRealpath = resolutionContract.package_manager.package_manifest.realpath; const installedManifestRealpath = fs.realpathSync(path.join(dependencyRoot, "package.json"));
    const expectPackageRejection = (mutate: (raw: Map<string, Buffer>, manager: J) => void): void => { const raw = new Map(retainedPackageBytes); const manager = structuredClone(packageManagerForTest); mutate(raw, manager); expect("H2_STAGE_PACKAGE_LOCK", () => deriveNpmPackageProof(manager, members, (realpath) => raw.get(realpath)!, false)); };
    expectPackageRejection((raw) => raw.set(lockRealpath, Buffer.from(pretty({ lockfileVersion: 3 }))));
    expectPackageRejection((raw) => { const value = JSON.parse(raw.get(lockRealpath)!.toString()); delete value.name; raw.set(lockRealpath, Buffer.from(pretty(value))); });
    expectPackageRejection((raw) => { const value = JSON.parse(raw.get(lockRealpath)!.toString()); delete value.version; raw.set(lockRealpath, Buffer.from(pretty(value))); });
    expectPackageRejection((raw) => { const value = JSON.parse(raw.get(lockRealpath)!.toString()); value.name = "other-root"; raw.set(lockRealpath, Buffer.from(pretty(value))); });
    expectPackageRejection((raw) => { const value = JSON.parse(raw.get(lockRealpath)!.toString()); value.version = "9.9.9"; raw.set(lockRealpath, Buffer.from(pretty(value))); });
    expectPackageRejection((raw) => raw.set(packageManifestRealpath, Buffer.from(pretty({ name: "gate-h2-test", version: "1.0.0", private: true }))));
    expectPackageRejection((raw) => { const value = JSON.parse(raw.get(lockRealpath)!.toString()); delete value.packages[""]; raw.set(lockRealpath, Buffer.from(pretty(value))); });
    expectPackageRejection((raw) => { const value = JSON.parse(raw.get(lockRealpath)!.toString()); delete value.packages["node_modules/exact-test-runtime"]; raw.set(lockRealpath, Buffer.from(pretty(value))); });
    expectPackageRejection((raw) => { const value = JSON.parse(raw.get(lockRealpath)!.toString()); value.packages["node_modules/exact-test-runtime"].version = "2.0.0"; raw.set(lockRealpath, Buffer.from(pretty(value))); });
    expectPackageRejection((raw) => raw.set(installedManifestRealpath, Buffer.from(pretty({ name: "exact-test-runtime", version: "2.0.0", main: "index.js" }))));
    expectPackageRejection((raw) => { const value = JSON.parse(raw.get(lockRealpath)!.toString()); value.packages["node_modules/extra"] = { name: "extra", version: "1.0.0", resolved: "file:synthetic-fixture-only" }; raw.set(lockRealpath, Buffer.from(pretty(value))); });
    ...
    executeSyntheticExternalStageOperation(operation, [{ path: output }]);
    ...
    expect("H2_STAGE_AUTHORITY_CONTEXT", () => executeExternalStageOperationInternal(operation, [{ path: output }], false));
    ...
    fs.writeFileSync(successorPackageManifest, pretty({ name: "gate-h2-successor-test", version: "1.0.0", private: true }), { mode: 0o600 }); fs.writeFileSync(successorLock, pretty({ name: "gate-h2-successor-test", version: "1.0.0", lockfileVersion: 3, requires: true, packages: { "": { name: "gate-h2-successor-test", version: "1.0.0" } } }), { mode: 0o600 });
    const successorRoots = [wrapper, process.execPath, successorProgramFile, successorLock, successorPackageManifest].map((file) => ({ path: file, realpath: fs.realpathSync(file), discovery_rule: "recursive_regular_files_no_symlinks_v1" })).sort((a, b) => a.realpath.localeCompare(b.realpath));
    const successorMembers = successorRoots.flatMap(enumerateDependencyRoot).sort((a: J, b: J) => a.realpath.localeCompare(b.realpath));
    const successorResolutionContract = makeSyntheticResolutionClosure(wrapperPin, runtimePin, successorProgramPin, filePin(successorLock), filePin(successorPackageManifest), successorRoots, successorMembers, successorProofRoot);
    ...
    const publicationSuccessorAuthority = structuredClone(successorAuthority);
    const publicationStage = publicationSuccessorAuthority.stage_execution.stages.find((entry: J) => entry.stage_id === "publication_assembly_plan"); publicationStage.operation = successorOperation; publicationStage.outputs = sourceStage.outputs;
    publicationSuccessorAuthority.https_exchange_authority.stage_bindings = [{ ...publicationSuccessorAuthority.https_exchange_authority.stage_bindings[0], stage_id: "publication_assembly_plan" }];
    const publicationContext = externalOperationAuthorityContext(publicationSuccessorAuthority, "publication_assembly_plan", successorTrust);
    expect("H2_STAGE_AUTHORITY_CONTEXT", () => executePublicationAssemblyPlanExternalOperation(publicationSuccessorAuthority, successorTrust, undefined as unknown as ExternalOperationAuthorityContext));
    const substitutedPublicationContext = structuredClone(publicationContext); substitutedPublicationContext.reviewedTrust.status = "substituted";
    expect("H2_STAGE_AUTHORITY_CONTEXT", () => executePublicationAssemblyPlanExternalOperation(publicationSuccessorAuthority, successorTrust, substitutedPublicationContext));
    expect("H2_EXECUTOR_SEMANTICS_SYNTHETIC", () => executePublicationAssemblyPlanExternalOperation(publicationSuccessorAuthority, successorTrust, publicationContext));
    ...
    fs.rmSync(successorProgramFile); fs.rmSync(successorManifestFile); fs.rmSync(successorProofRoot, { recursive: true });
    ...
    const substitutedCapability = structuredClone(semantics); substitutedCapability.capability_id = hash("caller-selected-capability"); expect("H2_EXECUTOR_CAPABILITY_DERIVATION", () => validateExecutorSemanticsBundle(substitutedCapability, semanticsContext, true));
    const alternateProgram = syntheticExecutorSemanticsBundle({ wrapper: wrapperPin, runtime: runtimePin, stageProgram: filePin(prompt), imageDigest, readableMounts: [], outputMounts, writablePaths: [output] }); expect("H2_EXECUTOR_SEMANTICS_JOIN", () => validateExecutorSemanticsBundle(alternateProgram, semanticsContext, true));
    const alternateExecutor = syntheticExecutorSemanticsBundle({ wrapper: wrapperPin, runtime: filePin(prompt), stageProgram: scriptPin, imageDigest, readableMounts: [], outputMounts, writablePaths: [output] }); expect("H2_EXECUTOR_SEMANTICS_JOIN", () => validateExecutorSemanticsBundle(alternateExecutor, semanticsContext, true));
    const alternateMounts = syntheticExecutorSemanticsBundle({ wrapper: wrapperPin, runtime: runtimePin, stageProgram: scriptPin, imageDigest, readableMounts: [], outputMounts: [{ ...outputMounts[0], guest_path: "/output/substituted" }], writablePaths: [output] }); expect("H2_EXECUTOR_SEMANTICS_JOIN", () => validateExecutorSemanticsBundle(alternateMounts, semanticsContext, true));
    ...
    const noBoundary = structuredClone(operation); noBoundary.execution_boundary.attestation_sha256 = "0".repeat(64); expect("H2_STAGE_SANDBOX_ATTESTATION", () => run(noBoundary));
    const networkEscape = structuredClone(operation); networkEscape.execution_boundary.network_policy = "allow_capabilities_only"; expect("H2_STAGE_SANDBOX_ATTESTATION", () => run(networkEscape));
    ...
    const race = (code: string, mutate: () => void, restore: () => void): void => { expect(code, () => executeSyntheticExternalStageOperation(operation, [{ path: output }], mutate)); codedAssert(!fs.existsSync(output), "H2_STAGE_IMMUTABLE_RACE_TEST", "rejected source race produced child output"); restore(); };
    ...
    const importCandidate = path.join(dependencyRoot, "candidate.cjs"); race("H2_STAGE_DEPENDENCY_TREE", () => fs.writeFileSync(importCandidate, "module.exports='late';\n", { mode: 0o400 }), () => fs.unlinkSync(importCandidate));
...
function stageRouteReceiptSchemaContractSelfTest(): J {
  const exactStages = ["visual_predict", "visual_freeze", "source_predict", "source_freeze", "gold_review", "gold_envelope_authoring", "private_prepare", "r2_retain", "private_finalize", "task_review", "metrics_score", "publication_assembly_plan"] as StageId[];
  codedAssert(canon(exactStages) === canon(STAGE_IDS) && exactStages.length === 12 && new Set(exactStages).size === 12, "H2_ROUTE_STAGE_SCHEMA_TEST", "route receipt test universe differs from the exact production stage route");
  const base: J = {
    schema_version: "reviewed_metrics_coordinator_route_receipt_v2.0.0",
    surface_id: hash("stage-route-schema-surface"), surface_inventory_digest: hash("stage-route-schema-inventory"), canonical_physical_host_id: hash("stage-route-schema-host"),
    candidate_commit: "a".repeat(40), authority_hash: hash("stage-route-schema-authority"), nonce: hash("stage-route-schema-nonce").slice(0, 32), invocation_id: hash("stage-route-schema-invocation"),
    role: "publisher", stage_id: "publication_assembly_plan", requested_root: "/route", canonical_root: "/route", existing_ancestor: "/", ancestor_device: 1, ancestor_inode: 1,
    measured_at: "2026-07-16T00:00:00.000Z", measurement_sha256: hash("stage-route-schema-measurement"), role_event_started_at: "2026-07-16T00:00:01.000Z", role_event_ended_at: "2026-07-16T00:01:00.000Z", issued_at: "2026-07-16T00:00:01.000Z", expires_at: "2026-07-16T00:01:00.000Z", signature_base64: Buffer.alloc(64, 1).toString("base64"),
  };
  for (const stageId of exactStages) schema("stage-route-receipt.schema.v2.json", { ...base, stage_id: stageId, role: STAGE_ROLES[stageId] });
  const rejects = (candidate: J): void => { let rejected = false; try { schema("stage-route-receipt.schema.v2.json", candidate); } catch { rejected = true; } codedAssert(rejected, "H2_ROUTE_STAGE_SCHEMA_TEST", "stage route receipt schema accepted an alias, omission, or extra stage"); };
  rejects({ ...base, stage_id: "publication" });
  rejects({ ...base, stage_id: "publication_plan" });
  const omitted = structuredClone(base); delete omitted.stage_id; rejects(omitted);
  return { status: "exact_stage_route_receipt_schema_contract_passed", stage_count: exactStages.length, final_stage: exactStages.at(-1), rejected: ["publication_alias", "publication_plan_extra", "stage_id_omission"] };
}
...
async function completedStageLedgerEvidence(authority: J, verifyCurrentOutputs = true, capability?: InternalSyntheticCapability, signingKeyFile?: string, injectedLedger?: StageLedgerAdapter): Promise<J> {
    ...
    for (const entry of authority.stage_execution.stages) {
      ...
      const claim = joined.claims.get(stageId)!;
      const completed = joined.completions.get(stageId)!;
      const beginRaw = Buffer.from(String(claim.begin_envelope));
      const completionRaw = Buffer.from(String(completed.completion_envelope));
      ...
      codedAssert(begin.schema_version === "reviewed_metrics_stage_begin_v4.0.0" && completion.schema_version === "reviewed_metrics_stage_completion_v4.0.0" && begin.attempt_id === completion.attempt_id && claim.attempt_id === begin.attempt_id && completed.attempt_id === begin.attempt_id, "H2_STAGE_LEDGER_MARKER", "remote stage row envelope differs");
      ...
      codedAssert(claim.begin_sha256 === hash(beginRaw) && completed.completion_sha256 === hash(completionRaw) && completion.begin_sha256 === hash(beginRaw), "H2_LEDGER_READBACK", "remote row hashes differ from canonical envelopes");
    ...
    const publicAttempts = joined.attempts.map((attempt) => ({ stage_id: attempt.stage_id, attempt_id: attempt.attempt_id, attempted_at: attempt.attempted_at, request_sha256: attempt.request_sha256 }));
    const envelope: J = {
      schema_version: "reviewed_metrics_stage_execution_ledger_evidence_v2.2.0",
      ...
      attempts_digest: hash(canon(publicAttempts)),
...
function validateStageExecutionEvidence(value: J, authority: J, requirePrivateLedger = false): void {
  ...
  same(value.attempts.map((entry: J) => entry.stage_id), STAGE_IDS, "public exact ordered attempt evidence");
  codedAssert(value.attempts_digest === hash(canon(value.attempts)), "H2_LEDGER_READBACK", "public attempt set commitment differs");
  ...
  for (let index = 0; index < STAGE_IDS.length; index++) {
    ...
    codedAssert(attempt.attempt_id === stage.attempt_id && attempt.request_sha256 === stage.begin_sha256 && Date.parse(attempt.attempted_at) <= Date.parse(stage.command_started_at), "H2_LEDGER_READBACK", "public attempt entry does not join its stage begin evidence");
...
async function candidateDocuments(output: string): Promise<Map<string, J>> {
  ...
  const status = {
    ...
    security_controls: {
      ...
      external_operation_linux_sandbox_attested: false,
      deployed_d1_schema_attested: false,
      signed_d1_live_attestation_exists: false,
      signed_linux_sandbox_attestation_exists: false,
      signed_pre_activation_verification_exists: false,
      reviewed_pre_activation_attestors_admitted: false,
      authority_v24_activation_available: false,
...
async function verifyCandidate(root = FIXTURE, registry = true, replayFailure?: () => void): Promise<J> {
...
type AuthorityValidationContext = { [INTERNAL_AUTHORITY_VALIDATION_CONTEXT]: true; repositoryRoot: string; trustRegistryPath: string; now: number; allowSyntheticExecutorSemantics: true };
function internalAuthorityValidationContext(authority: J, repositoryRoot: string, trustRegistryPath: string, now: number): AuthorityValidationContext {
  codedAssert(INTERNAL_SYNTHETIC_AUTHORITIES.has(authority) && authority.candidate_commit === SYNTHETIC_CANDIDATE_COMMIT, "H2_INTERNAL_CAPABILITY", "validation context is restricted to registered synthetic authority");
  codedAssert(path.resolve(repositoryRoot) !== ROOT, "H2_INTERNAL_CAPABILITY", "validation context cannot replace tracked production authority inputs");
  return { [INTERNAL_AUTHORITY_VALIDATION_CONTEXT]: true, repositoryRoot: path.resolve(repositoryRoot), trustRegistryPath, now, allowSyntheticExecutorSemantics: true };
}
function contextualPreActivationTrust(context: AuthorityValidationContext): J {
  codedAssert(context[INTERNAL_AUTHORITY_VALIDATION_CONTEXT] === true && Number.isFinite(context.now), "H2_INTERNAL_CAPABILITY", "invalid internal authority validation context");
  const relative = canonicalLogicalRoute(`/${context.trustRegistryPath}`).slice(1); const absolute = path.resolve(context.repositoryRoot, relative);
  codedAssert(absolute.startsWith(`${context.repositoryRoot}${path.sep}`), "H2_PREACTIVATION_TRUST", "context registry must remain inside its repository");
  const raw = fs.readFileSync(absolute); let committed: Buffer;
  try { committed = execFileSync(trustedExecutable("git"), ["show", `HEAD:${relative}`], { cwd: context.repositoryRoot, env: {} }); }
  catch { throw new GateH2Error("H2_PREACTIVATION_TRUST", "context registry is not committed at repository HEAD"); }
  codedAssert(raw.equals(committed), "H2_PREACTIVATION_TRUST", "context registry must equal the exact committed HEAD descendant");
...
function validatePreActivationAuthority(authority: J, allowSyntheticExecutorSemantics = false, context?: AuthorityValidationContext): void {
  ...
  for (const [name, pinValue, attestation] of [["d1", pins.d1, d1], ["sandbox", pins.sandbox, sandbox]] as const) {
    ...
    const observed = Date.parse(attestation.observation.observed_at); const authorized = Date.parse(authority.authorized_at);
    codedAssert(Number.isFinite(observed) && observed <= authorized && authorized - observed <= pins.max_age_seconds * 1000, "H2_PREACTIVATION_STALE", `${name} attestation is stale or postdates authority activation`);
  }
  canonicalLogicalRoute(verification?.verifier?.route); schema("pre-activation-verification.schema.v2.json", verification); rejectPlaceholderEvidence(verification); assertAdmissionValidityPolicy(verification.admission_validity_policy);
  ...
  for (const use of admittedUses) {
    ...
    validateAdmissionEvidenceUse(admissions, use.kind, use.used_at, authorized);
  }
  ...
  for (const entry of authority.stage_execution.stages.filter((candidate: J) => candidate.operation.kind === "external_command")) {
    ...
    codedAssert(boundary.attestation_artifact_sha256 === pins.sandbox.artifact.sha256 && canon(boundary.wrapper) === canon(sandbox.wrapper) && boundary.image_identity_kind === sandbox.image.identity_kind && boundary.image_immutable_identity === sandbox.image.immutable_identity && boundary.image_digest === sandbox.image.digest && boundary.repo_tree_sha256 === sandbox.repo_tree_sha256, "H2_PREACTIVATION_SANDBOX_BINDING", `stage ${entry.stage_id} sandbox identity differs from signed attestation`);
    codedAssert(boundary.wrapper_grammar_version === sandbox.wrapper_grammar_version && canon(boundary.fixed_argument_allowlist) === canon(sandbox.fixed_argument_allowlist) && canon(boundary.argv_prefix) === canon(sandbox.argv_prefix.map((item: J) => item.value)) && canon(boundary.argv_prefix_bindings) === canon(sandbox.argv_prefix.map(({ index, argument_kind, semantic_role, source_ref, value_sha256, semantic_fields, semantic_join_sha256 }: J) => ({ index, argument_kind, semantic_role, source_ref, value_sha256, semantic_fields, semantic_join_sha256 }))), "H2_PREACTIVATION_SANDBOX_ARGV", `stage ${entry.stage_id} wrapper argv prefix differs from signed typed semantics`);
    codedAssert(boundary.dependency_tree_sha256 === sandbox.runtime_tree.complete_tree_sha256 && boundary.dependency_tree_bytes === sandbox.runtime_tree.complete_tree_bytes && boundary.dependency_tree_members === sandbox.runtime_tree.member_count && canon(boundary.resolution_contract) === canon(sandbox.runtime_tree.resolution_contract), "H2_PREACTIVATION_RUNTIME_TREE", `stage ${entry.stage_id} complete runtime/module/native closure differs from signed attestation`);
    const dependencySnapshot = snapshotOperationFile(entry.operation.installed_dependency_tree, `stage ${entry.stage_id} installed dependency manifest`);
    try {
      const dependencyManifest = JSON.parse(dependencySnapshot.raw.toString("utf8")); schema("installed-dependency-tree.schema.v2.json", dependencyManifest);
      validateResolutionClosure(dependencyManifest.resolution_contract, dependencyManifest.physical_roots, dependencyManifest.members, "H2_PREACTIVATION_RUNTIME_TREE");
      codedAssert(canon(dependencyManifest.resolution_contract) === canon(sandbox.runtime_tree.resolution_contract) && canon(dependencyManifest.physical_roots) === canon(sandbox.runtime_tree.physical_roots) && canon(dependencyManifest.members) === canon(sandbox.runtime_tree.members) && dependencyManifest.tree_sha256 === sandbox.runtime_tree.complete_tree_sha256 && dependencyManifest.tree_bytes === sandbox.runtime_tree.complete_tree_bytes && dependencyManifest.member_count === sandbox.runtime_tree.member_count, "H2_PREACTIVATION_RUNTIME_TREE", `stage ${entry.stage_id} installed manifest does not equal the signed complete sandbox runtime/module/native closure`);
    } finally { fs.closeSync(dependencySnapshot.fd); }
    codedAssert(canon(boundary.readable_mounts) === canon(sandbox.mounts.readable_inputs) && canon(boundary.output_mounts) === canon(sandbox.mounts.output_mounts) && canon(boundary.writable_paths) === canon(sandbox.writable_confinement.writable_paths), "H2_PREACTIVATION_SANDBOX_MOUNTS", `stage ${entry.stage_id} mounts/writes differ from signed attestation`);
    codedAssert(boundary.network_policy === sandbox.network.policy && canon(boundary.network_capability_ids) === canon(sandbox.network.destination_capability_ids) && boundary.network_namespace_sha256 === sandbox.network.namespace_identity_sha256 && boundary.firewall_rules_sha256 === sandbox.network.firewall_rules_sha256, "H2_PREACTIVATION_SANDBOX_NETWORK", `stage ${entry.stage_id} network boundary differs from signed attestation`);
    const semantics = sandbox.executor_semantics.find((bundle: J) => bundle.capability_id === entry.operation.executor_capability_id);
...
const PUBLICATION_PRE_PLAN_PATHS = PUBLICATION_MEMBER_PATHS.filter((member) => !["candidate-descriptor-v2.json", "final-criterion-matrix-v2.json", "publication-assembly-plan-v2.json", "stage-execution-ledger-v2.json"].includes(member));
...
function publicationDirectoryPaths(baseDir: string): string[] { return files(baseDir); }
function validatePublicationAssemblyPlan(value: J, baseDir: string, authority: J, exactDirectoryPaths?: string[]): void {
  codedAssert(Array.isArray(value?.pre_plan_member_paths) && Array.isArray(value?.members_before_plan) && canon(value.pre_plan_member_paths) === canon(PUBLICATION_PRE_PLAN_PATHS) && canon(value.members_before_plan.map((member: J) => member?.path)) === canon(PUBLICATION_PRE_PLAN_PATHS), "H2_PUBLICATION_PRE_PLAN_UNIVERSE", "publication plan must contain the exact ordered pre-plan path set");
  ...
  if (exactDirectoryPaths) { const actualPaths = publicationDirectoryPaths(baseDir); const expectedPaths = [...exactDirectoryPaths].sort(); codedAssert(canon(actualPaths) === canon(expectedPaths), "H2_PUBLICATION_PRE_PLAN_UNIVERSE", `publication physical directory differs from the exact expected phase universe: actual=${canon(actualPaths)} expected=${canon(expectedPaths)}`); }
...
function finalMatrixRows(baseDir: string): J[] {
  const rolePins: Record<string, J> = {};
  for (const [role, filename] of Object.entries(FINAL_EVIDENCE_PATHS)) rolePins[role] = { ...pin(resolveEvidencePath(filename, baseDir), filename), role };
  return FINAL_CRITERION_IDS.map((criterion_id) => { const requirement = FINAL_MATRIX_REQUIREMENTS[criterion_id]; return { criterion_id, required: true, verdict: "satisfied", result_ids: requirement.results, evidence: requirement.roles.map((role) => structuredClone(rolePins[role])), limitations: [] }; });
}
function publicationDescriptorFromSnapshots(snapshots: Map<string, FileSnapshot>, authority: J, createdAt: string): J {
  const memberPins = [...snapshots.entries()].map(([member, snapshot]) => snapshotPin(snapshot, member)).sort((a, b) => a.path.localeCompare(b.path));
  const treeSha256 = hash(`${memberPins.map((x: J) => `${x.path}\t${x.sha256}\t${x.bytes}`).join("\n")}\n`);
  const dynamic = Object.fromEntries(Object.entries(PUBLICATION_DYNAMIC_PREDECESSORS).map(([role, filename]) => { const snapshot = snapshots.get(filename); codedAssert(snapshot !== undefined, "H2_PUBLICATION_EXACT_UNIVERSE", `retained publication snapshot missing ${filename}`); return [role, snapshotPin(snapshot, filename)]; }));
  return { schema_version: "reviewed_metrics_publication_descriptor_v2.0.0", artifact_id: PUBLICATION_ID, status: "published", created_at: createdAt, implementation_base_commit: IMPLEMENTATION_BASE_COMMIT, candidate_commit: authority.candidate_commit, members_before_descriptor: memberPins, tree_before_descriptor_sha256: treeSha256, counts: { files_before_descriptor: memberPins.length, bytes_before_descriptor: memberPins.reduce((sum: number, member: J) => sum + member.bytes, 0), unique_sources: 44, task_memberships: 78, published_members: memberPins.length + 1 }, predecessors: { ...predecessorPins(), ...dynamic }, completion: { candidate_complete: true, issue_92_complete: true, issue_69_complete: true, publication_exists: true }, mutations: { production: false, search_index: false, private_object_store_write: false, paid_gpu: false }, required_publication_members: PUBLICATION_MEMBER_PATHS, publisher: { principal: authority.publisher.principal, session_id: authority.publisher.session_id, model: authority.publisher.model, role: authority.publisher.role, published_at: createdAt } };
}
function publicationDescriptorFromDirectory(baseDir: string, authority: J, createdAt: string): J {
  const snapshots = new Map<string, FileSnapshot>();
  try {
    for (const member of files(baseDir)) snapshots.set(member, readJsonSnapshot(path.join(baseDir, member), "H2_PUBLICATION_MEMBER_SNAPSHOT", false, undefined, [0o400, 0o444, 0o600, 0o644]));
    return publicationDescriptorFromSnapshots(snapshots, authority, createdAt);
  } finally { for (const snapshot of snapshots.values()) closeSnapshot(snapshot); }
}
function assemblePublication(baseDir: string, planFile: string, ledgerFile: string, authority: J, beforeDescriptorCommit?: () => void): J {
  const descriptorFile = path.join(baseDir, "candidate-descriptor-v2.json"); const matrixFile = path.join(baseDir, "final-criterion-matrix-v2.json");
  codedAssert(!fs.existsSync(descriptorFile) && !fs.existsSync(matrixFile), "H2_PUBLICATION_COMMIT_LAST", "matrix/descriptor must be absent before coordinator assembly");
  codedAssert(path.resolve(planFile) === path.join(baseDir, "publication-assembly-plan-v2.json") && path.resolve(ledgerFile) === path.join(baseDir, "stage-execution-ledger-v2.json"), "H2_PUBLICATION_PLAN_BINDING", "assembler requires canonical plan and ledger paths");
  const plan = load(planFile); const ledger = load(ledgerFile);
  validatePublicationAssemblyPlan(plan, baseDir, authority, [...PUBLICATION_PRE_PLAN_PATHS, "publication-assembly-plan-v2.json", "stage-execution-ledger-v2.json"]);
  validateStageExecutionEvidence(ledger, authority);
  codedAssert(ledger.stages.at(-1)?.outputs.some((output: J) => output.artifact_role === "publication_assembly_plan" && output.sha256 === pin(planFile, "publication-assembly-plan-v2.json").sha256), "H2_PUBLICATION_PLAN_BINDING", "signed final stage does not bind the exact plan bytes");
  const matrix = { schema_version: "reviewed_metrics_final_criterion_matrix_v2.0.0", status: "final", candidate_id: CANDIDATE_ID, rows: finalMatrixRows(baseDir), stage_execution_ledger: pin(ledgerFile, "stage-execution-ledger-v2.json"), issue_92_complete: true, issue_69_complete: true };
  writeJsonExclusive(matrixFile, matrix);
  validateMatrixValue(matrix, baseDir, authority, ledger);
  const preDescriptorPaths = [...PUBLICATION_MEMBER_PATHS.filter((member) => member !== "candidate-descriptor-v2.json")].sort();
  codedAssert(canon(publicationDirectoryPaths(baseDir)) === canon(preDescriptorPaths), "H2_PUBLICATION_EXACT_UNIVERSE", "pre-descriptor exact finalized member universe differs");
  const snapshots = new Map<string, FileSnapshot>();
  try {
    for (const member of preDescriptorPaths) snapshots.set(member, readJsonSnapshot(path.join(baseDir, member), "H2_PUBLICATION_MEMBER_SNAPSHOT", false, undefined, [0o400, 0o444, 0o600, 0o644]));
    codedAssert(canon(snapshots.get("publication-assembly-plan-v2.json")?.value) === canon(plan) && canon(snapshots.get("stage-execution-ledger-v2.json")?.value) === canon(ledger), "H2_PUBLICATION_PLAN_BINDING", "retained plan or ledger differs from the validated value");
    const descriptor = publicationDescriptorFromSnapshots(snapshots, authority, ledger.sealed_at);
    validatePublicationValue(descriptor, baseDir, authority, true);
    const result = { status: "publication_assembled_commit_last", matrix_sha256: hash(Buffer.from(pretty(matrix))), descriptor_sha256: hash(Buffer.from(pretty(descriptor))), members: 17 };
    beforeDescriptorCommit?.();
    for (const snapshot of snapshots.values()) assertSnapshotPathUnchanged(snapshot);
    codedAssert(canon(publicationDirectoryPaths(baseDir)) === canon(preDescriptorPaths), "H2_PUBLICATION_EXACT_UNIVERSE", "directory changed after in-memory descriptor validation");
    const rederived = publicationDescriptorFromSnapshots(snapshots, authority, ledger.sealed_at);
    codedAssert(canon(rederived) === canon(descriptor), "H2_PUBLICATION_MEMBER_SNAPSHOT", "descriptor no longer rederives from the retained exact member bytes");
    codedAssert(canon(snapshots.get("publication-assembly-plan-v2.json")?.value) === canon(plan) && canon(snapshots.get("stage-execution-ledger-v2.json")?.value) === canon(ledger), "H2_PUBLICATION_PLAN_BINDING", "plan or ledger was substituted before commit-last descriptor creation");
    writeOwnedExclusive(descriptorFile, Buffer.from(pretty(descriptor)), "H2_PUBLICATION_COMMIT_LAST");
    return result;
  } finally { for (const snapshot of snapshots.values()) { try { closeSnapshot(snapshot); } catch { /* descriptor write remains the final semantic operation */ } } }
}
function internalPublicationRace(baseDir: string, specRaw: string): void {
  let spec: J; try { spec = JSON.parse(specRaw); } catch { throw new GateH2Error("H2_PUBLICATION_MEMBER_SNAPSHOT", "internal publication race spec is not JSON"); }
  const target = path.resolve(baseDir, spec.target);
  codedAssert(target.startsWith(`${path.resolve(baseDir)}${path.sep}`) && target !== path.join(baseDir, "candidate-descriptor-v2.json"), "H2_PUBLICATION_MEMBER_SNAPSHOT", "internal publication race target escapes the member directory");
  const original = fs.readFileSync(target); const alternate = Buffer.from(original); if (alternate.length > 0) alternate[alternate.length - 1] ^= 1;
  const temporary = path.join(baseDir, `.race-${crypto.randomUUID()}`);
  if (spec.operation === "same_inode_bytes") fs.writeFileSync(target, alternate);
  else if (spec.operation === "same_size_replace") { fs.writeFileSync(temporary, alternate, { mode: 0o600 }); fs.renameSync(temporary, target); }
  else if (spec.operation === "symlink_replace") { fs.unlinkSync(target); fs.symlinkSync(spec.source ?? path.join(baseDir, "input-authority-v2.json"), target); }
  else if (spec.operation === "hardlink_replace") { const source = path.join(baseDir, spec.source ?? "input-authority-v2.json"); fs.linkSync(source, temporary); fs.renameSync(temporary, target); }
  else throw new GateH2Error("H2_PUBLICATION_MEMBER_SNAPSHOT", "unknown internal publication race operation");
}
...
function validatePublicationValue(
  ...
  baseDir = ROOT,
  authority?: J,
  descriptorAbsentForCommit = false,
): void {
  ...
  codedAssert(
    canon([...pins.map((x: J) => x.path)].sort()) ===
      canon(PUBLICATION_MEMBER_PATHS.filter((member) => member !== "candidate-descriptor-v2.json")),
    "H2_PUBLICATION_EXACT_UNIVERSE",
  ...
  exactSet(
    actualFiles,
    descriptorAbsentForCommit
      ? PUBLICATION_MEMBER_PATHS.filter((member) => member !== "candidate-descriptor-v2.json")
...
async function baselineContractSelfTest(): Promise<J> {
  ...
  const tempRootNames = (prefix: string): string[] => fs.readdirSync(os.tmpdir()).filter((name) => name.startsWith(prefix)).sort();
  const preExistingReplayRoots = tempRootNames("rmv2-replay-");
  const preExistingSelfRoots = tempRootNames("rmv2-self-").filter((name) => name !== path.basename(root));
  try {
    ...
    const replayRootsBeforeFailure = tempRootNames("rmv2-replay-"); let replayFailureObserved = false;
    try { await verifyCandidate(candidate, false, () => { throw new GateH2Error("H2_TEST_TEMP_CLEANUP", "injected replay failure"); }); }
    catch (error) { replayFailureObserved = error instanceof GateH2Error && error.code === "H2_TEST_TEMP_CLEANUP"; }
    codedAssert(replayFailureObserved && canon(tempRootNames("rmv2-replay-")) === canon(replayRootsBeforeFailure), "H2_TEST_TEMP_CLEANUP", "injected replay failure left a new temporary root");
    const selfRootsBeforeFailure = tempRootNames("rmv2-self-"); let injectedSelfRoot = ""; let selfFailureObserved = false;
    try { injectedSelfRoot = fs.mkdtempSync(path.join(os.tmpdir(), "rmv2-self-")); try { throw new GateH2Error("H2_TEST_TEMP_CLEANUP", "injected self-test failure"); } finally { fs.rmSync(injectedSelfRoot, { recursive: true, force: true }); } }
    catch (error) { selfFailureObserved = error instanceof GateH2Error && error.code === "H2_TEST_TEMP_CLEANUP"; }
    codedAssert(selfFailureObserved && !fs.existsSync(injectedSelfRoot) && canon(tempRootNames("rmv2-self-")) === canon(selfRootsBeforeFailure), "H2_TEST_TEMP_CLEANUP", "injected self-test failure left a new temporary root");
    ...
    return {
      ...
      temp_cleanup: { injected_failure_roots_cleaned: true, pre_existing_not_removed: { replay: preExistingReplayRoots, self: preExistingSelfRoots } },
    };
  } finally {
...
async function sourceSearchSelfTest(): Promise<J> {
  ...
  try {
    ...
    for (const [label, mutate] of [["publication-plan-missing", (x: J) => x.members_before_plan.pop()], ["publication-plan-reordered", (x: J) => x.members_before_plan.reverse()]] as [string, (x: J) => void][]) { const bad = structuredClone(publicationPlan); mutate(bad); await reject("final_chain", label, () => validatePublicationAssemblyPlan(bad, finalDir, authority), "H2_PUBLICATION_PRE_PLAN_UNIVERSE"); }
    const extraFile = path.join(finalDir, "unplanned-extra.json"); fs.writeFileSync(extraFile, "{}\n");
    await reject("final_chain", "publication-plan-extra-file", () => validatePublicationAssemblyPlan(publicationPlan, finalDir, authority, [...PUBLICATION_PRE_PLAN_PATHS, "publication-assembly-plan-v2.json", "stage-execution-ledger-v2.json"]), "H2_PUBLICATION_PRE_PLAN_UNIVERSE"); fs.unlinkSync(extraFile);
    fs.unlinkSync(path.join(finalDir, "final-criterion-matrix-v2.json"));
    const planRaw = fs.readFileSync(publicationPayload); const substitutedPlan = structuredClone(publicationPlan); substitutedPlan.authority_hash = "0".repeat(64); fs.writeFileSync(publicationPayload, pretty(substitutedPlan));
    await reject("final_chain", "publication-plan-substitution", () => assemblePublication(finalDir, publicationPayload, path.join(finalDir, "stage-execution-ledger-v2.json"), authority), "H2_PUBLICATION_PLAN_BINDING"); fs.writeFileSync(publicationPayload, planRaw);
    const ledgerFile = path.join(finalDir, "stage-execution-ledger-v2.json"); const ledgerRaw = fs.readFileSync(ledgerFile); fs.writeFileSync(ledgerFile, pretty(forgedEvidence));
    await reject("final_chain", "publication-ledger-substitution", () => assemblePublication(finalDir, publicationPayload, ledgerFile, authority), "H2_LEDGER_SIGNATURE"); fs.writeFileSync(ledgerFile, ledgerRaw);
    writeJsonExclusive(path.join(finalDir, "candidate-descriptor-v2.json"), { early: true });
    await reject("final_chain", "publication-early-descriptor", () => assemblePublication(finalDir, publicationPayload, path.join(finalDir, "stage-execution-ledger-v2.json"), authority), "H2_PUBLICATION_COMMIT_LAST"); fs.unlinkSync(path.join(finalDir, "candidate-descriptor-v2.json"));
    const cli = path.join(ROOT, "node_modules/.bin/tsx"); const scriptFile = fileURLToPath(import.meta.url); const assemblyCapability = hash(`gate-h2-internal-two-process-v2\n${canon(authority)}`);
    const cliBase = path.join(root, "publication-cli-base"); fs.cpSync(finalDir, cliBase, { recursive: true });
    const assemblyArgs = (dir: string) => [scriptFile, "assemble-publication", "--authority", path.join(dir, "execution-authorization-v2.json"), "--workspace", dir, "--input", path.join(dir, "publication-assembly-plan-v2.json"), "--envelope", path.join(dir, "stage-execution-ledger-v2.json")];
    const runAssembly = (label: string, mutate?: (dir: string) => void, extraEnv: NodeJS.ProcessEnv = {}): { ok: boolean; stderr: string; dir: string } => {
      const dir = path.join(root, `publication-cli-${label}`); fs.cpSync(cliBase, dir, { recursive: true }); mutate?.(dir);
      try { execFileSync(cli, assemblyArgs(dir), { cwd: ROOT, env: { ...process.env, GATE_H2_INTERNAL_TEST_CAPABILITY: assemblyCapability, ...extraEnv }, stdio: ["ignore", "pipe", "pipe"] }); return { ok: true, stderr: "", dir }; }
      catch (error) { return { ok: false, stderr: (error as { stderr?: Buffer }).stderr?.toString("utf8") ?? "", dir }; }
    };
    const expectAssemblyFailure = (label: string, code: string, mutate?: (dir: string) => void, race = false, expectDescriptorAbsent = true): void => {
      const raceDir = path.join(root, `publication-cli-${label}`); const result = runAssembly(label, mutate, race ? { GATE_H2_INTERNAL_ASSEMBLY_RACE_FILE: path.join(raceDir, "late-race.json") } : {});
      const descriptorExists = fs.existsSync(path.join(result.dir, "candidate-descriptor-v2.json"));
      codedAssert(!result.ok && result.stderr.includes(code) && (expectDescriptorAbsent ? !descriptorExists : descriptorExists), "H2_PUBLICATION_CLI_TEST", `${label} did not fail ${code} with the expected descriptor state: ${result.stderr.slice(0, 500)}`);
    };
    expectAssemblyFailure("missing", "H2_PUBLICATION_PRE_PLAN_UNIVERSE", (dir) => { const value = load(path.join(dir, "publication-assembly-plan-v2.json")); value.members_before_plan.pop(); fs.writeFileSync(path.join(dir, "publication-assembly-plan-v2.json"), pretty(value)); });
    expectAssemblyFailure("reorder", "H2_PUBLICATION_PRE_PLAN_UNIVERSE", (dir) => { const value = load(path.join(dir, "publication-assembly-plan-v2.json")); value.members_before_plan.reverse(); fs.writeFileSync(path.join(dir, "publication-assembly-plan-v2.json"), pretty(value)); });
    expectAssemblyFailure("extra", "H2_PUBLICATION_PRE_PLAN_UNIVERSE", (dir) => fs.writeFileSync(path.join(dir, "extra.json"), "{}\n"));
    expectAssemblyFailure("plan-substitution", "H2_PUBLICATION_PLAN_BINDING", (dir) => { const value = load(path.join(dir, "publication-assembly-plan-v2.json")); value.authority_hash = "0".repeat(64); fs.writeFileSync(path.join(dir, "publication-assembly-plan-v2.json"), pretty(value)); });
    expectAssemblyFailure("ledger-substitution", "H2_LEDGER_SIGNATURE", (dir) => { const value = load(path.join(dir, "stage-execution-ledger-v2.json")); value.signature_base64 = Buffer.alloc(64, 7).toString("base64"); fs.writeFileSync(path.join(dir, "stage-execution-ledger-v2.json"), pretty(value)); });
    expectAssemblyFailure("early", "H2_PUBLICATION_COMMIT_LAST", (dir) => fs.writeFileSync(path.join(dir, "candidate-descriptor-v2.json"), "{}\n"), false, false);
    expectAssemblyFailure("late-race", "H2_PUBLICATION_EXACT_UNIVERSE", undefined, true);
    const expectMemberRace = (label: string, operation: string, target: string, code: string, source?: string): void => {
      const result = runAssembly(label, undefined, { GATE_H2_INTERNAL_ASSEMBLY_RACE_SPEC: JSON.stringify({ operation, target, source }) });
      codedAssert(!result.ok && result.stderr.includes(code) && !fs.existsSync(path.join(result.dir, "candidate-descriptor-v2.json")), "H2_PUBLICATION_CLI_TEST", `${label} did not reject retained-member race with ${code}: ${result.stderr.slice(0, 500)}`);
    };
    expectMemberRace("same-inode-bytes", "same_inode_bytes", "reviewed-metrics-v2.json", "H2_FINALIZATION_EXACT_BYTES");
    expectMemberRace("same-size-replace", "same_size_replace", "reviewed-metrics-v2.json", "H2_FINALIZATION_PATH_SUBSTITUTION");
    expectMemberRace("plan-replace", "same_size_replace", "publication-assembly-plan-v2.json", "H2_FINALIZATION_PATH_SUBSTITUTION");
    expectMemberRace("ledger-replace", "same_size_replace", "stage-execution-ledger-v2.json", "H2_FINALIZATION_PATH_SUBSTITUTION");
    expectMemberRace("symlink-replace", "symlink_replace", "reviewed-metrics-v2.json", "H2_FINALIZATION_PATH_SUBSTITUTION", path.join(root, "publication-cli-symlink-replace", "input-authority-v2.json"));
    expectMemberRace("hardlink-replace", "hardlink_replace", "reviewed-metrics-v2.json", "H2_FINALIZATION_PATH_SUBSTITUTION", "input-authority-v2.json");
    const cliSuccess = runAssembly("success"); codedAssert(cliSuccess.ok && fs.existsSync(path.join(cliSuccess.dir, "candidate-descriptor-v2.json")), "H2_PUBLICATION_CLI_TEST", `assemble-publication CLI success failed: ${cliSuccess.stderr}`);
    const concurrentDir = path.join(root, "publication-cli-concurrent"); fs.cpSync(cliBase, concurrentDir, { recursive: true });
    const concurrentRun = () => new Promise<{ code: number | null; stderr: string }>((resolve) => { const child = spawn(cli, assemblyArgs(concurrentDir), { cwd: ROOT, env: { ...process.env, GATE_H2_INTERNAL_TEST_CAPABILITY: assemblyCapability }, stdio: ["ignore", "ignore", "pipe"] }); let stderr = ""; child.stderr.on("data", (chunk) => { stderr += chunk.toString(); }); child.on("close", (code) => resolve({ code, stderr })); });
    const concurrentResults = await Promise.all([concurrentRun(), concurrentRun()]); codedAssert(concurrentResults.filter((item) => item.code === 0).length === 1 && concurrentResults.filter((item) => item.code !== 0).length === 1 && fs.existsSync(path.join(concurrentDir, "candidate-descriptor-v2.json")), "H2_PUBLICATION_CLI_TEST", "concurrent assemble-publication CLI must produce one commit and one rejection");
    await accept("final_chain", "publication-production-assembler", () => assemblePublication(finalDir, publicationPayload, path.join(finalDir, "stage-execution-ledger-v2.json"), authority));
    await reject("final_chain", "publication-concurrent-assembler", () => assemblePublication(finalDir, publicationPayload, ledgerFile, authority), "H2_PUBLICATION_COMMIT_LAST");
    const publication = load(path.join(finalDir, "candidate-descriptor-v2.json"));
...
async function d1SealOrchestrationSelfTest(root: string): Promise<J> {
  const account = "synthetic-account-capability"; const database = "synthetic-database-capability"; const apiToken = "synthetic-token-capability";
  const authority = syntheticExecutionAuthority(hash("synthetic-d1-seal-bundle"));
  authority.stage_execution.ledger.account_capability_digest = hash(`gate-h2-d1-account-capability-v1\0${account}`);
  authority.stage_execution.ledger.database_uuid_digest = hash(`gate-h2-d1-database-uuid-v1\0${database}`);
  for (const entry of authority.stage_execution.stages) { const output = path.join(root, `d1-${entry.stage_id}.json`); fs.writeFileSync(output, `${JSON.stringify({ stage: entry.stage_id })}\n`, { mode: 0o600 }); entry.outputs = [{ artifact_role: `${entry.stage_id}_output`, path: output }]; }
  sealSyntheticAuthority(authority, true); await materializeSyntheticCompletedStageLedger(authority);
  const ledger = stageLedger(authority, { [INTERNAL_SYNTHETIC_CAPABILITY]: true }) as InternalAppendOnlyStageLedger;
  const readback = await ledger.readAll(authority.candidate_commit, authorityBindingHash(authority)); const schemaRows = gateH2LedgerSchemaAttestation(ROOT).rows;
  const authorityFile = path.join(root, "d1-seal-authority.json"); const keyFile = path.join(root, "d1-seal-key.pem");
  ...
  const mutations: Array<[string, string, (fixture: J) => void]> = [
    ...
    ["missing", "H2_LEDGER_READBACK", (f) => { f.readback.completions.pop(); }],
    ["duplicate", "H2_LEDGER_READBACK", (f) => { f.readback.attempts.push(structuredClone(f.readback.attempts[0])); }],
    ["envelope_hash", "H2_LEDGER_READBACK", (f) => { f.readback.claims[0].begin_sha256 = "e".repeat(64); }],
  ...
  const runCli = (label: string, fixture: J): { ok: boolean; stderr: string; output: string } => {
    ...
    try { execFileSync(cli, [scriptFile, "seal-stage-ledger", "--authority", authorityFile, "--signing-key", keyFile, "--output", output], { cwd: ROOT, env: { ...process.env, GATE_H2_INTERNAL_TEST_CAPABILITY: capability, GATE_H2_INTERNAL_D1_FIXTURE: fixturePath }, stdio: ["ignore", "pipe", "pipe"] }); return { ok: true, stderr: "", output }; }
    catch (error) { return { ok: false, stderr: (error as { stderr?: Buffer }).stderr?.toString("utf8") ?? "", output }; }
  };
  for (const [label, code, mutate] of mutations) {
    const fixture = structuredClone(baseFixture); mutate(fixture);
    const adapter = internalD1FixtureAdapter(authority, fixtureFile(`direct-${label}`, fixture), { [INTERNAL_SYNTHETIC_CAPABILITY]: true });
    let directCode = ""; try { await completedStageLedgerEvidence(authority, true, { [INTERNAL_SYNTHETIC_CAPABILITY]: true }, keyFile, adapter); } catch (error) { directCode = error instanceof GateH2Error ? error.code : ""; }
    codedAssert(directCode === code, "H2_D1_SEAL_TEST", `completedStageLedgerEvidence ${label} expected ${code}, observed ${directCode || "none"}`);
    const result = runCli(label, fixture); codedAssert(!result.ok && result.stderr.includes(code) && !fs.existsSync(result.output), "H2_D1_SEAL_TEST", `seal-stage-ledger ${label} did not fail with ${code} before output`);
  }
  const successAdapter = internalD1FixtureAdapter(authority, fixtureFile("direct-success", baseFixture), { [INTERNAL_SYNTHETIC_CAPABILITY]: true });
  const direct = await completedStageLedgerEvidence(authority, true, { [INTERNAL_SYNTHETIC_CAPABILITY]: true }, keyFile, successAdapter);
  codedAssert(canon(direct.attempts.map((entry: J) => entry.stage_id)) === canon(STAGE_IDS) && direct.attempts_digest === hash(canon(direct.attempts)), "H2_D1_SEAL_TEST", "successful direct seal lacks exact ordered attempts/digest");
  const success = runCli("success", baseFixture); codedAssert(success.ok, "H2_D1_SEAL_TEST", `successful seal-stage-ledger failed: ${success.stderr}`); validateStageExecutionEvidence(load(success.output), authority);
  return { status: "production_shaped_d1_seal_orchestration_passed", cases: [...mutations.map(([label]) => label), "success"], schema_attested_before_readback: true, no_real_d1_write: true };
}
function stageRunSubprocessSelfTest(root: string): J {
  ...
  fs.mkdirSync(dependencyRoot, { recursive: true }); fs.writeFileSync(dependencyMember, "module.exports='stage';\n", { mode: 0o400 }); fs.writeFileSync(path.join(dependencyRoot, "package.json"), pretty({ name: "stage-runtime", version: "1.0.0", main: "index.js" }), { mode: 0o400 }); fs.writeFileSync(script, `const fs=require('node:fs');
...
`, { mode: 0o600 }); fs.writeFileSync(wrapper, syntheticInProcessWrapperSource(), { mode: 0o500 }); fs.writeFileSync(prompt, "opaque input\n", { mode: 0o600 }); fs.writeFileSync(packageManifest, pretty({ name: "stage-run-test", version: "1.0.0", private: true, dependencies: { "stage-runtime": "1.0.0" } }), { mode: 0o600 }); fs.writeFileSync(lock, pretty({ name: "stage-run-test", version: "1.0.0", lockfileVersion: 3, requires: true, packages: { "": { name: "stage-run-test", version: "1.0.0", dependencies: { "stage-runtime": "1.0.0" } }, "node_modules/stage-runtime": { name: "stage-runtime", version: "1.0.0", resolved: "file:synthetic-fixture-only" } } }), { mode: 0o600 });
  ...
  const dependencyValue = load(dependencies); const wrapperPin = filePin(wrapper); const runtimePin = filePin(process.execPath, process.version); const scriptPin = filePin(script); const imageDigest = `sha256:${hash("local-stage-run-boundary")}`; const outputMounts = [{ artifact_role: "stage_output", host_path: output, guest_path: "/output/result", mode: "rw", content_sha256: hash("empty-stage-run-output") }]; const semantics = syntheticExecutorSemanticsBundle({ wrapper: wrapperPin, runtime: runtimePin, stageProgram: scriptPin, imageDigest, readableMounts: [], outputMounts, writablePaths: [output] }); const boundaryPayload = { kind: "linux_sandbox_attestation_v1", platform: "linux", wrapper: wrapperPin, argv_prefix: [], image_identity_kind: "oci_manifest", image_immutable_identity: "synthetic.invalid/stage-run@sha256", image_digest: imageDigest, resolution_contract: dependencyValue.resolution_contract, dependency_tree_sha256: dependencyValue.tree_sha256, executor_semantics: semantics, network_policy: "deny_all", network_capability_ids: [], writable_paths: [output], repo_tree_sha256: repoTree };
  const operation = { kind: "external_command", execution_boundary: { ...boundaryPayload, attestation_sha256: hash(canon(boundaryPayload)) }, runtime: runtimePin, script: scriptPin, artifacts: [{ artifact_role: "sanitized_prompt", pin: filePin(prompt) }], dependency_lock: filePin(lock), installed_dependency_tree: filePin(dependencies), repo_tree_sha256: repoTree, cwd: fs.realpathSync(work), argv: [fs.realpathSync(script), fs.realpathSync(prompt)], argv_bindings: [{ index: 0, artifact_role: "script", value_sha256: hash(fs.realpathSync(script)) }, { index: 1, artifact_role: "sanitized_prompt", value_sha256: hash(fs.realpathSync(prompt)) }], declared_output_paths: [output], executor_capability_id: semantics.capability_id, network_policy: "deny_all", network_capability_ids: [], environment: { public: [{ name: "OUT", value: output, value_sha256: hash(output), binding: "declared_output" }], secret_capability_ids: [] } };
  const account = "stage-run-account"; const database = "stage-run-database"; const authority = syntheticExecutionAuthority(hash("stage-run-bundle")); authority.stage_execution.ledger.account_capability_digest = hash(`gate-h2-d1-account-capability-v1\0${account}`); authority.stage_execution.ledger.database_uuid_digest = hash(`gate-h2-d1-database-uuid-v1\0${database}`); authority.stage_execution.stages.find((entry: J) => entry.stage_id === "visual_predict").operation = operation; authority.stage_execution.stages.find((entry: J) => entry.stage_id === "visual_predict").outputs = [{ artifact_role: "visual_predict_output", path: output }]; authority.predictor.route = authority.predictor.canonical_root = fs.realpathSync(work);
  const now = Date.now(); const at = (offset: number) => new Date(now + offset).toISOString(); authority.authorized_at = at(-5_000); authority.started_at = at(-2_000); authority.ended_at = at(60_000); authority.freeze_at = at(61_000); authority.source_search_started_at = at(62_000); authority.source_search_ended_at = at(63_000); authority.source_search_freeze_at = at(64_000); authority.source_dossier_authored_at = at(65_000); authority.private_envelope_sealed_at = at(66_000); authority.expires_at = at(120_000); sealSyntheticAuthority(authority, true);
  const keyFile = path.join(root, "stage-run-key.pem"); fs.writeFileSync(keyFile, SYNTHETIC_COORDINATOR_KEYS.privateKey.export({ type: "pkcs8", format: "pem" }), { mode: 0o600 }); const cli = path.join(ROOT, "node_modules/.bin/tsx"); const scriptFile = fileURLToPath(import.meta.url);
  const invoke = (label: string, mutate?: (value: J) => void, race?: string): { ok: boolean; stderr: string; fixture: J } => {
...
async function preActivationFailClosedSelfTest(root: string): Promise<J> {
  ...
  const runtimeScript = filePinForAttestation(fileURLToPath(import.meta.url), "gate-h2-script-v2"); const runtimePackageFile = path.join(root, "runtime-package.json"); const runtimeLockFile = path.join(root, "runtime-lock.json"); fs.writeFileSync(runtimePackageFile, pretty({ name: "gate-h2-preactivation-fixture", version: "1.0.0", private: true }), { mode: 0o600 }); fs.writeFileSync(runtimeLockFile, pretty({ name: "gate-h2-preactivation-fixture", version: "1.0.0", lockfileVersion: 3, requires: true, packages: { "": { name: "gate-h2-preactivation-fixture", version: "1.0.0" } } }), { mode: 0o600 }); const runtimeLock = filePinForAttestation(runtimeLockFile, "npm-lock-v3"); const runtimePackage = filePinForAttestation(runtimePackageFile, "npm-package-v1");
  const runtimeMembers = [wrapper, runtimeScript, runtimeLock, runtimePackage].sort((a: J, b: J) => a.realpath.localeCompare(b.realpath)); const runtimeRoots = runtimeMembers.map((member: J) => ({ path: member.path, realpath: member.realpath, discovery_rule: "recursive_regular_files_no_symlinks_v1" })); const runtimeResolution = makeZeroDependencyStageProgramResolutionClosure(wrapper, wrapper, runtimeScript, runtimeLock, runtimePackage, runtimeRoots, runtimeMembers, root); const runtimeBytes = dependencyTreeBytes(runtimeResolution, runtimeRoots, runtimeMembers);
  const imageDigest = `sha256:${hash("image-alpha")}`; const readableInputs = [{ artifact_role: "runtime", host_path: wrapper.realpath, guest_path: "/runtime/node", mode: "ro", content_sha256: wrapper.sha256 }]; const outputMounts = [{ artifact_role: "stage_output", host_path: output, guest_path: "/output/result", mode: "rw", content_sha256: hash("empty-output-alpha") }]; const executorSemantics = syntheticExecutorSemanticsBundle({ wrapper, runtime: wrapper, stageProgram: runtimeScript, imageDigest, readableMounts: readableInputs, outputMounts, writablePaths: [output], observedAt });
  const sandbox: J = { schema_version: "reviewed_metrics_linux_sandbox_attestation_v2.3.0", status: "observed_live_enforced", synthetic: false, platform: "linux", wrapper, image: { identity_kind: "oci_manifest", immutable_identity: "registry.local/gate-h2@sha256", digest: imageDigest }, runtime_tree: { runtime: wrapper, resolution_contract: runtimeResolution, physical_roots: runtimeRoots, members: runtimeMembers, complete_tree_sha256: hash(runtimeBytes), complete_tree_bytes: runtimeBytes.length, member_count: runtimeMembers.length, enumeration_contract: "sorted_physical_regular_files_path_sha256_bytes_v2" }, executor_semantics: [executorSemantics], wrapper_grammar_version: "gate_h2_linux_wrapper_argv_v3", fixed_argument_allowlist: [], argv_prefix: [], mounts: { readable_inputs: readableInputs, output_mounts: outputMounts }, network: { namespace_identity_sha256: hash("network-namespace-alpha"), firewall_rules_sha256: hash("firewall-alpha"), policy: "deny_all", destination_capability_ids: [], observed_destinations_digest: hash("no-destinations-alpha") }, writable_confinement: { writable_paths: [output], read_only_root: true, pre_state_sha256: hash("pre-state-alpha"), post_state_sha256: hash("post-state-alpha"), enforcement_evidence_sha256: hash("enforcement-alpha") }, repo_tree_sha256: hash("repo-tree-alpha"), observation: { observed_at: observedAt, principal: sandboxIdentity.principal, session_id: sandboxIdentity.session_id, route: sandboxIdentity.route, physical_route_identity_sha256: sandboxIdentity.physical_route_identity_sha256 }, signer: sandboxIdentity, signature_base64: "" }; sandbox.argv_prefix = deriveSandboxArgvPrefix(sandbox); sign(sandbox, 1);
  ...
  const buildAuthority = (mutate?: (values: { authority: J; d1: J; sandbox: J; verification: J }) => void): J => {
    const values = { authority: syntheticExecutionAuthority(hash("preactivation-bundle-alpha")), d1: structuredClone(d1), sandbox: structuredClone(sandbox), verification: {} as J };
    values.authority.schema_version = "reviewed_metrics_execution_authorization_v2.4.0"; const authorityBase = Date.now(); values.authority.authorized_at = new Date(authorityBase).toISOString(); values.authority.started_at = new Date(authorityBase + 1_000).toISOString(); values.authority.ended_at = new Date(authorityBase + 2_000).toISOString(); values.authority.freeze_at = new Date(authorityBase + 3_000).toISOString(); values.authority.source_search_started_at = new Date(authorityBase + 4_000).toISOString(); values.authority.source_search_ended_at = new Date(authorityBase + 5_000).toISOString(); values.authority.source_search_freeze_at = new Date(authorityBase + 6_000).toISOString(); values.authority.source_dossier_authored_at = new Date(authorityBase + 7_000).toISOString(); values.authority.private_envelope_sealed_at = new Date(authorityBase + 8_000).toISOString(); values.authority.expires_at = new Date(authorityBase + 300_000).toISOString(); values.authority.stage_execution.ledger = { ...values.authority.stage_execution.ledger, account_capability_digest: d1.account_capability_digest, database_uuid_digest: d1.database_uuid_digest, namespace_digest: d1.namespace_digest, table_schema_sha256: schemaEvidence.sha256, table_schema_bytes: schemaEvidence.bytes };
    const d1Pin = writeArtifact(`d1-${crypto.randomUUID()}.json`, values.d1); const sandboxPin = writeArtifact(`sandbox-${crypto.randomUUID()}.json`, values.sandbox);
    values.verification = sign({ schema_version: "reviewed_metrics_pre_activation_verification_v2.2.0", status: "live_prerequisites_verified", synthetic: false, d1_attestation: { sha256: d1Pin.sha256, bytes: d1Pin.bytes }, sandbox_attestation: { sha256: sandboxPin.sha256, bytes: sandboxPin.bytes }, live_d1_schema_sha256: schemaEvidence.sha256, executor_capability_ids: values.sandbox.executor_semantics.map((bundle: J) => bundle.capability_id).sort(), admission_validity_policy: PREACTIVATION_ADMISSION_VALIDITY_POLICY, verified_at: verifiedAt, verifier: verifierIdentity, signature_base64: "" }, 2);
    mutate?.(values);
    const finalD1Pin = writeArtifact(`d1-final-${crypto.randomUUID()}.json`, values.d1); const finalSandboxPin = writeArtifact(`sandbox-final-${crypto.randomUUID()}.json`, values.sandbox); values.verification.d1_attestation = { sha256: finalD1Pin.sha256, bytes: finalD1Pin.bytes }; values.verification.sandbox_attestation = { sha256: finalSandboxPin.sha256, bytes: finalSandboxPin.bytes }; values.verification.signature_base64 = crypto.sign(null, unsignedAttestationPayload(values.verification), keys[2].privateKey).toString("base64"); const verificationPin = writeArtifact(`verification-${crypto.randomUUID()}.json`, values.verification);
    const attestationPin = (artifact: J, schemaName: string, signer: J) => ({ artifact, schema_sha256: schemaFilePin(schemaName).sha256, attestor_principal: signer.principal, attestor_session_id: signer.session_id, attestor_route: signer.route, attestor_physical_route_identity_sha256: signer.physical_route_identity_sha256, signer_public_key_sha256: signer.public_key_sha256 });
    values.authority.pre_activation_attestations = { contract_version: "gate_h2_pre_activation_attestations_v1", max_age_seconds: 3600, d1: attestationPin(finalD1Pin, "d1-live-attestation.schema.v2.json", values.d1.signer), sandbox: attestationPin(finalSandboxPin, "linux-sandbox-attestation.schema.v2.json", values.sandbox.signer), verification: attestationPin(verificationPin, "pre-activation-verification.schema.v2.json", values.verification.verifier) }; sealSyntheticAuthority(values.authority, true); return values.authority;
  };
  const expect = (label: string, code: string | undefined, mutate?: Parameters<typeof buildAuthority>[0], after?: (authority: J) => void): void => { const authority = buildAuthority(mutate); after?.(authority); let observed = ""; try { validatePreActivationAuthority(authority, true); } catch (error) { observed = error instanceof GateH2Error ? error.code : "schema"; } codedAssert(observed !== "" && (code === undefined || observed === code), "H2_PREACTIVATION_TEST", `${label} expected ${code ?? "a rejection"}, observed ${observed || "none"}`); };
  expect("untrusted-local-fixture", "H2_PREACTIVATION_TRUST");
  expect("synthetic", undefined, ({ d1 }) => { d1.synthetic = true; });
  expect("placeholder", "H2_PREACTIVATION_PLACEHOLDER", ({ d1 }) => { d1.observation.principal = d1.signer.principal = "placeholder-principal"; d1.signature_base64 = crypto.sign(null, unsignedAttestationPayload(d1), keys[0].privateKey).toString("base64"); });
  ...
  expect("caller-selected-executor-capability", "H2_EXECUTOR_CAPABILITY_DERIVATION", ({ sandbox: candidate }) => { candidate.executor_semantics[0].capability_id = hash("caller-selected-executor-capability"); candidate.signature_base64 = crypto.sign(null, unsignedAttestationPayload(candidate), keys[1].privateKey).toString("base64"); });
  expect("missing", "H2_PREACTIVATION_MISSING", undefined, (authority) => { fs.unlinkSync(authority.pre_activation_attestations.d1.artifact.path); });
  const fixtureAccount = "preactivation-fixture-account"; const fixtureDatabase = "preactivation-fixture-database";
  const raceAuthority = buildAuthority(({ authority, d1: candidate }) => {
    candidate.account_capability_digest = hash(`gate-h2-d1-account-capability-v1\0${fixtureAccount}`); candidate.database_uuid_digest = hash(`gate-h2-d1-database-uuid-v1\0${fixtureDatabase}`); candidate.signature_base64 = crypto.sign(null, unsignedAttestationPayload(candidate), keys[0].privateKey).toString("base64");
    authority.stage_execution.ledger.account_capability_digest = candidate.account_capability_digest; authority.stage_execution.ledger.database_uuid_digest = candidate.database_uuid_digest;
  });
  ...
  const routeAliases = ["/opt/gate/b/../a", "/opt/gate//a", "/opt/gate/a/", "/opt/gate/./a", "\\opt\\gate\\a", "/opt/gate/%61", "/opt/gate/a\u0301", "/opt/gate/a\0alias", "opt/gate/a"];
  for (const [index, alias] of routeAliases.entries()) expectTrust(`logical-route-alias-${index}`, "H2_LOGICAL_ROUTE_CANONICAL", (candidate) => { candidate.attestors[0].logical_route = "/opt/gate/a"; candidate.attestors[1].logical_route = alias; });
  ...
  const cli = path.join(ROOT, "node_modules/.bin/tsx"); const scriptFile = fileURLToPath(import.meta.url); const capability = hash(`gate-h2-internal-two-process-v2\n${canon(raceAuthority)}`);
  const baseVerifierArgs = (outputReceipt: string) => [scriptFile, "pre-activation-verify", "--authority", raceAuthorityFile, "--d1-attestation", raceD1, "--sandbox-attestation", raceSandbox, "--max-age-seconds", "3600", "--signing-key", keyFile, "--verifier-principal", verifierIdentity.principal, "--verifier-session", verifierIdentity.session_id, "--verifier-route", verifierIdentity.route, "--output", outputReceipt];
  const expectTrustSubprocess = (label: string, mutate: (candidate: J) => void, code: string, validateAdmissions = false): void => {
    const candidate = structuredClone(trustValue); mutate(candidate); const candidateFile = path.join(root, `trust-${label}.json`); const outputReceipt = path.join(root, `trust-${label}-receipt.json`); fs.writeFileSync(candidateFile, pretty(candidate), { mode: 0o600 });
    let stderr = ""; try { execFileSync(cli, baseVerifierArgs(outputReceipt), { cwd: ROOT, env: { ...process.env, GATE_H2_INTERNAL_TEST_CAPABILITY: capability, GATE_H2_INTERNAL_PREACTIVATION_TRUST: candidateFile, GATE_H2_INTERNAL_D1_FIXTURE: fixtureFile, ...(validateAdmissions ? { GATE_H2_INTERNAL_VALIDATE_ADMISSIONS: "1" } : {}) }, stdio: ["ignore", "ignore", "pipe"] }); } catch (error) { stderr = (error as { stderr?: Buffer }).stderr?.toString("utf8") ?? ""; }
    codedAssert(stderr.includes(code) && !fs.existsSync(outputReceipt), "H2_PREACTIVATION_TRUST_TEST", `${label} subprocess did not reject ${code} without receipt: ${stderr.slice(0, 500)}`);
  };
  for (const field of ["principal", "session_id", "logical_route", "physical_route_identity_sha256", "public_key_sha256"]) expectTrustSubprocess(`same-${field}`, (candidate) => { candidate.attestors[1][field] = candidate.attestors[0][field]; }, "H2_PREACTIVATION_INDEPENDENCE");
  expectTrustSubprocess("wrong-kind", (candidate) => { candidate.attestors[1].evidence_kind = "d1_live"; }, "H2_PREACTIVATION_TRUST_KIND");
  ...
  const admissionRepository = path.join(root, "admission-git-repository"); fs.mkdirSync(admissionRepository);
  const git = trustedExecutable("git"); const gitRun = (args: string[], encoding?: BufferEncoding): string => execFileSync(git, args, { cwd: admissionRepository, env: {}, encoding: encoding ?? "utf8" }).trim();
  gitRun(["init", "--quiet"]); gitRun(["config", "user.name", "Gate H2 Test"]); gitRun(["config", "user.email", "gate-h2-test@example.invalid"]);
  const admissionNow = Date.parse(raceAuthority.authorized_at); const coordinatorTrust = raceAuthority.coordinator_trust;
  const admissionReviewerKeys: ReturnType<typeof crypto.generateKeyPairSync>[] = []; const admissionReceipts: J[] = []; const admissionEntries: J[] = [];
  for (let index = 0; index < PREACTIVATION_EVIDENCE_KINDS.length; index++) {
    const sourceEntry = structuredClone(trustValue.attestors[index]); delete sourceEntry.admission_commit; delete sourceEntry.admission_receipt;
    const reviewerKeys = crypto.generateKeyPairSync("ed25519"); admissionReviewerKeys.push(reviewerKeys);
    const reviewerPem = reviewerKeys.publicKey.export({ type: "spki", format: "pem" }).toString();
    const reviewerInventory = syntheticAwsInventory(`admission_reviewer_${index}`, { instance_id: `i-fedcba98765432${(index + 1).toString(16).padStart(2, "0")}`, identity_document_sha256: hash(`reviewer-iid-${index}`), identity_signature_pkcs7_sha256: hash(`reviewer-pkcs7-${index}`), pending_time: new Date(Date.parse(observedAt) - 10_000).toISOString() }); reviewerInventory.inventory_id = `admission_reviewer_host_${index}`; reviewerInventory.coordinator_signature_base64 = crypto.sign(null, inventorySigningPayload(reviewerInventory), SYNTHETIC_COORDINATOR_KEYS.privateKey).toString("base64");
    const reviewer = { principal: `admission-reviewer-${index}`, session_id: `admission-reviewer-session-${index}`, logical_route: `/review/gate-h2/admission-${index}`, physical_route_identity_sha256: hash(`reviewer-route-${index}`), public_key_sha256: hash(reviewerPem), public_key_pem: reviewerPem, surface_id: trustedSurfaceId(reviewerInventory), surface_inventory_digest: trustedInventoryDigest(reviewerInventory), canonical_physical_host_id: reviewerInventory.canonical_physical_host_id, trusted_surface_inventory: reviewerInventory };
    const receipt: J = { schema_version: "reviewed_metrics_pre_activation_attestor_admission_v2.1.0", status: "independently_reviewed_and_admitted", synthetic: false, evidence_kind: sourceEntry.evidence_kind, subject: admittedSubject(sourceEntry), reviewed_at: new Date(Date.parse(observedAt) - 6_000 + index * 1_000).toISOString(), validity_policy: PREACTIVATION_ADMISSION_VALIDITY_POLICY, reviewer, signature_base64: "" };
    receipt.signature_base64 = crypto.sign(null, admissionPayload(receipt), reviewerKeys.privateKey).toString("base64"); const raw = Buffer.from(pretty(receipt));
    const repositoryPath = `docs/dataset-factory/authorities/reviewed-metrics-v2/admissions/admission-${index}.json`; const absolutePath = path.join(admissionRepository, repositoryPath); fs.mkdirSync(path.dirname(absolutePath), { recursive: true }); fs.writeFileSync(absolutePath, raw);
    gitRun(["add", repositoryPath]); gitRun(["commit", "--quiet", "-m", `admission ${index}`]);
    const entry = { ...sourceEntry, admission_commit: gitRun(["rev-parse", "HEAD"]), admission_receipt: { path: repositoryPath, sha256: hash(raw), bytes: raw.length } };
    admissionReceipts.push(receipt); admissionEntries.push(entry);
  }
  const acceptedTrust = { schema_version: "reviewed_metrics_pre_activation_attestor_trust_v2.2.0", status: "reviewed_real_attestors_admitted", admission_validity_policy: PREACTIVATION_ADMISSION_VALIDITY_POLICY, attestors: admissionEntries };
  const registryPath = "docs/dataset-factory/authorities/reviewed-metrics-v2/pre-activation-attestor-trust-v2.json"; const registryAbsolute = path.join(admissionRepository, registryPath); fs.writeFileSync(registryAbsolute, pretty(acceptedTrust)); gitRun(["add", registryPath]); gitRun(["commit", "--quiet", "-m", "exact admission registry descendant"]);
  const authorityContext = (now = admissionNow) => internalAuthorityValidationContext(raceAuthority, admissionRepository, registryPath, now); validateAuthorityValue(raceAuthority, authorityContext());
  codedAssert(execFileSync(git, ["show", `HEAD:${registryPath}`], { cwd: admissionRepository, env: {} }).equals(Buffer.from(pretty(acceptedTrust))), "H2_PREACTIVATION_TRUST_TEST", "caller-level authority validation did not consume the exact five-identity registry descendant");
  let untrustedContextCode = ""; try { internalAuthorityValidationContext(structuredClone(raceAuthority), admissionRepository, registryPath, admissionNow); } catch (error) { untrustedContextCode = error instanceof GateH2Error ? error.code : ""; } codedAssert(untrustedContextCode === "H2_INTERNAL_CAPABILITY", "H2_PREACTIVATION_TRUST_TEST", "unregistered or tracked authority accepted injected validation context");
  const expectAuthorityRegistryReject = (label: string, candidate: J, code: string, now = admissionNow): void => { fs.writeFileSync(registryAbsolute, pretty(candidate)); gitRun(["add", registryPath]); gitRun(["commit", "--quiet", "-m", label]); let observed = ""; try { validateAuthorityValue(raceAuthority, authorityContext(now)); } catch (error) { observed = error instanceof GateH2Error ? error.code : "schema"; } codedAssert(observed === code, "H2_PREACTIVATION_TRUST_TEST", `${label} caller-level authority path expected ${code}, observed ${observed || "none"}`); };
  const alteredReceiptPin = structuredClone(acceptedTrust); alteredReceiptPin.attestors[0].admission_receipt.sha256 = hash("altered-receipt"); expectAuthorityRegistryReject("git-dag-altered-receipt", alteredReceiptPin, "H2_PREACTIVATION_ADMISSION");
  const alteredCommit = structuredClone(acceptedTrust); alteredCommit.attestors[0].admission_commit = "f".repeat(40); expectAuthorityRegistryReject("git-dag-altered-commit", alteredCommit, "H2_PREACTIVATION_ADMISSION_COMMIT");
  const alteredRoute = structuredClone(acceptedTrust); alteredRoute.attestors[1].logical_route = `${alteredRoute.attestors[0].logical_route}/../${path.posix.basename(alteredRoute.attestors[1].logical_route)}`; expectAuthorityRegistryReject("git-dag-altered-route-alias", alteredRoute, "H2_LOGICAL_ROUTE_CANONICAL");
  const resignAdmissionAt = (index: number, reviewedAt: string): { receipt: J; raw: Buffer } => { const receipt = structuredClone(admissionReceipts[index]); receipt.reviewed_at = reviewedAt; receipt.signature_base64 = crypto.sign(null, admissionPayload(receipt), admissionReviewerKeys[index].privateKey).toString("base64"); return { receipt, raw: Buffer.from(pretty(receipt)) }; };
  const expectSignedTime = (label: string, reviewedAt: string, now: number, accepted: boolean): void => { const candidate = resignAdmissionAt(0, reviewedAt); let observed = ""; try { validateAdmissionReceipt(candidate.receipt, admissionEntries[0], candidate.raw, coordinatorTrust, now); } catch (error) { observed = error instanceof GateH2Error ? error.code : "schema"; } codedAssert(accepted ? observed === "" : observed === "H2_PREACTIVATION_ADMISSION_FRESHNESS", "H2_PREACTIVATION_TRUST_TEST", `${label} signed admission boundary result was ${observed || "accepted"}`); };
  const exactBoundaryReview = new Date(Date.parse(observedAt) - 10_000).toISOString(); const exactBoundaryNow = Date.parse(exactBoundaryReview) + PREACTIVATION_ADMISSION_VALIDITY_POLICY.admission_max_age_seconds * 1_000;
  expectSignedTime("signed-exact-max-age-boundary", exactBoundaryReview, exactBoundaryNow, true);
  expectSignedTime("signed-stale-over-boundary", exactBoundaryReview, exactBoundaryNow + 1, false);
  expectSignedTime("signed-exact-future-skew-boundary", new Date(admissionNow + PREACTIVATION_ADMISSION_VALIDITY_POLICY.clock_skew_seconds * 1000).toISOString(), admissionNow, true);
  expectSignedTime("signed-future-over-boundary", new Date(admissionNow + PREACTIVATION_ADMISSION_VALIDITY_POLICY.clock_skew_seconds * 1000 + 1).toISOString(), admissionNow, false);
  const staleCommitted = resignAdmissionAt(0, exactBoundaryReview); const staleReceiptPath = admissionEntries[0].admission_receipt.path; fs.writeFileSync(path.join(admissionRepository, staleReceiptPath), staleCommitted.raw); gitRun(["add", staleReceiptPath]); gitRun(["commit", "--quiet", "-m", "adversarial stale signed admission"]);
  const staleCommittedTrust = structuredClone(acceptedTrust); staleCommittedTrust.attestors[0].admission_commit = gitRun(["rev-parse", "HEAD"]); staleCommittedTrust.attestors[0].admission_receipt = { path: staleReceiptPath, sha256: hash(staleCommitted.raw), bytes: staleCommitted.raw.length };
  expectAuthorityRegistryReject("git-dag-altered-signed-time", staleCommittedTrust, "H2_PREACTIVATION_ADMISSION_FRESHNESS", exactBoundaryNow + 1);
  ...
  const childStatus = (child: ReturnType<typeof spawn>): string | undefined => child.exitCode !== null || child.signalCode !== null ? `code=${child.exitCode} signal=${child.signalCode}` : undefined;
  type TrackedChildClose = { observed: boolean; promise: Promise<string> }; const trackedChildCloses = new WeakMap<ReturnType<typeof spawn>, TrackedChildClose>();
  const trackChildClose = <T extends ReturnType<typeof spawn>>(child: T): T => { const tracked: TrackedChildClose = { observed: false, promise: undefined as unknown as Promise<string> }; tracked.promise = new Promise<string>((resolve) => { child.once("close", (code, signal) => { tracked.observed = true; resolve(`code=${code} signal=${signal}`); }); child.once("error", (error) => { if (child.pid === undefined) { tracked.observed = true; resolve(`spawn_error=${error.message}`); } }); }); trackedChildCloses.set(child, tracked); return child; };
  const awaitChildClose = async (child: ReturnType<typeof spawn>, timeoutMs: number): Promise<string> => { const tracked = trackedChildCloses.get(child) ?? (trackChildClose(child), trackedChildCloses.get(child)!); let timer: NodeJS.Timeout | undefined; try { return await Promise.race([tracked.promise, new Promise<string>((_resolve, reject) => { timer = setTimeout(() => reject(new GateH2Error("H2_PREACTIVATION_RACE_TEST", `timed out after ${timeoutMs}ms waiting for mandatory child close; ${childStatus(child) ?? "still running"}`)), timeoutMs); })]); } finally { if (timer) clearTimeout(timer); } };
  const terminateAndReapChild = async (child: ReturnType<typeof spawn>, graceMs = 500): Promise<string> => {
    const tracked = trackedChildCloses.get(child) ?? (trackChildClose(child), trackedChildCloses.get(child)!);
    if (!childStatus(child) && !tracked.observed) { child.kill("SIGTERM"); let graceTimer: NodeJS.Timeout | undefined; try { await Promise.race([tracked.promise, new Promise<void>((resolve) => { graceTimer = setTimeout(resolve, graceMs); })]); } finally { if (graceTimer) clearTimeout(graceTimer); } if (!childStatus(child) && !tracked.observed) child.kill("SIGKILL"); }
    return await awaitChildClose(child, Math.max(10_000, graceMs + 5_000));
  };
  const waitForChildMarker = async (child: ReturnType<typeof spawn>, marker: string, output: () => string, timeoutMs = 15_000): Promise<void> => {
    ...
    await new Promise<void>((resolve, reject) => {
      let settled = false; let timer: NodeJS.Timeout;
      const cleanup = () => { clearInterval(timer); child.off("error", onError); child.off("close", onClose); };
      const fail = (message: string) => { if (settled) return; settled = true; cleanup(); reject(new GateH2Error("H2_PREACTIVATION_RACE_TEST", `${message}; output=${output().slice(0, 1000)}`)); };
      ...
      const check = () => {
        if (settled) return;
        const observed = childStatus(child); if (observed) { fail(`child exited before marker: ${observed}`); return; }
        if (fs.existsSync(marker)) { settled = true; cleanup(); resolve(); return; }
        if (process.hrtime.bigint() >= deadline) {
          settled = true; cleanup();
          void terminateAndReapChild(child).then((status) => reject(new GateH2Error("H2_PREACTIVATION_RACE_TEST", `timed out after ${timeoutMs}ms waiting for child marker; reaped ${status}; output=${output().slice(0, 1000)}`)), (error) => reject(error));
        }
      };
      const beforeListeners = childStatus(child); if (beforeListeners) { fail(`child exited before marker: ${beforeListeners}`); return; }
      timer = setInterval(check, 20); child.once("error", onError); child.once("close", onClose); check();
    });
  };
  const early = trackChildClose(spawn(process.execPath, ["-e", "process.stdout.write('early-out');process.stderr.write('early-err');process.exit(23)"], { stdio: ["ignore", "pipe", "pipe"] }));
  ...
  await terminateAndReapChild(early);
  const inheritedDescriptorStart = process.hrtime.bigint(); const inheritedDescriptorChild = trackChildClose(spawn(process.execPath, ["-e", "const{spawn}=require('child_process');spawn(process.execPath,['-e','setTimeout(()=>{},1700)'],{stdio:['ignore',process.stdout,process.stderr]});process.exit(29)"], { stdio: ["ignore", "pipe", "pipe"] })); inheritedDescriptorChild.stdout.resume(); inheritedDescriptorChild.stderr.resume();
  while (childStatus(inheritedDescriptorChild) === undefined) await new Promise((resolve) => setTimeout(resolve, 5));
  const inheritedDescriptorStatus = await terminateAndReapChild(inheritedDescriptorChild); const inheritedDescriptorElapsedMs = Number(process.hrtime.bigint() - inheritedDescriptorStart) / 1_000_000;
  codedAssert(inheritedDescriptorStatus.includes("code=29") && trackedChildCloses.get(inheritedDescriptorChild)?.observed === true && inheritedDescriptorElapsedMs >= 1_500, "H2_PREACTIVATION_RACE_TEST", `fixture restoration did not wait for inherited descriptor close: status=${inheritedDescriptorStatus} elapsed_ms=${inheritedDescriptorElapsedMs}`);
  const timeoutTemp = path.join(root, "sigterm-ignoring-child.tmp");
  const timeoutChild = trackChildClose(spawn(process.execPath, ["-e", `const fs=require('fs');fs.writeFileSync(${JSON.stringify(timeoutTemp)},String(process.pid));process.stdout.write('timeout-child-ready');process.on('SIGTERM',()=>process.stderr.write('ignored-sigterm'));setInterval(()=>{},1000)`], { stdio: ["ignore", "pipe", "pipe"] }));
  const timeoutPid = timeoutChild.pid; let timeoutOutput = ""; timeoutChild.stdout.on("data", (chunk) => { timeoutOutput += chunk.toString(); }); timeoutChild.stderr.on("data", (chunk) => { timeoutOutput += chunk.toString(); });
  while (!fs.existsSync(timeoutTemp)) await new Promise((resolve) => setTimeout(resolve, 10));
  let timeoutDiagnostic = ""; try { await waitForChildMarker(timeoutChild, path.join(root, "never-created-timeout-marker"), () => timeoutOutput, 5_000); } catch (error) { timeoutDiagnostic = error instanceof Error ? error.message : String(error); }
  let timeoutProcessAlive = false; if (timeoutPid !== undefined) try { process.kill(timeoutPid, 0); timeoutProcessAlive = true; } catch (error) { codedAssert((error as NodeJS.ErrnoException).code === "ESRCH", "H2_PREACTIVATION_RACE_TEST", `unexpected timeout child process probe error: ${String(error)}`); }
  fs.rmSync(timeoutTemp, { force: true });
  codedAssert(timeoutDiagnostic.includes("timed out after 5000ms") && timeoutDiagnostic.includes("signal=SIGKILL") && timeoutDiagnostic.includes("timeout-child-ready") && timeoutDiagnostic.includes("ignored-sigterm") && timeoutChild.signalCode === "SIGKILL" && !timeoutProcessAlive && !fs.existsSync(timeoutTemp), "H2_PREACTIVATION_RACE_TEST", `timeout child was not killed, reaped, diagnosed, and cleaned before fixture restoration: ${timeoutDiagnostic}`);
  const race = async (label: string, target: string, mutate: (file: string, raw: Buffer, backup: string) => void, expectedCode: string, startupDelayMs = 0): Promise<void> => {
    ...
    try {
      ...
      const closeStatus = await awaitChildClose(child, 10_000);
      codedAssert(!closeStatus.includes("code=0 ") && stderr.includes(expectedCode) && !fs.existsSync(outputReceipt), "H2_PREACTIVATION_RACE_TEST", `${label} did not reject ${expectedCode} without a receipt: status=${closeStatus} stdout=${stdout.slice(0, 300)} stderr=${stderr.slice(0, 700)}`);
    } finally {
      await terminateAndReapChild(child, 1_000);
  ...
  return { status: "pre_activation_fail_closed_contract_passed", production_ready: false, real_attestors_admitted: false, cases: ["missing", "synthetic", "stale", "self_authored", "executor_attestor_authority_role_overlap", "placeholder", "unsigned", "wrong_principal_session_route", "mismatched_evidence", "mismatched_executor_capability_receipt", "caller_selected_executor_capability", "empty_trust_registry", "canonical_signature_encoding_matrix", "five_role_route_host_key_independence", "executor_admission_omission_and_inventory_substitution", "distinct_claimed_keys_on_authority_route_and_surface", "caller_level_exact_five_admission_git_dag", "caller_level_pin_commit_route_time_tamper_rejection", "internal_validation_context_rejects_unregistered_authority", "subprocess_pairwise_principal_session_logical_physical_route_key", "subprocess_wrong_kind", "subprocess_nonexistent_or_nonprior_commit", "subprocess_correctly_signed_untrusted_artifact", "subprocess_child_early_exit_diagnostics", "subprocess_inherited_descriptor_close_before_restoration", "subprocess_timeout_kill_reap_diagnostics", "subprocess_delayed_start_beyond_one_second", "subprocess_rename_race", "subprocess_symlink_race", "subprocess_same_size_race", "subprocess_valid_alternate_race"] };
}
...
async function main(): Promise<void> {
  ...
  const parsed = parseArgs({
    ...
    options: {
      ...
      "d1-attestation": { type: "string" },
      "sandbox-attestation": { type: "string" },
  ...
  const internalAuthority = (() => {
    ...
    INTERNAL_SYNTHETIC_AUTHORITIES.add(candidate);
    internalSyntheticProcessAuthorized = true;
    return candidate;
  })();
  ...
  if (command === "measure-route") {
    ...
    const keyFile = path.resolve(assertString(o["signing-key"], "--signing-key required"));
    const output = physicalPathSafety(path.resolve(assertString(o.output, "--output required")));
    codedAssert(!fs.existsSync(output), "H2_PREACTIVATION_VERIFICATION", "pre-activation receipt output must be absent");
    const retained: (FileSnapshot | RawSnapshot)[] = [];
    try {
      ...
      const internalNow = internalCapability && process.env.GATE_H2_INTERNAL_NOW ? Date.parse(process.env.GATE_H2_INTERNAL_NOW) : undefined;
      codedAssert(internalNow === undefined || Number.isFinite(internalNow), "H2_PREACTIVATION_ADMISSION_FRESHNESS", "internal controlled now must be canonical date-time");
      const now = internalNow ?? Date.now();
      const admissions = internalTrustFile && process.env.GATE_H2_INTERNAL_VALIDATE_ADMISSIONS !== "1" ? (validatePreActivationTrustIdentitySet(trust), new Map<string, J>()) : validatePreActivationTrustRegistry(trust, authority.coordinator_trust, { now });
      ...
      for (const attestation of [d1, sandbox]) codedAssert(Date.parse(attestation.observation.observed_at) <= now && now - Date.parse(attestation.observation.observed_at) <= maxAgeSeconds * 1000, "H2_PREACTIVATION_STALE", "pre-activation attestation is stale or future-dated");
      const privateKey = crypto.createPrivateKey(keySnapshot.raw);
      codedAssert(privateKey.asymmetricKeyType === "ed25519", "H2_PREACTIVATION_VERIFICATION", "pre-activation verifier key must be Ed25519");
      const publicKeyPem = crypto.createPublicKey(privateKey).export({ type: "spki", format: "pem" }).toString();
      const verifierRoute = canonicalLogicalRoute(assertString(o["verifier-route"], "--verifier-route required"));
      ...
      codedAssert(internalCapability === undefined, "H2_PREACTIVATION_SYNTHETIC", "internal pre-activation fixtures can exercise retention and D1 orchestration but can never issue an activation receipt");
      const receipt: J = { schema_version: "reviewed_metrics_pre_activation_verification_v2.2.0", status: "live_prerequisites_verified", synthetic: false, d1_attestation: { sha256: hash(d1Snapshot.raw), bytes: d1Snapshot.raw.length }, sandbox_attestation: { sha256: hash(sandboxSnapshot.raw), bytes: sandboxSnapshot.raw.length }, live_d1_schema_sha256: d1.deployed_schema.canonical_schema_sha256, executor_capability_ids: sandbox.executor_semantics.map((bundle: J) => bundle.capability_id).sort(), admission_validity_policy: PREACTIVATION_ADMISSION_VALIDITY_POLICY, verified_at: new Date(now).toISOString(), verifier, signature_base64: "" };
      rejectPlaceholderEvidence(receipt); receipt.signature_base64 = crypto.sign(null, unsignedAttestationPayload(receipt), privateKey).toString("base64"); schema("pre-activation-verification.schema.v2.json", receipt);
      for (const snapshot of retained) assertSnapshotPathUnchanged(snapshot);
      writeOwnedExclusive(output, Buffer.from(pretty(receipt)), "H2_PREACTIVATION_VERIFICATION");
      result = { status: "pre_activation_live_attestations_verified", authority_activation_prerequisites_verified: true, verification_sha256: hash(Buffer.from(pretty(receipt))), d1_attestation_sha256: receipt.d1_attestation.sha256, sandbox_attestation_sha256: receipt.sandbox_attestation.sha256 };
    } catch (error) {
      throw error;
    } finally { for (const snapshot of retained) { try { closeSnapshot(snapshot); } catch { /* descriptors are no longer authority inputs */ } } }
    ...
    const authority = internalAuthority ?? executionAuthority();
    const fixtureLedger = internalCapability && process.env.GATE_H2_INTERNAL_D1_FIXTURE ? internalD1FixtureAdapter(authority, path.resolve(process.env.GATE_H2_INTERNAL_D1_FIXTURE), internalCapability) : undefined;
    const evidence = await completedStageLedgerEvidence(authority, true, internalCapability, o["signing-key"] ? path.resolve(o["signing-key"]) : undefined, fixtureLedger);
    const output = physicalPathSafety(path.resolve(assertString(o.output, "--output required")));
    ...
    result = await runAttestedStage("publication_assembly_plan", () => {
      const authority = internalAuthority ?? executionAuthority();
      if (internalCapability !== undefined) {
        const entry = stageManifestEntry(authority, "publication_assembly_plan");
        executeSyntheticExternalStageOperation(entry.operation, entry.outputs);
      } else executeReviewedPublicationAssemblyPlanExternalOperation(authority);
      const planFile = stageManifestEntry(authority, "publication_assembly_plan").outputs[0].path;
      validatePublicationAssemblyPlan(load(planFile), path.dirname(planFile), authority, [...PUBLICATION_PRE_PLAN_PATHS, "publication-assembly-plan-v2.json"]);
      return { status: "publication_assembly_plan_completed" };
    });
  else if (command === "assemble-publication") {
    const authority = internalAuthority ?? executionAuthority();
    const raceFile = internalCapability ? process.env.GATE_H2_INTERNAL_ASSEMBLY_RACE_FILE : undefined;
    const raceSpec = internalCapability ? process.env.GATE_H2_INTERNAL_ASSEMBLY_RACE_SPEC : undefined;
    const workspace = path.resolve(assertString(o.workspace, "--workspace required"));
