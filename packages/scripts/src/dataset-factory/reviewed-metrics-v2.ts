...
function canonicalCommittedHeadAuthorityBytes(): Buffer {
  const head = execFileSync("git", ["cat-file", "blob", `HEAD:${CANONICAL_AUTHORITY_REL}`], { cwd: ROOT, stdio: ["ignore", "pipe", "pipe"] });
  assert(head.length === TRACKED_AUTHORITY_BYTES && hash(head) === TRACKED_AUTHORITY_SHA256, "canonical input authority committed HEAD blob exact bytes drift");
  const worktree = fs.readFileSync(CANONICAL_AUTHORITY);
  assert(worktree.equals(head), "canonical input authority worktree bytes must equal the committed HEAD blob");
  const value = JSON.parse(head.toString("utf8")); validateInputAuthorityValue(value);
  return head;
}
...
function validateMatrixValue(value: J, baseDir = ROOT, suppliedAuthority?: J): void {
  ...
  const matrixAuthorityPin = byRole.get("input_authority"); const canonicalAuthorityRaw = canonicalCommittedHeadAuthorityBytes(); assert(matrixAuthorityPin.sha256 === TRACKED_AUTHORITY_SHA256 && matrixAuthorityPin.bytes === TRACKED_AUTHORITY_BYTES, "final matrix canonical input authority committed-file pin"); const matrixAuthorityRaw = fs.readFileSync(resolveEvidencePath(matrixAuthorityPin.path, baseDir)); assert(matrixAuthorityRaw.equals(canonicalAuthorityRaw), "final matrix input authority must be canonical committed HEAD bytes"); validateInputAuthorityValue(JSON.parse(matrixAuthorityRaw.toString("utf8")));
...
async function selfTest(): Promise<J> {
  ...
  const canonicalWorkingAuthority = fs.readFileSync(CANONICAL_AUTHORITY); const finalWorkingAuthority = fs.readFileSync(path.join(finalDir, "input-authority-v2.json")); const finalWorkingMatrix = fs.readFileSync(finalMatrixFile);
  try {
    const uncommittedAuthority = JSON.parse(canonicalWorkingAuthority.toString("utf8")); uncommittedAuthority.created_at = "2026-07-15T00:00:00.001Z"; const uncommittedAuthorityRaw = Buffer.from(pretty(uncommittedAuthority));
    fs.writeFileSync(CANONICAL_AUTHORITY, uncommittedAuthorityRaw); fs.writeFileSync(path.join(finalDir, "input-authority-v2.json"), uncommittedAuthorityRaw);
    const coordinatedUncommittedMatrix = structuredClone(completedMatrix); for (const row of coordinatedUncommittedMatrix.rows) for (const evidence of row.evidence) if (evidence.role === "input_authority") { evidence.sha256 = hash(uncommittedAuthorityRaw); evidence.bytes = uncommittedAuthorityRaw.length; } writeJson(finalMatrixFile, coordinatedUncommittedMatrix);
    const coordinatedUncommittedPublication = makePublication(); await reject(() => validateMatrixValue(coordinatedUncommittedMatrix, finalDir, syntheticAuth)); await reject(() => validatePublicationValue(coordinatedUncommittedPublication, finalDir, syntheticAuth));
  } finally {
    fs.writeFileSync(CANONICAL_AUTHORITY, canonicalWorkingAuthority); fs.writeFileSync(path.join(finalDir, "input-authority-v2.json"), finalWorkingAuthority); fs.writeFileSync(finalMatrixFile, finalWorkingMatrix);
  }
  assert(fs.readFileSync(CANONICAL_AUTHORITY).equals(canonicalWorkingAuthority) && fs.readFileSync(path.join(finalDir, "input-authority-v2.json")).equals(finalWorkingAuthority) && fs.readFileSync(finalMatrixFile).equals(finalWorkingMatrix), "canonical authority adversarial restoration drift");
  await accept(() => validateMatrixValue(completedMatrix, finalDir, syntheticAuth)); await accept(() => validatePublicationValue(completedPublication, finalDir, syntheticAuth));
