...
function executePublicationAssemblyPlanExternalOperation(authority: J, reviewedTrust: J, authorityContext: ExternalOperationAuthorityContext): ReturnType<typeof externalOperationPreflight> {
  ...
  codedAssert(authorityContext.operationSha256 === hash(Buffer.from(canon(entry.operation))) && authorityContext.reviewedTrustSha256 === hash(Buffer.from(pretty(reviewedTrust))) && canon(authorityContext.reviewedTrust) === canon(reviewedTrust), "H2_STAGE_AUTHORITY_CONTEXT", "publication assembly operation digest or reviewed trust snapshot was substituted");
...
export async function externalOperationContractSelfTest(): Promise<J> {
  ...
  try {
    ...
    const publicationContext = externalOperationAuthorityContext(successorAuthority, "publication_assembly_plan", successorTrust);
    expect("H2_STAGE_AUTHORITY_CONTEXT", () => executePublicationAssemblyPlanExternalOperation(successorAuthority, successorTrust, undefined as unknown as ExternalOperationAuthorityContext));
    ...
    expect("H2_STAGE_AUTHORITY_CONTEXT", () => executePublicationAssemblyPlanExternalOperation(successorAuthority, successorTrust, substitutedPublicationContext));
    const substitutedOperationContext = { ...publicationContext, operationSha256: hash("substituted-publication-operation") };
