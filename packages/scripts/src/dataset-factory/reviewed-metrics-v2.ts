...
function validateLinuxSandboxAttestation(value: J): void {
  validateSignedAttestation(value, "linux-sandbox-attestation.schema.v2.json");
...
function deriveNpmPackageProof(packageManager: J, members: J[], rawFor: (realpath: string) => Buffer, production: boolean): J {
  ...
  const root = lock.packages[""]; codedAssert(root, "H2_STAGE_PACKAGE_LOCK", "lock root package is missing"); exactKeys(root, ["name", "version", "dependencies"], "lock root package"); codedAssert(root.name === manifest.name && root.version === manifest.version && canon(root.dependencies ?? {}) === canon(rootDependencies), "H2_STAGE_PACKAGE_LOCK", "lock root package does not mirror retained package.json name/version/dependencies");
...
function validateProductionStageProgram(raw: Buffer, operation: J): J {
  ...
  codedAssert(value.schema_sha256 === schemaFilePin("stage-program.schema.v2.json").sha256 && operation.execution_capability_ids.length === 1 && value.capability_id === operation.execution_capability_ids[0] && canon(value.input_artifact_roles) === canon(operation.artifacts.map((artifact: J) => artifact.artifact_role)) && canon(value.output_indexes) === canon(operation.declared_output_paths.map((_file: string, index: number) => index)) && canon(value.network_capability_ids) === canon(operation.network_capability_ids), "H2_STAGE_PROGRAM_GRAMMAR", "stage program schema/roles/capabilities do not exactly join the authority operation");
  return value;
}
...
function externalOperationPreflight(operation: J, outputs: J[], syntheticBoundary = false): ExternalPreflight {
  ...
  codedAssert(Array.isArray(operation.execution_capability_ids) && operation.execution_capability_ids.length === 1 && /^[a-f0-9]{64}$/.test(operation.execution_capability_ids[0]), "H2_STAGE_PROGRAM_GRAMMAR", "external operation must pin exactly one stage executor capability");
...
function externalOperationContractSelfTest(): J {
  ...
  try {
    ...
    const stageProgram = { schema_version: "reviewed_metrics_stage_program_v2.0.0", schema_sha256: schemaFilePin("stage-program.schema.v2.json").sha256, executor_contract: "gate_h2_static_stage_program_executor_v1", action: "invoke_attested_capability", capability_id: hash("synthetic-stage-capability"), input_artifact_roles: ["sanitized_prompt"], output_indexes: [0], network_capability_ids: [] }; validateProductionStageProgram(Buffer.from(pretty(stageProgram)), operation);
...
function validatePreActivationAuthority(authority: J): void {
  ...
  validateD1LiveAttestation(d1); validateLinuxSandboxAttestation(sandbox);
  ...
  codedAssert(verification.d1_attestation.sha256 === pins.d1.artifact.sha256 && verification.d1_attestation.bytes === pins.d1.artifact.bytes && verification.sandbox_attestation.sha256 === pins.sandbox.artifact.sha256 && verification.sandbox_attestation.bytes === pins.sandbox.artifact.bytes && verification.live_d1_schema_sha256 === d1.deployed_schema.canonical_schema_sha256, "H2_PREACTIVATION_VERIFICATION", "pre-activation receipt does not bind exact attestations and live D1 schema");
...
`, { mode: 0o600 }); fs.writeFileSync(wrapper, syntheticInProcessWrapperSource(), { mode: 0o500 }); fs.writeFileSync(prompt, "opaque input\n", { mode: 0o600 }); fs.writeFileSync(packageManifest, pretty({ name: "stage-run-test", version: "1.0.0", private: true, dependencies: { "stage-runtime": "1.0.0" } }), { mode: 0o600 }); fs.writeFileSync(lock, pretty({ name: "stage-run-test", version: "1.0.0", lockfileVersion: 3, requires: true, packages: { "": { name: "stage-run-test", version: "1.0.0", dependencies: { "stage-runtime": "1.0.0" } }, "node_modules/stage-runtime": { name: "stage-runtime", version: "1.0.0", resolved: "file:synthetic-fixture-only" } } }), { mode: 0o600 });
  ...
  const operation = { kind: "external_command", execution_boundary: { ...boundaryPayload, attestation_sha256: hash(canon(boundaryPayload)) }, runtime: filePin(process.execPath, process.version), script: filePin(script), artifacts: [{ artifact_role: "sanitized_prompt", pin: filePin(prompt) }], dependency_lock: filePin(lock), installed_dependency_tree: filePin(dependencies), repo_tree_sha256: repoTree, cwd: fs.realpathSync(work), argv: [fs.realpathSync(script), fs.realpathSync(prompt)], argv_bindings: [{ index: 0, artifact_role: "script", value_sha256: hash(fs.realpathSync(script)) }, { index: 1, artifact_role: "sanitized_prompt", value_sha256: hash(fs.realpathSync(prompt)) }], declared_output_paths: [output], execution_capability_ids: [hash("synthetic-stage-capability")], network_policy: "deny_all", network_capability_ids: [], environment: { public: [{ name: "OUT", value: output, value_sha256: hash(output), binding: "declared_output" }], secret_capability_ids: [] } };
