...
const REVIEWED_CANDIDATE_COMMIT = "8eb42713059622a5c94a092da5e5a2141463c85f";
...
function executionAuthority(injected?: J, capability?: InternalSyntheticCapability, reader: ExecutionAuthorityReader = gitExecutionAuthorityEvidence, injectedClock?: Clock): J {
  ...
  if (injected !== undefined) { assert(capability?.[INTERNAL_SYNTHETIC_CAPABILITY] === true, "internal execution authority injection capability required"); value = injected; }
    ...
    assert(evidence.parents.length === 1 && evidence.parents[0] === REVIEWED_CANDIDATE_COMMIT, "prediction freeze unavailable: authority HEAD is not the direct child of reviewed candidate HEAD");
...
function validateSearchTaskValue(value: J, gold?: J): void { schema("search-task.schema.v2.json", value); assert(value.status === "published" && value.review_state === "accepted" && value.internal_provenance && value.public_projection && value.private_expected_commitment && value.rights_policy && value.component && value.split, "completed matrix/task review requires an authorized non-placeholder search task"); if (gold) { same(value.private_expected_commitment, gold.private_expected_commitment, "search task private commitment authority"); assert(gold.reviews.flatMap((row: J) => row.place_opportunities).some((place: J) => place.supported && place.official_url), "issue #97 requires at least one reviewed source-search place opportunity"); } }
...
function validatePublicationValue(value: J, baseDir = ROOT, authority?: J): void {
  ...
  assert(authority, "published descriptor requires committed execution authority"); validateAuthorityValue(authority); assert(value.candidate_commit === REVIEWED_CANDIDATE_COMMIT && value.artifact_id === PUBLICATION_ID, "published descriptor identity"); exactSet(pins.map((x: J) => x.path), PUBLICATION_MEMBER_PATHS, "publication exact member filenames"); same(value.required_publication_members, PUBLICATION_MEMBER_PATHS, "publication required member order"); pins.forEach((member: J) => verifyFilePin(member, baseDir)); assert(value.counts.published_members === pins.length + 1 && value.counts.unique_sources === 44 && value.counts.task_memberships === 78, "publication member/count arithmetic");
...
async function selfTest(): Promise<J> {
  ...
  const authorityBytes = Buffer.from(pretty(syntheticExecutionAuthority("a".repeat(64)))); const authorityEvidence = (overrides: Partial<ExecutionAuthorityEvidence> = {}): ExecutionAuthorityEvidence => ({ head: "d".repeat(40), parents: [REVIEWED_CANDIDATE_COMMIT], changedPaths: [EXECUTION_AUTHORITY_REL], repositoryClean: true, tracked: true, headBytes: authorityBytes, indexBytes: authorityBytes, worktreeBytes: authorityBytes, indexClean: true, worktreeClean: true, ...overrides } as ExecutionAuthorityEvidence);
  ...
  const syntheticTask = { schema_version: "reviewed_metrics_search_task_v2.0.0", status: "published", candidate_id: CANDIDATE_ID, internal_provenance: { internal_task_id: "synthetic-task", accepted_claim_id: "synthetic-claim", promotion_ledger: { sha256: "1".repeat(64), bytes: 1 }, review_receipt: { sha256: "2".repeat(64), bytes: 1 }, source_representation: { sha256: "3".repeat(64), bytes: 1 }, no_pixel_identity_claim: true }, public_projection: { prompt: "Synthetic source-search task", official_origins: ["https://example.invalid/"], output_fields: ["place"], scoring_version: "synthetic-v1" }, source_only_boundary: true, private_expected_commitment: finalGold.private_expected_commitment, rights_policy: { citation_only: true, private_body_redistribution_allowed: false, bounded_paraphrase_required: true }, component: "synthetic-component", split: "test", review_state: "accepted" }; const taskFile = path.join(finalDir, "search-task-v2.json"); writeJson(taskFile, syntheticTask);
