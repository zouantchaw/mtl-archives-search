...
async function writeCandidate(root: string, mediaRoot: string, sourceRoot: string): Promise<Json> { assert(!fs.existsSync(root), 'immutable candidate rebuild/overwrite refused'); const values = await candidateArtifacts(mediaRoot, sourceRoot); fs.mkdirSync(root, { recursive: false }); for (const [name, value] of Object.entries(values)) writeJson(path.join(root, name), value); writeJson(path.join(root, 'descriptor-v1.json'), descriptor(root)); return verify(root, { mediaRoot, sourceRoot, registryPath: null }); }
