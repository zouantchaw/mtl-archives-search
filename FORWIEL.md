...
The important engineering choice is to fail closed where this Mac cannot supply evidence. Local tests use a scripted transport to stress ordering, replay, DNS policy, peer pinning, TLS observations, redirects, compression, status/type limits, overflow, timeouts, malformed framing, auth insertion, redaction, and cleanup without touching a network. The actual TLS constructor remains disabled rather than using `curl`, a generic proxy, or custom cryptography. A Linux-only script describes the two clean static builds and two OCI exports required next; it refuses to run here and therefore cannot accidentally turn a macOS artifact into a reproducibility claim. See `crates/gate-h2-broker/README.md` for the exact implemented and retained boundaries.
