...
Reviewed Metrics v2 is deliberately still a sealed candidate, not a completed evaluation. A future authority-only commit may unlock one prediction only if its authority declares that commit's sole parent as `candidate_commit`, changes only the fixed authority path, is otherwise clean, and carries byte-identical authority in `HEAD`, index, and worktree during a live UTC window. A separate authority review must confirm that sole parent is the approved candidate HEAD. Completed metrics are recomputed from exact frozen prediction/gold bytes; final matrix/publication also byte-bind their input authority to the canonical tracked file. Issue #97 can close the place gate only with the exact accepted `gate-h-v2:official-source-search:c0-rpcq` task, Gate E/RPCQ provenance, matching gold place/dossier, frozen prediction, passing private score, and independent accepted task review.
