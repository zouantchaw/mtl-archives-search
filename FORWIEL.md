...
Issue #100 is now merged and complete, but it is deliberately an inactive synthetic contract rather than proof of a real container environment. Issue #101 Packet 1 freezes a 69-case Linux/Podman/D1/admission checklist and its 71-file registered synthetic fixture, `dfv0_gate_h2_linux_conformance_v1_20260806`. It binds that checklist to the exact #100 predecessor commit, artifact, descriptors, and source allowlist, so later evidence cannot quietly prove a different system. Run `npm run gate-h2:linux-conformance-fixture-verify` and `npm run gate-h2:linux-conformance-self-test` to check the 69 cases and 39 exact-code adversarial rejections. Normal verification accepts only the synthetic/nonproduction triad. The tracked synthetic fixture under `--strict-production` fails early with `H2_LINUX_CONFORMANCE_STRICT_SYNTHETIC`. A future bundle with the required non-synthetic strict shape and metadata must still fail `H2_LINUX_CONFORMANCE_STRICT_SEMANTICS` until real per-case Linux semantic validators are implemented. No paid host, Podman run, D1 action, credential, provider/model call, activation, publication, or deploy has happened. The next build step is a pinned hermetic builder recipe and a cross-surface receipt comparator; real Linux/D1 execution needs explicit approval.
...
Trusted host orchestration exports only an exact mode/path allowlist, hashes its manifest and exact crate lockfile, and independently derives the expected Cargo SBOM before giving the no-Git builder source, trust roots, an empty candidate directory, and a dedicated executable build tmpfs while unrelated `/tmp` stays `noexec`. The pinned builder compiles each admission/publication helper twice and emits pass-specific artifacts plus an exact digest manifest. Host code never compiles helpers: it verifies all three copies, unlinks the executable path on Linux, retained-FD executes with a strict environment, and keeps admission plus publication in one process with one unlinked admission descriptor FD, one retained destination-parent directory FD, and six ordered unlinked admitted member FDs; admission closes the candidate source-directory FD before returning; publication never consumes a source pathname or source-directory capability. Replay requires a privileged pre-opened append journal whose exact inode is embedded in trust. The production stage requires already-connected inherited Unix streams and exactly 43 canonical base64url signing-key bytes. Publication binds exact descriptor bytes/hash, source and parent identities, member hashes/sizes/modes, and durable terminal state. These contracts are inactive here. Issue #100 owns implementation of the Podman supervisor, broker lifecycle, connected channel provisioning, D1 begin/completion chronology, and transcript/output joins. Issue #101 depends on #100 and owns independent real-Linux conformance and admission proof; it cannot replace or skip #100 implementation.
