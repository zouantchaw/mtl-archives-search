...
Evidence keeps the merged #98 v1 semantics and IDs unchanged so the production TypeScript validator remains the oracle. Runtime transcript bytes are exactly JCS UTF-8 plus LF, and the v2 envelope hashes those retained bytes. Its signer is joined to an exact schema-pinned trust-entry artifact rather than an envelope-selected key. Evidence append/fsync faults after consumption permanently stop authority, release no unsupported UDS response, and cannot produce a signature over a nonterminal lifecycle. Raw `.bin` outputs preserve binary, newline, and empty bodies exactly. OCI assembly uses a fixed umask and explicit archived modes, with a local cross-umask synthetic proof. No admitted launch config or production authority is tracked; issue #101 must still supply the reviewed builder digest and independent Linux static ELF, sealed-memfd, sandbox, external TLS, and two-clean-source byte-identical OCI proof.
