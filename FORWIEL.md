...
Reviewed Metrics v2 is deliberately still a sealed candidate, not a completed evaluation. A future authority-only commit must bind its sole parent, change only the fixed authority path, and provide distinct visual predictor, search predictor, private evaluator, gold reviewer, task reviewer, implementation, and publisher identities with strict visual-then-search chronology. The `c0-rpcq` place gate now has a separate public search bundle/prediction/freeze and an uncommitted salted expected envelope. Only the deterministic private scorer's passing public receipt for that exact task can create place TP/support; visual rows and reviewer booleans cannot. Task review, final matrix, and publication pin and parse the full receipt/detail-hash/commitment/Gate E authority chain. No real authority, envelope, prediction, score, or publication exists.
