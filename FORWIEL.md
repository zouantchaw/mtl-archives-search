...
Evidence keeps the merged #98 v1 transcript unchanged so the production TypeScript validator remains the oracle. A successor v2 envelope signs the exact v1 bytes with Ed25519 and binds the D1 begin/attempt, session, broker/runtime/root pins, socket/token commitments, manifest, signer trust entry, and outcome. Terminal acceptance is withheld until those files are durable. Production launch admission is a detached Ed25519 signature over the complete strict config, checked against independently embedded trust, validity/sequence bounds, and durable replay state; ordinary builds embed neither code admission nor launch trust. Transport milestones are fsynced when DNS, TLS, and request transmission actually occur, and raw `.bin` outputs preserve binary, newline, and empty bodies exactly through the real broker/UDS/stage path. The OCI definition requires one separately reviewed digest-pinned builder image for all output-affecting tools and records no guessed digest. No admitted launch config or production authority is tracked. Issue #101 must supply that builder digest and independently run the static ELF, sealed-memfd, sandbox, external TLS, and two-clean-source byte-identical OCI proof on reviewed Linux.
