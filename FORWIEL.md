...
Reviewed Metrics v2 is deliberately still a sealed candidate, not a completed evaluation. A future authority-only commit must bind its sole parent, change only the fixed authority path, and provide distinct principals/sessions plus pairwise-distinct routes for visual prediction, search prediction, private evaluation, gold/dossier review, task review, and publication. The repo/history-denied search route receives only the pre-freeze neutral public task, opaque ID, generic official-domain policy, and bundle-local schema. Prediction freezes before a separate reviewer authors the dossier/envelope/commitment. Gate E history remains tracked/readable; salting protects integrity and workflow blinding, not secrecy from history readers. Only the deterministic private scorer's passing public receipt can create place support. No real authority, envelope, prediction, score, or publication exists.
