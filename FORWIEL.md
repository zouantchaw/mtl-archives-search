...
Evidence keeps the merged #98 v1 transcript unchanged so the production TypeScript validator remains the oracle. A successor v2 envelope signs the exact v1 bytes with Ed25519 and binds the D1 begin/attempt, session, broker/runtime/root pins, socket/token commitments, manifest, signer trust entry, and outcome. The launcher exists, but no admitted launch config or production authority is tracked. On Linux it requires fully sealed inherited memfds for the token, credentials, and signing seed. The remaining #101 work is evidence, not missing implementation: independently run the static ELF, sealed-memfd, sandbox, and two-clean-source byte-identical OCI proof on the reviewed Linux worker.
