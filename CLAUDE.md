...
Do not create or activate the issue #96 execution authority, run either prediction, author private expected data/gold, score, or publish from candidate work. A later v2.1 authority must bind its sole parent, change only the fixed authority path, remain byte-identical at `HEAD`/index/worktree, and name distinct principals/sessions plus pairwise-distinct routes for visual prediction, repo/history-denied source search, private evaluation, gold/dossier review, task review, and publication. The public search bundle must predate prediction and contain only the neutral prompt, opaque task ID, generic official-domain policy, and bundle-local schema. Freeze prediction before dossier/envelope/commitment authoring. Gate E history is tracked/readable; salting provides workflow blinding and integrity, not secrecy from history readers. Final place metrics derive only from the passing public receipt; task review must not author `private_score_passed`.
