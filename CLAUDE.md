...
Do not create or activate the issue #96 execution authority, run prediction, author gold, or publish from candidate work. Any later authority must declare its commit's sole parent as `candidate_commit`, change only the fixed authority path, remain clean and byte-identical across `HEAD`/index/worktree, be live at actual UTC execution/freeze time, and contain distinct role provenance with strictly increasing event times. A separate authority review must confirm that sole parent is the approved candidate HEAD. Normal scoring remains gated on that committed authority plus the exact freeze and completed gold; final matrix/publication also require canonical tracked input-authority bytes and issue #97's exact accepted `gate-h-v2:official-source-search:c0-rpcq` Gate E/RPCQ task, dossier/place join, private score pass, and independent review.
