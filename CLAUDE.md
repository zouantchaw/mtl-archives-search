...
Gate H2 broker/runtime work lives only in `crates/gate-h2-broker`. Keep Cargo locked and offline, never put destination manifests or credential bytes in stage inputs/evidence/errors, and do not enable the production launcher until issue #101 proves pinned TLS/PKIX, Linux static builds, OCI identity, and sandbox evidence. `npm run gate-h2:broker-self-test` is local-fixture-only and grants no production authority.
