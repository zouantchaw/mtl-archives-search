...
9. Verified Multimodal Intelligence Batch 001: tracked synthetic/hermetic contract foundation → bounded canonical-real candidate selector with approved byte pins, full derivative decode, and canonical joins → 26 mechanically eligible records (16 ranked visual-review pool + 10 reserves). The stage has zero verified claims or final selections and is not the completed issue #69 batch.
...
- Verified Multimodal Intelligence Batch 001 for issue #69 has a synthetic/hermetic contract foundation and a bounded canonical-real candidate selector. Keep the packet contracts under `packages/scripts/src/dataset-factory/verified-multimodal-batch-001-contract.ts`, selection code under `packages/scripts/src/dataset-factory/*real-pilot-candidate-selection-v1.ts`, schemas under `docs/dataset-factory/schemas/`, and tracked fixtures/descriptors under `docs/dataset-factory/fixtures/verified-multimodal-batch-001/`. The real selector pins approved recovery/graph bytes, fully decodes 209 derivatives, and currently yields 26 mechanically eligible records: 16 ranked visual-review candidates and 10 reserves. It has zero verified claims, dossiers, benchmark tasks, or final selections. Run `dataset-factory:real-pilot-candidates-v1`, `:verify-v1`, and `:integration-test-v1`; direct visual promotion review is the next gate. Do not claim the 60-record target is complete until reviewed dossiers, metrics, overlays, external verification, and independent review are present.
