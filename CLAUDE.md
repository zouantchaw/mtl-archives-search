...
Gate H2 broker/runtime work lives only in `crates/gate-h2-broker`. Keep Cargo and `oci/toolchain-lock.v1.json` locked after review, never put destination manifests or credential bytes in stage inputs/evidence/errors, and do not admit production authority until #101. Production stage communication is connected inherited Unix streams, replay is a privileged pre-opened append journal bound by trusted inode identity, signing keys are exactly 43 canonical unpadded base64url bytes, and request/credential descriptor cardinalities and bounds are checked before reads. Raw output is hashed from its retained FD. Export only `oci/source-allowlist.v1.txt`; the digest-pinned builder compiles helpers twice on `/gate-h2-build`, emits pass-specific digests, and leaves `/tmp` noexec. Host code must not compile helpers. It verifies/unlinks/retained-FD executes them and keeps admission/publication in one process. The publication capability contains one unlinked admission descriptor FD, one retained destination-parent directory FD, and six ordered unlinked admitted member FDs; admission closes the candidate source-directory FD before returning, and publication never consumes a source pathname or source-directory capability. Publication retains descriptor-hash-bound terminal state and classifies only observed exact no-replace conflict as conflict. Issue #100 retains supervisor and lifecycle implementation responsibility; issue #101 retains dependent independent real-Linux evidence responsibility. Frozen #98 bytes/IDs, canonical tar/OCI joins, complete host-derived SBOM, monotonic deadlines, and local-only tests remain mandatory.
