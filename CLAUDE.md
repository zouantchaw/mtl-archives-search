...
Gate H2 broker/runtime work lives only in `crates/gate-h2-broker`. Keep Cargo and `oci/toolchain-lock.v1.json` locked after review, never put destination manifests or credential bytes in stage inputs/evidence/errors, and do not admit a production launch configuration until issue #101 supplies independent Linux static, sealed-memfd, sandbox, external-TLS, and two-clean-build OCI evidence. Launcher builds embed only a reviewed code identity; the later exact config admission binds that identity and measured final binary without a hash cycle. The real rustls transport and launcher are implemented; `npm run gate-h2:broker-self-test` uses local loopback/TLS/UDS fixtures and grants no production authority.
