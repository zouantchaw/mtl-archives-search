...
Gate H2 broker/runtime work lives only in `crates/gate-h2-broker`. Keep Cargo and `oci/toolchain-lock.v1.json` locked after review, never put destination manifests or credential bytes in stage inputs/evidence/errors, and do not admit a production launch configuration until issue #101 supplies a reviewed builder-image digest plus independent Linux static, sealed-memfd, sandbox, external-TLS, and two-clean-build OCI evidence. Evidence durability is fail-sticky after handle consumption; unsupported responses must not be released and nonterminal evidence must not be signed. V2 validation must load exact schema-pinned signer trust-entry bytes/key and hash the exact canonical-LF transcript file. OCI paths use a fixed umask and explicit modes. `npm run gate-h2:broker-self-test` uses only local fixtures and grants no production authority.
