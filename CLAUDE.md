...
Gate H2 broker/runtime work lives only in `crates/gate-h2-broker`. Keep Cargo locked after dependency review, never put destination manifests or credential bytes in stage inputs/evidence/errors, and do not admit a production launch configuration until issue #101 supplies independent Linux static, sealed-memfd, sandbox, and two-clean-build OCI evidence. The real rustls transport and launcher are implemented; `npm run gate-h2:broker-self-test` uses local loopback/TLS/UDS fixtures and grants no production authority.
