...
Do not create or activate the issue #96 execution authority, run either prediction, author private expected data/gold, score, or publish from candidate work. A later v2.1 unified authority must bind its sole parent, change only the fixed authority path, remain byte-identical at `HEAD`/index/worktree, and name distinct visual predictor, search predictor, private evaluator, gold reviewer, task reviewer, implementation, and publisher identities/routes with strict visual-then-search chronology. Source-search scoring requires the exact public bundle/prediction/freeze plus an uncommitted 32-byte-salted expected envelope and exact Gate E/RPCQ pins. Final place metrics derive only from the passing public private-score receipt; task review must pin its private-detail hash/bytes and must not author `private_score_passed`.
