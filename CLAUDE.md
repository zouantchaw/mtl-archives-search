...
Do not create or activate the issue #96 execution authority, run prediction, author gold, or publish from candidate work. Any later authority must be the direct child of reviewed HEAD `8eb42713059622a5c94a092da5e5a2141463c85f`, change only the fixed authority path, remain clean and byte-identical across `HEAD`/index/worktree, be live at actual UTC execution/freeze time, and contain distinct role provenance with strictly increasing event times. Normal scoring remains gated on that committed authority plus the exact freeze and completed gold; final matrix/publication additionally require issue #97 positive reviewed place support and observed place metrics.
