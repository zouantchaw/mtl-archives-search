...
Gate H2 broker/runtime work lives only in `crates/gate-h2-broker`. Keep Cargo and `oci/toolchain-lock.v1.json` locked after review, never put destination manifests or credential bytes in stage inputs/evidence/errors, and do not admit a production launch configuration until issue #101 supplies a reviewed builder-image digest plus independent Linux static, sealed-memfd, sandbox, external-TLS, and two-clean-build OCI evidence. Production launcher builds must embed both a reviewed code identity and an independent Ed25519 launch-authority trust entry; the detached signature covers the complete final-binary-pinned config with validity, sequence, and replay controls. The real rustls transport and launcher are implemented; `npm run gate-h2:broker-self-test` uses local loopback/TLS/UDS fixtures and grants no production authority.
