...
function MapZoomControl({ className, ...props }: React.ComponentPropsWithoutRef<"div">) {
    const map = useMap()
    ...
    return (
        <MapControlContainer className={cn(className ?? "top-1 left-1")}>
            <ButtonGroup
...
function MapDrawUndo({ className, ...props }: React.ComponentPropsWithoutRef<"button">) {
    const drawContext = useMapDrawContext()
    if (!drawContext)
