...
export function collectionSummary(
  ...
  const aside = photos.filter((p) => p.visualCheck.status === "no_match").length;
  if (lang === "en")
    return `${checked} candidate photographs were visually checked. ${matches.length ? `${matches.length} appear to match your description ${citations}.` : "None of the checked photographs were a clear object match."}${uncertain ? ` ${uncertain} displayed ${uncertain === 1 ? "photo needs" : "photos need"} a closer look.` : ""}${aside ? ` ${aside} ${aside === 1 ? "was" : "were"} labeled as not showing those objects and ${aside === 1 ? "stays" : "stay"} on the wall.` : ""}${failed ? ` ${failed} check${failed === 1 ? "" : "s"} failed technically and ${failed === 1 ? "is" : "are"} not counted as ${failed === 1 ? "a match" : "matches"}.` : ""} This is an AI assessment of a small candidate set.`;
  return `${checked} photographies candidates ont été examinées visuellement. ${matches.length ? `${matches.length} semblent correspondre à votre description ${citations}.` : "Aucune des photographies vérifiées n’est une correspondance d’objet claire."}${uncertain ? ` ${uncertain} photographie${uncertain > 1 ? "s" : ""} affichée${uncertain > 1 ? "s" : ""} reste${uncertain > 1 ? "nt" : ""} à examiner.` : ""}${aside ? ` ${aside} ${aside > 1 ? "sont étiquetées" : "est étiquetée"} comme ne montrant pas ces objets et restent sur le mur.` : ""}${failed ? ` ${failed} vérification${failed > 1 ? "s" : ""} a${failed > 1 ? "ont" : ""} échoué${failed > 1 ? "s" : ""} et n’est pas comptée comme correspondance.` : ""} Il s’agit d’une appréciation par IA sur un petit échantillon.`;
}
