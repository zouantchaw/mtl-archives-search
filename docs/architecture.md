...
Gold Label Batch 002 separates hidden acquisition state from neutral pixel review. Worker rows contain neutral IDs, pixel-evidence hashes, detail limitations, and four bounded orientation views only. Primary and blind rows use an abstention-capable pixel schema, disjoint paths, independent reviewer IDs, and hash-bound pass seals. Record/source/right mapping occurs only in trusted adjudication after both passes seal. Completion is now reviewed and validated for issue #68: metrics are derived from rows, adjudication-change rates are denominator-defined over the 300-row batch, and validation fails closed on schema, packet, view, pass, mapping, audit, support, shortfall, search-authority drift, or report literal drift. The ignored local evidence stays under `data/mtl_archives/reports/gold_label_batch_002`; durable publication is the separate R2 archive `r2://wiel-codex-worker-cache/artifacts/mtl-archives/gold-label-batch-002/9a36710f1b658f48e6afef1a05ebd29419320903734f5febab75d08b0f5c1f43.tar.gz` with SHA-256 `9a36710f1b658f48e6afef1a05ebd29419320903734f5febab75d08b0f5c1f43`.
