...
Reviewed Metrics v2 adds a Git-backed activation boundary above the blind bundle: the fixed execution authority must exist identically at `HEAD`, index, and worktree and bind candidate `73ec8a9acbbbc6cd8713f2ae2d944ca2aebd7140`. That committed authority is the source of role/session/model provenance for predictor, gold reviewer, task reviewer, publisher, implementer, and forbidden prior reviewers. Downstream completed artifacts are accepted only through exact fixed universes, role-specific pins, file-backed predecessor receipts, and strictly increasing timestamps.
