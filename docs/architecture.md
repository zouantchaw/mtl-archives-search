...
Gold Label Batch 002 separates hidden acquisition state from visual review packets. Selection manifests retain record IDs, V1 leakage components/splits, metadata-derived signals, and rationale. Worker packet rows contain only neutral IDs and bounded local image evidence. Primary and blind outputs use disjoint paths and reviewer identities; adjudication consumes both only after each pass is frozen. The completion validator fails closed on packet/schema/image drift, hidden-field leakage, component/split conflicts, unsupported promotion, or omitted silver dispositions.
