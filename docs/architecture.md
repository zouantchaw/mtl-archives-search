...
Reviewed Metrics v2 adds a Git-backed activation boundary above the blind bundle: the activation commit must be the direct authority-only child of reviewed HEAD `8eb42713059622a5c94a092da5e5a2141463c85f`, change exactly the fixed authority path, remain repository-clean, and expose identical authority bytes at `HEAD`, index, and worktree during its live UTC interval. That committed authority fixes role/session/model provenance. The freeze pins the prediction's exact raw bytes; scoring parses those bytes and exact gold bytes to rederive every confusion/edit/selective-error count. Completed task, matrix, and publication validators then schema- and semantic-validate their exact role-labelled files. Final publication remains blocked until issue #97 supplies positive reviewed place-opportunity support and observed place metrics.
