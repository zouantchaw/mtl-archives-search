...
The network client trait exists only inside the crate for deterministic local fixtures. The public broker constructor accepts the concrete production client, which performs resolve-once DNS policy, pinned-peer TCP, rustls TLS 1.3 SNI/PKIX, and exact HTTP framing. `crates/gate-h2-broker/scripts/build-stage-oci.sh` retains the clean Linux/musl/two-build/Podman/rootfs/SBOM/provenance checks as issue #101 evidence gates. No admitted launch config, runtime, or image from this tree is production-authorized.
...
The merged issue #98 v1 transcript is preserved as the admission oracle. The issue #99 v2 Ed25519 authority envelope references and signs its exact bytes while adding D1 attempt/begin, session/attempt, broker/runtime/root, socket/token, manifest, signer-trust, and final-outcome bindings. Terminal UDS acceptance is withheld until both are durably sealed. Launcher admission separates the embedded reviewed code identity from the later exact config ID, which binds the measured final broker binary and therefore remains constructible without weakening either identity. Native roots are joined by hash, byte count, and canonical serialization version; stage receipts are joined to the exact request, authority role/status, and retained raw bytes. A checked toolchain identity lock is part of build provenance. No admitted launcher config exists in the repository, so this implementation is production-shaped but inactive. Issue #101 is limited to independent real-Linux static, memfd, sandbox, external TLS, and reproducible OCI execution evidence.
