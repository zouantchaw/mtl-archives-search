...
Issue #100 is merged and complete as an inactive synthetic Podman supervisor contract. Issue #101 Packets 1 and 2 remain merged historical synthetic/local contracts. Packet 3A is the approved local, no-secrets, pre-launch implementation contract; see [`gate-h2-builder-packet-3a-v1.md`](dataset-factory/gate-h2-builder-packet-3a-v1.md). The frozen Packet 2 four-file fixture and pending recipe remain byte-for-byte historical evidence and do not define the future runtime floor. Packet 3A is not real Linux evidence or a completed Issue #101 build. The remaining boundary is reviewed immutable external pins, an offline image, two independent real x86 Linux builds, receipt comparison, worker termination, and later #101 Podman/network/lifecycle/broker/D1/admission packets.
