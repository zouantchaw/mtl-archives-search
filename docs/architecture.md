...
The network client trait exists only inside the crate for deterministic local fixtures. The public broker constructor accepts the concrete production client, which performs resolve-once DNS policy, pinned-peer TCP, rustls TLS 1.3 SNI/PKIX, exact HTTP framing, and synchronous fsynced DNS/TLS/request-sent milestones. `crates/gate-h2-broker/scripts/build-stage-oci.sh` uses host Podman only to start one network-disabled, digest-pinned reviewed builder image; compilation, inspection, rootfs normalization, and deterministic OCI layout/archive assembly all occur inside that image. No builder digest, admitted launch config, runtime, or image from this tree is production-authorized.
...
The stage remains raw-network denied and receives only exact request artifacts, opaque one-use handles, a one-run token, and an owner-bound Unix socket. The host launcher verifies an admitted config and exact binary/runtime/root pins before constructing the broker. DNS resolution, complete special-use rejection, peer selection, exact socket connect, connected-peer verification, TLS 1.3 SNI/PKIX, credentials, HTTP framing, deadlines, response caps, raw output commit, and signed evidence remain inside the host process.

The merged issue #98 v1 transcript is preserved as the admission oracle. The issue #99 v2 Ed25519 evidence envelope references and signs its exact bytes while adding D1 attempt/begin, session/attempt, broker/runtime/root, socket/token, manifest, signer-trust, and final-outcome bindings. Terminal UDS acceptance is withheld until both are durably sealed; a nonterminal response-write failure consumes the next authorized exchange and seals a valid failed transcript. Launcher admission separately embeds reviewed code identity and Ed25519 authority trust, then verifies a detached signature over the complete canonical config, including the measured final broker binary, validity window, monotonic sequence, and replay identity. Native roots are joined by hash, byte count, and canonical serialization version; stage receipts join the exact request, authority role/status, and reopened raw `.bin` bytes, including empty output. The hermetic builder lock and supplied reviewed image digest are retained in provenance. No builder digest, authority trust, or admitted launcher config exists in the repository; issue #101 supplies and observes those real-Linux static, memfd, sandbox, external TLS, and reproducible OCI inputs/results.
