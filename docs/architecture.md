...
Reviewed Metrics v2 adds a Git-backed unified activation boundary above two isolated prediction runs. Its v2.1 authority requires distinct principals/sessions and pairwise-distinct filesystem routes for visual prediction, repo/history-denied source search, private evaluation, gold/dossier review, task review, and publication. The portable public search bundle exists before prediction and contains only a neutral prompt, opaque task ID, generic HTTPS Quebec-government hostname-suffix policy, and bundle-local schema bytes. The source prediction freezes before the dossier reviewer authors the private envelope and commitment. The scorer dynamically joins that post-freeze authority to tracked/readable Gate E history; the salt protects workflow blinding and integrity rather than claiming secrecy from history readers. Final matrix and the 15-member public publication parse both freezes, receipt/detail hash/commitment, task, dossier pins, authorities, metrics, and canonical tracked input authority; private envelope/detail are excluded.
