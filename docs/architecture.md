...
| `apps/web` | Optional snapshot explorer. Shared `/api/search` (`mode=smart` by default). Map proximity is model similarity, not geography. Legacy R2 files load until a versioned manifest is published. See [explorer-modernization.md](explorer-modernization.md). |
