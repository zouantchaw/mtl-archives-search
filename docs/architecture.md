...
Reviewed Metrics v2 adds a Git-backed unified activation boundary above two isolated runs. The v2.1 authority retains the visual predictor fields, adds distinct search-predictor/private-evaluator identities and routes, and requires visual start/end/freeze before source-search start/end/freeze. The source-only `c0-rpcq` chain binds a sanitized public bundle, raw prediction bytes, source-search freeze, uncommitted domain-separated salted expected envelope, private score detail, non-leaking public receipt, exact Gate E/RPCQ provenance, and independent task review. Place rows derive only from a passing receipt for the exact task. Final matrix and the 14-member public publication parse and semantically join both freezes, the receipt/detail hash/commitment, task, dossier pins, authorities, metrics, and canonical tracked input authority; private envelope/detail are excluded from publication.
