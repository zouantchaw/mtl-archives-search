...
The merged issue #98 v1 transcript semantics and IDs are preserved as the admission oracle. Runtime transcript files are exactly JCS UTF-8 plus LF, and the issue #99 v2 Ed25519 envelope hashes those retained bytes. Validation loads the exact schema-pinned signer trust-entry bytes and joins their signer ID/public key before signature verification. Any evidence append/fsync failure after a handle is consumed permanently closes authority; the UDS closes without an unsupported response, later manifest exchanges cannot run, and sealing rejects nonterminal or invalid evidence. A nonterminal response-write failure still consumes the next authorized exchange and seals a valid failed transcript. OCI assembly fixes the umask and all archived modes; local cross-umask assembly is proven, while the full Linux build remains issue #101. No builder digest, authority trust, or admitted launcher config exists in the repository.
