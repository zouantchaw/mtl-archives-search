...
The network client trait exists only inside the crate for deterministic local fixtures. The concrete production client performs resolve-once DNS policy, pinned-peer TCP, rustls TLS 1.3 SNI/PKIX, exact HTTP framing, and fsynced milestones. The exact source allowlist enters a digest-pinned builder where host helpers are compiled twice on a dedicated executable tmpfs; `/tmp` remains noexec. Host code compares the pass artifacts and manifest, unlinks executable helper paths on Linux, and strict-environment retained-FD executes them without runtime compilation. Admission and publication share one host process. Their publication capability contains one unlinked admission descriptor FD, one retained destination-parent directory FD, and six ordered unlinked admitted member FDs. Admission closes the candidate source-directory FD before returning, and publication never consumes a source pathname or source-directory capability. Replay derives high-water from a privileged pre-opened append journal whose inode identity is embedded in local trust. Issue #100 owns supervisor and lifecycle implementation; issue #101 owns dependent independent real-Linux evidence. No builder digest, privileged supervisor, admitted launch config, runtime, or image from this tree is production-authorized.

Issue #100 is merged and complete as an inactive synthetic Podman supervisor contract. Issue #101 Packet 1 freezes the 69-case Linux/Podman/D1/admission universe and the 71-file registered synthetic fixture `dfv0_gate_h2_linux_conformance_v1_20260806`. Packet 2 completes the local synthetic/pending/nonproduction builder-receipts reproducibility contract: [`dfv0_gate_h2_builder_receipts_v1_20260806`](dataset-factory/gate-h2-builder-receipts-v1.md) is exactly 4 files, 17,539 bytes, SHA-256 `c9e2b0764b1070b479836abae1c7bd2fa362ad95710e3bd02f74c23c535a6688`, with 81 ordered adversarial cases. Immutable source identity comes from pinned Git objects at `4ddf00e812610e3e029059f25ad3d951577f667d`; external builder image/vendor/trust/tool-artifact pins remain intentionally pending/null. The synthetic two-build receipts and comparison exercise the future contract only, not real builds, Linux/Podman evidence, or production admission. No paid host, Podman execution, D1 access or mutation, credential, provider/model call, activation, publication, or deploy occurred. The next boundary is explicit approval to pin external inputs, run independent real Linux/Podman builds, then perform real conformance/D1 admission.
