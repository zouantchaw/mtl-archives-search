...
- `packages/scripts/src/canonical-corpus-v1/` normalizes identities, captures read-only snapshots, assigns one primary state per observed identity, and checks the observed-universe arithmetic independently.
...
- R2 is separated into archive images, social content, content assets, and other objects. Key/size/ETag/last-modified coverage is exact; HEAD/content-type and 32-byte magic evidence is a deterministic bounded sample.
