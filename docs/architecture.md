...
- Cross-platform content correlation runs through `packages/scripts/src/analysis/cross-platform-content-analytics.ts`. It accepts external Meta and Vercel export roots, assigns posts to America/Toronto calendar dates, joins current post metrics to daily/monthly Meta account metrics and monthly Vercel page/event tables, and writes structured JSON/CSV/Markdown outputs without copying private analytics into Git.
