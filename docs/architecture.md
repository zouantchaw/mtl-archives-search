...
The merged issue #98 v1 transcript is preserved as the admission oracle. The issue #99 v2 Ed25519 authority envelope references and signs its exact bytes while adding D1 attempt/begin, session/attempt, broker/runtime/root, socket/token, manifest, signer-trust, and final-outcome bindings. No admitted launcher config exists in the repository, so this implementation is production-shaped but inactive. Issue #101 is limited to independent Linux static, memfd, sandbox, and reproducible OCI evidence.
