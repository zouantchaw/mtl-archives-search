...
- Issue #70 Benchmark v1 is currently a pre-lock candidate/preflight layer only. `dataset-factory:benchmark-v1-candidate` binds exact input IDs/hashes, audits Visual Family Graph component splits, separates reviewed-gold/silver/stress retrieval rows, and emits a human-review queue; it cannot assert lock authority or issue completion.
- Issue #71 adds `dataset-factory:promotion-challenge-preflight-v1`, a permanently no-ship, offline scaffold that checks the #70 fail-closed report shape and enumerates the evidence required before any reranker challenge. It runs no model/GPU/production operation and records no promotion result.
...
`crates/gate-h2-broker` is a separate locked Rust package. A stage remains `--network none` and sends schema-defined requests only over connected Unix streams inherited at fixed FD positions; production never validates a pathname socket and reconnects through a second lookup. The host broker admits an exact schema-bound capability in ordinal order, verifies token/peer/request pins before upstream I/O, inserts only the reviewed typed auth header from an inherited descriptor, commits and hashes raw output through the retained file FD, fsyncs hash-only lifecycle events, and seals a signed transcript.

The network client trait exists only inside the crate for deterministic local fixtures. The concrete production client performs resolve-once DNS policy, pinned-peer TCP, rustls TLS 1.3 SNI/PKIX, exact HTTP framing, and fsynced milestones. Packet 3A's future builder uses an image-owned source proof and sealed measured snapshot, an exact root-owned runtime closure, and Rust/Cargo `1.85.0` plus Node `22.22.0`; it builds static ELF64 little-endian x86-64 output with offline Cargo/vendor/config, musl, CMake, and Ninja. Rootless Podman uses keep-id mapping and a proof supervisor with only `SETUID`, `SETGID`, and `SETPCAP` before the exact zero-capability `65532:65532` child. Exactly two host helper families are compiled twice and host-admitted, while host code never compiles helpers. Expected SBOM bytes come from Cargo tree and candidate SBOM bytes from Cargo metadata; exact disagreement fails. Admission and publication share one host process. Issue #100 owns supervisor and lifecycle implementation; Issue #101 owns the dependent independent real-Linux evidence. No external builder digest, privileged supervisor, admitted launch config, runtime, or image from this tree is production-authorized.

Issue #100 is merged and complete as an inactive synthetic Podman supervisor contract. Issue #101 Packets 1 and 2 remain merged historical synthetic/local contracts. Packet 3A is the approved local, no-secrets, pre-launch implementation contract; see [`gate-h2-builder-packet-3a-v1.md`](dataset-factory/gate-h2-builder-packet-3a-v1.md). Packet 3B now defines strict external-input-lock and builder-receipt schemas/validators plus Linux-only descriptor-anchored byte-tree verification and an intentional offline stop; see [`gate-h2-builder-packet-3b-input-lock-v1.md`](dataset-factory/gate-h2-builder-packet-3b-input-lock-v1.md). No actual lock, image, build, or receipt has been produced. The remaining boundary is reviewed immutable external pins, offline image materialization, two independent real x86 Linux builds, receipt comparison, worker termination, and later #101 Podman/network/lifecycle/broker/D1/admission packets.
...
- Cross-platform content correlation runs through `packages/scripts/src/analysis/cross-platform-content-analytics.ts`. It accepts external Meta and Vercel export roots, assigns posts to America/Toronto calendar dates, joins current post metrics to daily/monthly Meta account metrics and monthly Vercel page/event tables, and writes structured JSON/CSV/Markdown outputs without copying private analytics into Git. The report requires explicit `real_export` or `synthetic_fixture` provenance, rejects `no_personal_data` raw queries/candidate lists, requires exact permalinks for published identity joins, and labels aggregate `captured_at` as report-generation time because source-capture timestamps are not available. Product event IDs are checked only within one export; durable cross-export deduplication remains an external ingestion/instrumentation gate.
- That report now requires the local `content-signal-v1` identity contract: an explicit platform-post → package → canonical-record/visual-family/source-asset map plus the canonical manifest. Optional product events are validated as `product_behavior` / `reward_not_fact` signals, with randomized-experiment propensities and safety budgets required. Missing or ambiguous joins fail closed; see `docs/dataset-factory/content-signal-v1.md`.
...
Reviewed Metrics v2 remains candidate-only for issue #96. Activation still requires the authority-only direct-child commit and exact clean-parent controls. Every consequential command receives a fresh stage-specific coordinator receipt, actual wall-clock validation, and an `O_EXCL`, fsynced durable start/completion marker pair that rejects cross-process replay before side effects.
...
## Gate H2 HTTPS Broker

The stage remains raw-network denied and receives only exact request artifacts, opaque one-use handles, a one-run token, and an owner-bound Unix socket. The host launcher verifies an admitted config and exact binary/runtime/root pins before constructing the broker: the broker pin comes from the kernel-bound running executable object, while the stage runtime is opened no-follow and retained after stable owner/type/link/mode/size/digest/inode validation. DNS resolution, complete special-use rejection including AS112-v4 `192.31.196.0/24` and IPv4-mapped aliases, peer selection, exact socket connect, connected-peer verification, TLS 1.3 SNI/PKIX, credentials, HTTP framing, deadlines, response caps, raw output commit, and signed evidence remain inside the host process.

The merged issue #98 v1 exchange/transcript semantics and IDs remain the admission oracle. Runtime transcript files are exactly JCS UTF-8 plus LF, and validation joins the exact schema-pinned signer trust entry. Terminal response delivery uses a runtime-only second UDS acknowledgement bound to owner UID, one-run token, manifest/capability/order, request ID, exact serialized response, output role/hash/bytes/status, and durable receipt. The stage accepts before sending it; only then may the broker seal complete. The later 204 is optional, while initial delivery or invalid/missing ACK replaces the last pre-seal committed-response event and seals failed. Nonterminal delivery failure retains the frozen next-capability consumption behavior. All request reads share an absolute monotonic deadline. Trusted host orchestration performs Git/cleanliness checks and measures the archive; before either pass, a separately delivered verifier performs capped no-follow measurement of the exact mounted allowlist, compares path/mode/size/hash/count/manifest identity with the host descriptor, copies only those measured bytes to a builder-owned snapshot, and remeasures it. Both passes build that snapshot; the inner builder has no `.git`, Git dependency, final-parent access, or admission authority. Host admission cross-joins source, toolchain/image, binaries, rootfs/OCI, trust roots, and SBOM before OS-level atomic no-replace publication; post-rename parent-fsync failure leaves explicit inspection state. Issue #101 exclusively owns independent real-Linux/Podman conformance and admission; no authority is activated here.

Issue #104 adds an independent source-pinned post-begin enrollment contract before the future D1 begin. Its fixed descriptor binds exact enrollment/schema/migration bytes, and the migration blocks malformed values plus replacement/upsert bypasses. The document freezes relay/verifier SPKI, fixed D1 account/database/endpoint/coordinator-capability identities, and native retained-descriptor identities for request bytes, endpoint, replay journal, report channel, authority/launch, and completion expectations. The one production entry point derives that root itself and returns inactive for this synthetic fixture; it does not use a caller adapter or JavaScript object as authority. #100 must validate actual inherited FDs and bind them through the coordinator-owned native boundary. A missing completion readback is indeterminate rather than a begin-only failure; the future supervisor must reconcile the exact immutable attempt, accept only its exact completion row, and never automatically rerun the no-row case.
...
Offline #148 Gateway vision comparison (`openai/gpt-5.4`, `xai/grok-4.6`) is documented in [vision-gateway-v1/README.md](vision-gateway-v1/README.md). Frozen OCR/orientation eval is in [vision-eval-v1/README.md](vision-eval-v1/README.md). Image-kind and orientation provenance is in [orientation-eval-v1/README.md](orientation-eval-v1/README.md). OCR-as-separate-evidence is in [ocr-eval-v1/README.md](ocr-eval-v1/README.md). Versioned ingest and candidate indexes are in [ingest-v1/README.md](ingest-v1/README.md). Eve vs Cloudflare runtime split is in [runtime-v1/README.md](runtime-v1/README.md). Second-source onboarding is in [onboard-v1/README.md](onboard-v1/README.md). Operator D1 jobs and the Eve/Gateway agent are in [operator-v1/README.md](operator-v1/README.md). Budgeted GPU jobs are in [gpu-v1/README.md](gpu-v1/README.md). Selective frontier fallback (sideways/documents only) is in [fallback-v1/README.md](fallback-v1/README.md). Query-time `/research` inspect fallback is in [inspect-fallback-v1/README.md](inspect-fallback-v1/README.md). Candidate captions stay inactive.
