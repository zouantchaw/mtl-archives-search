...
|------|----------|--------------|
| **Visual** | Single concepts, objects, scenes | Server-side CLIP matches your query against image embeddings |
...
2. It's converted to a 512-dimensional vector using CLIP (via HuggingFace Inference API)
3. This vector is compared against pre-computed image embeddings in Vectorize
...
- **CLIP Model**: `clip-ViT-B-32` (512 dimensions), runs server-side via HuggingFace Inference API
