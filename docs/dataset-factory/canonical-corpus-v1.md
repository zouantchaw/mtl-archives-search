...
- For a record present locally and in D1, the normalized local primary source is the record-specific source identity. All distinct normalized local/D1 URLs remain in `source_urls`; local datasets, source record IDs, name, description, cote, and attribution take precedence, with D1 used only as a fallback. This boundary prevents D1 disagreement from silently redefining a local identity.
- Every alias carries deterministic `source-group:<sha256(normalized-source)>`, the rule reason, and the complete sorted member list. The checker reconstructs every group from verified raw rows and requires exactly one D1-backed target; generated rows never define their own alias authority.
