...
The generic `dataset-factory:verified-dossiers-self-test-v1` command is valid before and after that pin-only activation. In the unconfigured state it verifies null authority fields and fail-closed normal CLI behavior. In the active state it reads only the committed pin, validates its strict candidate descriptor, authorization file hash/bytes, reviewer, authorizing authority, and timestamp bindings, and continues to require rejection of synthetic authorization bytes. It does not read or require the coordinator's external authorization file; exact authorization-file verification remains the responsibility of normal `validate-review`, `publish`, and `verify-published` execution.
