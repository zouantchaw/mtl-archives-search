...
The frozen v1 semantic object and identifiers remain unchanged. A runtime transcript artifact has exactly `UTF8(JCS(transcript)) || LF`; the v2 `transcript_sha256` covers those retained bytes, and authority validation rejects whitespace, missing/extra newlines, or reformatting. The envelope signer digest is not a caller assertion: validation loads the exact schema-pinned signer trust-entry artifact, hashes its bytes, joins its `signer_id` and Ed25519 public key to the envelope, and verifies with that trusted key.
