...
Custom layouts run cosine UMAP in a disposable Web Worker, with explicit Apply, phase/epoch progress, and immediate termination on cancellation. The last successful layout stays visible until the replacement has passed finite-coordinate and ID coverage checks. A bounded eight-layout memory and IndexedDB cache avoids repeat work, including reloads. Cached coordinates are validated against the snapshot, engine version, settings, IDs and finite-coordinate checks. A custom-layout link without a valid local cache opens the published map and asks whether to restore the custom layout; it never starts UMAP automatically on reload. Storage failures leave the map usable. All axes use one common scale, preserving the projection's aspect ratio. A layout change preserves the query, selected photograph and saved collection; it does not change search ranking or underlying vector similarities.
...
The search field contains its retrieval selector: Combined (image similarity plus catalogue text) or Image similarity (text-to-image matching). The API/URL values remain `smart` and `visual` for existing links. Map controls and contextual workspace actions wrap within the available width, including when Export appears after selecting a photograph.

Closing a photograph or resetting the view clears its selection. Unknown photo IDs do not create placeholder selections; similarity-web anchors are distinct from selected photographs.
